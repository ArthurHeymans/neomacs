//! Build script for neomacs-display-runtime.
//!
...
fn main() {
    ...
    generate_application_identity(&PathBuf::from(&crate_dir), &out_dir);
...
/// Make the installed desktop entry the single source of truth for the
/// runtime identity advertised to Linux compositors.
fn generate_application_identity(crate_dir: &std::path::Path, out_dir: &std::path::Path) {
