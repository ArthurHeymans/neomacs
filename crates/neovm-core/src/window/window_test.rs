...
/// Redisplay reads four window parameters (the three chrome-format overrides
/// and the cursor effect); a write to any other parameter is invisible to it.
/// GNU never treats `set-window-parameter` as a redisplay event, and Lisp
/// entered from redisplay rewrites its own bookkeeping parameters on every
/// evaluation (`tab-line-format` stores `tab-line-cache` and
/// `tab-line-buffers`), so counting every write made each layout attempt that
/// evaluated a tab line invalidate itself.
#[test]
fn window_layout_inputs_track_only_the_parameters_redisplay_reads() {
    let mut manager = FrameManager::new();
    let frame_id = manager.create_frame("layout-parameters", 800, 600, BufferId(1));
    let window = manager.get(frame_id).expect("frame").selected_window;
    let inputs = |manager: &FrameManager| {
        manager
            .get(frame_id)
            .expect("frame")
            .window_layout_inputs(window)
            .expect("layout inputs")
    };

    let before = inputs(&manager);
    manager.set_window_parameter(
        window,
        Value::symbol("tab-line-buffers"),
    ...
    manager.set_window_parameter(
        window,
        Value::symbol("tab-line-cache"),
        Value::cons(Value::NIL, Value::string("tab")),
    );
    assert_eq!(
        inputs(&manager),
        before,
        "bookkeeping parameters are not layout inputs"
    );
    ...
    for parameter in [
        ...
        let before = inputs(&manager);
        manager.set_window_parameter(window, Value::symbol(parameter), Value::string("changed"));
        assert_ne!(
            inputs(&manager),
            before,
            "{parameter} is read by redisplay, so writing it changes the layout inputs"
        );
    }
}
