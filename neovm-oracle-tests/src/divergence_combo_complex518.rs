...
"##,
        expect_test::expect![[r#""ERR (error \"Invalid hash table test\" equalp)""#]],
    );
}
...
"##,
        expect_test::expect![[r#""ERR (void-function hash-table-values)""#]],
    );
}
...
fn div_cx518_hash_table_iterate() {
  ...
  (let (results)
    ...
    results))
"##,
        expect_test::expect![[r#""OK ((\"b\" . 2) (\"a\" . 1))""#]],
    );
}
...
"##,
        expect_test::expect![[r#""OK (val1 nil)""#]],
    );
}
...
fn div_cx518_hash_table_custom_test() {
  ...
  (error (car e)))
"##,
        expect_test::expect![[r#""OK (= (closure (t) (x) (abs x)))""#]],
    );
}
...
fn div_cx518_hash_table_plist_to_hash() {
  ...
  (let ((ht (alist-to-hash-table '((a . 1) (b . 2) (c . 3)))))
    (hash-table-count ht)))
"##,
        expect_test::expect![[r#""ERR (void-function alist-to-hash-table)""#]],
    );
}
