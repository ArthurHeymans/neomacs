//! Combo: cl-remove-if/cl-delete-if + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests sequence removal predicates with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_remove_if_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "rib")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((lst (list 'a 'b 'c 'd 'b 'a))
               (result (cl-remove-if (lambda (x) (eq x 'b)) lst))
               (result2 (cl-remove 'c lst)))
...
#[test]
fn combo_cl_delete_if_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "dic")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "dic-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((lst (list 'a 'b 'c 'b 'a))
                 (result (cl-delete-if (lambda (x) (eq x 'b)) lst)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
...
#[test]
fn combo_cl_remove_if_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "ri1"))
        (b2 (generate-new-buffer "ri2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b2
        (undo-boundary)
        (let* ((lst (list 'd 'e 'f 'e 'd))
               (result (cl-remove-if (lambda (x) (eq x 'e)) lst)))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_remove_if_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_delete_if_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
