...
fn divergence_bignum_expt() {
  ...
  (list big
        ...
        (integerp big)))"#,
        expect_test::expect![[r#""OK (1267650600228229401496703205376 t t)""#]],
    );
}
...
fn divergence_bignum_division() {
  ...
  (list result
        (= result (expt 10 25))
        (mod big div)))"#,
        expect_test::expect![[r#""OK (10000000000000000000000000 t 0)""#]],
    );
}
...
fn divergence_float_bignum_mixed() {
  ...
  (floatp (float (expt 2 100))))"#,
        expect_test::expect![[r#""OK (1.2676506002282294e+30 t t t)""#]],
    );
}
...
fn divergence_ash_bitwise_shift() {
  ...
  (ash -1 8))"#,
        expect_test::expect![[r#""OK (1024 0 4611686018427387902 -256)""#]],
    );
}
...
fn divergence_logand_logior() {
  ...
  (lognot 0)
  (lognot -1))"#,
        expect_test::expect![[r#""OK (15 255 240 -1 0)""#]],
    );
}
...
fn divergence_logbitp() {
  ...
  (logcount 255)
  (integer-length 255))"#,
        expect_test::expect![[r#""ERR (void-function logbitp)""#]],
    );
}
