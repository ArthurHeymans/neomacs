//! Complex combo batch 300 — MEGA MILESTONE: 5 extreme integration stress
...
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX300=v1" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX300"))))
      (exit-code (let ((p (make-process :name "neo-cx300-ec1"
                                          :command '("sh" "-c" "exit 5")))
      ...
      (rec (record 'neo-cx300-tag :a :b :c))
      (lexical-binding t)
      (calls nil))
  ...
  (puthash "alpha" 1 ht)
  (puthash "beta" 2 ht)
  (defun neo-cx300-target (x) (push (list :primary x) calls) (* x 2))
  (advice-add 'neo-cx300-target :before
              (lambda (x) (push (list :before x) calls))
              '((name . mega-advice)))
  (let* ((text "café 世界 😀 mega-300-milestone")
    ...
    (let ((buf (get-buffer-create " *neo-cx300-mega-1*")))
      (with-current-buffer buf
        (buffer-enable-undo)
        (setq-local neo-cx300-counter 0)
        (insert text)
        (put-text-property 1 4 'face 'bold)
        (put-text-property 6 10 'display "XX")
        (add-to-invisibility-spec 'neo-cx300-h)
        (let ((m (set-marker (make-marker) 8))
              (invis-ov (make-overlay 3 8))
              (face-ov (make-overlay 6 14)))
          (overlay-put invis-ov 'invisible 'neo-cx300-h)
          (overlay-put face-ov 'face 'italic)
          (overlay-put face-ov 'priority 5)
          (narrow-to-region 2 16)
          (cl-incf neo-cx300-counter)
          (aset rec 2 :modified)
          (delete-region 5 8)
          (insert "MEGA")
          (cl-incf neo-cx300-counter)
          (let ((r (neo-cx300-target 5)))
            (let ((state (list timer-fired env-val exit-code
              ...
              (undo) (undo) (undo)
              (widen)
              (advice-remove 'neo-cx300-target 'mega-advice)
              (kill-buffer buf)
...
(let ((lexical-binding t)
  ...
  (defun neo-cx300-target () :orig)
  (advice-add 'neo-cx300-target :override (lambda () :overridden))
  (with-temp-buffer
    (buffer-enable-undo)
    ...
    (let ((m (set-marker (make-marker) 12))
      ...
      (let* ((matches (cl-loop for i from 0 below 3
             ...
             (advice-result (neo-cx300-target())))
        (let ((snapshot (list matches pcase-result time-str advice-result
          ...
          (widen)
          (jump-to-register ?c)
          (advice-remove 'neo-cx300-target (advice--p (advice-member-p nil 'neo-cx300-target)))
...
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX300=v2" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX300"))))
      (exit-code (let ((p (make-process :name "neo-cx300-ec2"
                                          :command '("sh" "-c" "exit 8")))
                       (weak-ht (make-hash-table :weakness 'key :test 'eq)))
        (puthash (cons 1 nil) :v weak-ht)
        (garbage-collect)
  ...
  (run-with-timer 0 nil (lambda () (setq timer-fired :t)))
  (sit-for 0.01)
  (let ((buf (get-buffer-create " *neo-cx300-final*")))
    (with-current-buffer buf
      (buffer-enable-undo)
      (setq-local neo-cx300-buf :local)
      (insert "Mega 300 café 世界 buffer content")
      (put-text-property 1 6 'face 'bold)
      (put-text-property 8 14 'display "XX"))
    (let ((p (make-process :name "neo-cx300-mega-p"
                           :command '("sh" "-c" "printf 'MEGA300'")
                           :buffer buf)))
...
(let ((lexical-binding t)
      (calls nil))
  (defmacro neo-cx300-double (form) `(progn ,form ,form))
  (defun neo-cx300-target (x) (push (list :primary x) calls) (* x 2))
  (advice-add 'neo-cx300-target :before
              (lambda (x) (push (list :before x) calls))
              '((name . mega-advice)))
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c C-a") 'neo-cx300-cmd-a)
    (define-key map [f5] 'neo-cx300-f5)
    (letrec ((counter 0)
             (inc-fn (lambda () (cl-incf counter))))
      (let* ((ht (make-hash-table :test 'equal))
             (ob (make-obarray 31))
             (sym1 (intern "neo-cx300-alpha" ob))
             (text "café 世界"))
        (puthash "a" 1 ht)
        (puthash "b" 2 ht)
        (put sym1 'neo-cx300-prop :val)
        (let* ((hash (secure-hash 'sha256 text))
               (cl-loop-result (cl-loop for k being the hash-keys of ht using (hash-values v)
                                        collect (cons k v)))
               (macro-result (eval '(macroexpand '(neo-cx300-double (cl-incf counter))) t))
               (target-result (neo-cx300-target 5)))
          (with-temp-buffer
            (buffer-enable-undo)
            (insert "Keymap/eval/hash mega milestone test")
            (put-text-property 1 6 'face 'bold)
            (let ((m (set-marker (make-marker) 8))
...
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX300=v3" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX300"))))
      (exit-code (let ((p (make-process :name "neo-cx300-final-ec"
                                          :command '("sh" "-c" "exit 7")))
                       (weak-ht (make-hash-table :weakness 'key :test 'eq)))
        (puthash (cons 1 nil) :v weak-ht)
        (garbage-collect)
  ...
  (puthash "alpha" 1 ht)
  (puthash "beta" 2 ht)
  (let* ((text "café 世界 final mega 300")
         (enc (encode-coding-string text 'utf-8))
         (hash (secure-hash 'sha256 enc))
         (buf (get-buffer-create " *neo-cx300-ultimate*")))
    (with-current-buffer buf
      (buffer-enable-undo)
      (setq-local neo-cx300-local-counter 0)
      (insert text)
      (put-text-property 1 5 'face 'bold)
      ...
      (let ((m (set-marker (make-marker) 8))
        ...
        (overlay-put ov 'evaporate t)
        (narrow-to-region 2 18)
        (cl-incf neo-cx300-local-counter)
        (aset rec 2 :modified)
