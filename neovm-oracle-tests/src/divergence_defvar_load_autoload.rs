...
fn divergence_defvar_only_sets_when_void() {
  ...
  my-dv-test-1)"#, expect_test::expect![[r#""OK 100""#]]);
}
...
fn divergence_defconst_always_sets() {
  ...
  my-dc-test-1)"#, expect_test::expect![[r#""OK 999""#]]);
}
...
fn divergence_defvar_inside_let_shadow() {
  ...
  (let ((my-dv-shadow 42))
    ...
    (list my-dv-shadow
          (eval 'my-dv-shadow)))
  my-dv-shadow)"#, expect_test::expect![[r#""OK 0""#]]);
}
...
fn divergence_special_variable_p() {
  ...
  (list (special-variable-p 'my-spec-var)
        ...
        (special-variable-p 'buffer-read-only)))"#, expect_test::expect![[r#""OK (t nil t t)""#]]);
}
...
fn divergence_defvar_local_bare_declare() {
  ...
  (list (special-variable-p 'my-bare-dv)
        (boundp 'my-bare-dv)))"#, expect_test::expect![[r#""OK (nil nil)""#]]);
}
...
fn divergence_provide_subfeature() {
  ...
  (list (featurep 'my-test-sub)
        (featurep 'my-test-sub 42)))"#, expect_test::expect![[r#""ERR (wrong-type-argument symbolp (my-test-sub . 42))""#]]);
}
...
fn divergence_autoload_function() {
  ...
  (list (fboundp 'my-autoload-fn)
        ...
        (documentation 'my-autoload-fn)))"#, expect_test::expect![[r#""OK (t t \"doc\")""#]]);
}
...
fn divergence_load_path_search() {
    ...
    crate::common::assert_oracle_parity_expect(
        r#"(list (member (expand-file-name "emacs-lisp" (car load-path)) load-path)
              (consp load-path)
              (> (length load-path) 0))"#, expect_test::expect![[r#""ERR (error \"Lisp nesting exceeds ‘max-lisp-eval-depth’\")""#]]);
}
...
fn divergence_load_file_name_during_eval() {
    ...
    crate::common::assert_oracle_parity_expect(
        r#"(list load-file-name
              load-in-progress
              (booleanp load-in-progress))"#, expect_test::expect![[r#""OK (\"/tmp/nix-shell.XcUf3d/neovm-inline-oracle-sf3ylzib/program-15400-100.el\" t t)""#]]);
}
...
fn divergence_variable_alias() {
  ...
  (list my-alias-target
        ...
        my-alias-source))"#, expect_test::expect![[r#""ERR (void-function variable-binding-alias)""#]]);
}
