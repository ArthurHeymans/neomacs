//! Combo: cl-do* + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests sequential iterative binding with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_do_star_marker_overlay_textprop_buflocal_narrow_undo() {
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (cl-do* ((i 0 (1+ i))
          ...
          (goto-char (point-min))
          (insert (format "%s-%d-%s-" ch i my-var)))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_do_star_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_do_star_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "ds1"))
        (b2 (generate-new-buffer "ds2")))
...
#[test]
fn combo_cl_do_star_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "dss")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (cl-do* ((i 0 (1+ i))
          ...
          (replace-match (format "%s-%d-%s" ch i my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_do_star_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
