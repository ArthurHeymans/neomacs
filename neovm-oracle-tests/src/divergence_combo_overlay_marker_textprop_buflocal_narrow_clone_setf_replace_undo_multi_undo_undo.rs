//! Combo: overlay + marker + textprop + buflocal + narrow + clone + setf + replace + undo + multi-buffer + undo + undo.
//! Tests 12 subsystems in different order.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_setf_replace_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "q1a"))
        (b2 (generate-new-buffer "q1b")))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "q1c")))
           (c2 (with-current-buffer b2 (clone-buffer "q1d"))))
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_replace_setf_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "s2a"))
        (b2 (generate-new-buffer "s2b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "s2c")))
           (c2 (with-current-buffer b2 (clone-buffer "s2d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_multi_overlay_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "u1a"))
        (b2 (generate-new-buffer "u1b")))
    (with-current-buffer b1
    ...
    (let* ((ov1a (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "u1c")))
           (c2 (with-current-buffer b2 (clone-buffer "u1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_textprop_replace_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "v1a"))
        (b2 (generate-new-buffer "v1b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'kind 'a)
      (put-text-property 6 10 'kind 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "v1c")))
           (c2 (with-current-buffer b2 (clone-buffer "v1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_setf_replace_undo_multi_undo_undo_2() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "w1a"))
        (b2 (generate-new-buffer "w1b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "w1c")))
           (c2 (with-current-buffer b2 (clone-buffer "w1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
