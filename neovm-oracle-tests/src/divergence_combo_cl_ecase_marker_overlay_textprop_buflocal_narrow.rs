//! Combo: cl-ecase + marker + overlay + textprop + buflocal + narrow + undo.
...
#[test]
fn combo_cl_ecase_marker_overlay_textprop_buflocal_narrow_undo() {
...
#[test]
fn combo_cl_ecase_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "ecc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_ecase_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "ec1"))
        (b2 (generate-new-buffer "ec2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (cl-ecase my-var
          (b1-base (goto-char 6) (insert (format "%s-" my-var)))
          (b2-base (goto-char 6) (insert "B2-")))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (cl-ecase my-var
          (b1-base (goto-char 6) (insert "B1-"))
          (b2-base (goto-char 6) (insert (format "%s-" my-var))))
        (undo-boundary))
...
#[test]
fn combo_cl_ecase_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "esr")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_ecase_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
