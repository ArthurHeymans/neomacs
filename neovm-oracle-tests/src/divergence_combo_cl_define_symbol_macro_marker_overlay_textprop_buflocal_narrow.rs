//! Combo: cl-define-symbol-macro + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests define-symbol-macro with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_define_symbol_macro_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "dsm")))
...
#[test]
fn combo_cl_define_symbol_macro_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "dsc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_define_symbol_macro_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((b1 (generate-new-buffer "dm1"))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (cl-symbol-macrolet ((my-macro "M1"))
          (goto-char 6)
          (insert (format "%s-%s-" my-macro my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (cl-symbol-macrolet ((my-macro "M2"))
          (goto-char 6)
          (insert (format "%s-%s-" my-macro my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_define_symbol_macro_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (cl-symbol-macrolet ((my-macro "EXPANDED"))
          ...
          (replace-match (format "%s-%s" my-macro my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_define_symbol_macro_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
