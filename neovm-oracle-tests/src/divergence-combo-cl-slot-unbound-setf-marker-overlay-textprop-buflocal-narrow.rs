//! Combo: cl-slot-unbound-setf + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests slot-unbound-setf with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_slot_unbound_setf_marker_overlay_textprop_buflocal_narrow_undo() {
...
#[test]
fn combo_cl_slot_unbound_setf_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "suc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_slot_unbound_setf_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_slot_unbound_setf_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_slot_unbound_setf_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
