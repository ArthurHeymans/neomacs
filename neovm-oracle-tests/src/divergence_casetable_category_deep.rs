...
fn divergence_case_table_access() {
  ...
  (list (char-table-p ct)
        (aref ct ?A)
        (aref ct ?a)
        (aref ct ?z)))"#, expect_test::expect![[r#""OK (t 97 97 122)""#]]);
}
...
fn divergence_case_table_set_identity() {
  ...
  (eq (downcase ?A) ?a))"#, expect_test::expect![[r#""OK t""#]]);
}
...
fn divergence_translation_table() {
  ...
  (fboundp 'translate-region))"#, expect_test::expect![[r##""ERR (invalid-read-syntax \"#\\\"\" 4 38)""##]]);
}
...
fn divergence_char_category_set() {
  ...
  (list (category-table-p ct)
        ...
        (category-set-mnemonics (aref ct ?0))))"#, expect_test::expect![[r#""OK (t \".Lalr\" \".al\" \".6alr\")""#]]);
}
...
fn divergence_modify_category_entry() {
  ...
  (let ((cs (aref (standard-category-table) ?X)))
    (list (if cs (category-set-mnemonics cs) nil))))"#, expect_test::expect![[r#""ERR (error \"Undefined category: x\")""#]]);
}
...
fn divergence_string_search_case_fold() {
  ...
  (list (string-match "hello" "HELLO WORLD")
        ...
        (match-beginning 0)
        (match-end 0)))"#, expect_test::expect![[r#""OK (0 4 4 9)""#]]);
}
...
fn divergence_string_search_no_fold() {
  ...
  (list (string-match "hello" "HELLO WORLD")
        ...
        (string-match "HELLO" "hello world")))"#, expect_test::expect![[r#""OK (nil 0 nil)""#]]);
}
...
fn divergence_char_inspect_name() {
  ...
  (stringp (char-name ?\x01)))"#, expect_test::expect![[r#""ERR (void-function char-name)""#]]);
}
...
fn divergence_char_general_category() {
  ...
  (get-char-code-property ?\n 'general-category))"#, expect_test::expect![[r#""OK (Lu Ll Nd Zs Cc)""#]]);
}
