//! Combo: overlay + marker + textprop + buflocal + narrow + clone + setf + replace + undo + multi-buffer.
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_setf_replace_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "onb"))
        (b2 (generate-new-buffer "ond")))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "onc")))
           (c2 (with-current-buffer b2 (clone-buffer "onf"))))
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_replace_setf_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "prb"))
        (b2 (generate-new-buffer "prd")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "prc")))
           (c2 (with-current-buffer b2 (clone-buffer "prf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_multi_overlay_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "qmb"))
        (b2 (generate-new-buffer "qmd")))
    (with-current-buffer b1
    ...
    (let* ((ov1a (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "qmc")))
           (c2 (with-current-buffer b2 (clone-buffer "qmf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_textprop_replace_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "stb"))
        (b2 (generate-new-buffer "std")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'kind 'a)
      (put-text-property 6 10 'kind 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "stc")))
           (c2 (with-current-buffer b2 (clone-buffer "stf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_narrow_clone_setf_replace_undo_multi_2() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((b1 (generate-new-buffer "umb"))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "umc")))
           (c2 (with-current-buffer b2 (clone-buffer "umf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
