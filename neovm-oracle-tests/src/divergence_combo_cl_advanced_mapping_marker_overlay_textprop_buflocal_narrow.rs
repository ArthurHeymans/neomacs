//! Combo: cl-map-into/cl-mapcan/cl-mapl + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests advanced sequence mapping with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_map_into_marker_overlay_textprop_buflocal_narrow_undo() {
...
#[test]
fn combo_cl_mapcan_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (undo-boundary)
          (let* ((lst (list 'a 'b 'c))
                 (result (cl-mapcan (lambda (x) (list x (list x))) lst)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
  ...
  (let ((b1 (generate-new-buffer "ml1"))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (let* ((lst (list 'a 'b 'c))
               (result (cl-mapl (lambda (x) (list (car x))) lst)))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (let* ((lst (list 'd 'e 'f))
               (result (cl-mapcon (lambda (x) (list (car x))) lst)))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_map_into_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
