//! Combo: cl-get/cl-put + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests symbol property access with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_get_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "gtb")))
...
#[test]
fn combo_cl_get_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "gtc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "gtc-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((sym (make-symbol "test"))
                 (_ (put sym 'my-prop 99))
                 (v (get sym 'my-prop)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
...
#[test]
fn combo_cl_get_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "gt1"))
        (b2 (generate-new-buffer "gt2")))
...
#[test]
fn combo_cl_get_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_get_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
