//! Complex combo batch 330 — final MEGA: 3 extreme integration stress
...
#[test]
fn div_cx330_mega_1_full_subsystem_ultimate() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX330=v1" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX330"))))
      (exit-code (let ((p (make-process :name "neo-cx330-ec1"
                                          :command '("sh" "-c" "exit 5")))
      ...
      (rec (record 'neo-cx330-tag :a :b :c))
      (lexical-binding t)
      (calls nil))
  ...
  (puthash "alpha" 1 ht)
  (puthash "beta" 2 ht)
  (defun neo-cx330-target (x) (push (list :primary x) calls) (* x 2))
  (advice-add 'neo-cx330-target :before
              (lambda (x) (push (list :before x) calls))
              '((name . mega-advice)))
  (let* ((text "café 世界 😀 mega-330")
    ...
    (let ((buf (get-buffer-create " *neo-cx330-mega-1*")))
      (with-current-buffer buf
        (buffer-enable-undo)
        (setq-local neo-cx330-counter 0)
        (insert text)
        (put-text-property 1 4 'face 'bold)
        (put-text-property 6 10 'display "XX")
        (add-to-invisibility-spec 'neo-cx330-h)
        (let ((m (set-marker (make-marker) 8))
              (invis-ov (make-overlay 3 8))
              (face-ov (make-overlay 6 14)))
          (overlay-put invis-ov 'invisible 'neo-cx330-h)
          (overlay-put face-ov 'face 'italic)
          (overlay-put face-ov 'priority 5)
          (narrow-to-region 2 16)
          (cl-incf neo-cx330-counter)
          (aset rec 2 :modified)
          (delete-region 5 8)
          (insert "MEGA")
          (cl-incf neo-cx330-counter)
          (let ((r (neo-cx330-target 5)))
            (let ((state (list timer-fired env-val exit-code
              ...
              (undo) (undo) (undo)
              (widen)
              (advice-remove 'neo-cx330-target 'mega-advice)
              (kill-buffer buf)
...
#[test]
fn div_cx330_mega_2_pcase_rx_syntax_advice_register_window_format_time() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"
(let ((lexical-binding t)
      (t0 (encode-time 30 30 14 16 6 2026 nil)))
  (defun neo-cx330-target () :orig)
  (advice-add 'neo-cx330-target :override (lambda () :overridden))
  (with-temp-buffer
    (buffer-enable-undo)
    ...
    (let ((m (set-marker (make-marker) 12))
      ...
      (let* ((matches (cl-loop for i from 0 below 3
             ...
             (advice-result (neo-cx330-target())))
        (let ((snapshot (list matches pcase-result time-str advice-result
          ...
          (widen)
          (jump-to-register ?c)
          (advice-remove 'neo-cx330-target (advice--p (advice-member-p nil 'neo-cx330-target)))
...
#[test]
fn div_cx330_mega_3_process_buflocal_coding_env_timer_weak_hash_textprop_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX330=v2" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX330"))))
      (exit-code (let ((p (make-process :name "neo-cx330-ec2"
                                          :command '("sh" "-c" "exit 8")))
                       (weak-ht (make-hash-table :weakness 'key :test 'eq)))
        (puthash (cons 1 nil) :v weak-ht)
        (garbage-collect)
  ...
  (run-with-timer 0 nil (lambda () (setq timer-fired :t)))
  (sit-for 0.01)
  (let ((buf (get-buffer-create " *neo-cx330-final*")))
    (with-current-buffer buf
      (buffer-enable-undo)
      (setq-local neo-cx330-buf :local)
      (insert "Mega 330 café 世界 buffer content")
      (put-text-property 1 6 'face 'bold)
      (put-text-property 8 14 'display "XX"))
    (let ((p (make-process :name "neo-cx330-mega-p"
                           :command '("sh" "-c" "printf 'MEGA330'")
                           :buffer buf)))
