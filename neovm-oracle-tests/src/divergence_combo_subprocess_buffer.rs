...
fn divergence_call_process_propertize_search() {
  ...
  (let ((result nil))
    ...
    (list (length (nreverse result))
        ...
        expect_test::expect![[r#""OK (10 nil (#(\"10\" 0 2 (parity even)) even) nil)""#]],
    );
}
...
fn divergence_shell_parse_into_hash() {
  ...
  (list (hash-table-count ht)
        ...
        (gethash \"key2\" ht)
        (gethash \"key3\" ht)
        (= (hash-table-count ht) 3))) ", expect_test::expect![[r#""OK (3 \"val1\" \"val2\" \"val3\" t)""#]]);
}
...
fn divergence_temp_file_write_read_props() {
  ...
  (let ((result
    ...
    (delete-file tmp)
    result)) ",
        expect_test::expect![[r#""OK (\"Hello World\" nil 11)""#]],
    );
}
  ...
  (let ((m1 (set-marker (make-marker) (point-max))))
    ...
    (let ((m2 (set-marker (make-marker) (point-max))))
      ...
      (list (marker-position m1) (marker-position m2)
        ...
        expect_test::expect![[r#""OK (7 14 \"first\nsecond\nthird\n\" t)""#]],
    );
}
...
fn divergence_call_process_region_transform() {
  ...
  (list (buffer-string)
        ...
        expect_test::expect![[r#""OK (\"HELLO WORLD\" nil t)""#]],
    );
}
...
fn divergence_write_read_large_temp() {
  ...
  (let ((result
    ...
    (delete-file tmp)
    result)) ",
        expect_test::expect![[r#""OK (100 t 1 t)""#]],
    );
}
