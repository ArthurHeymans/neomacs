//! Combo: cl-symbol-macrolet + markers + overlays + textprop + undo.
//! Tests symbol macrolet with buffer state.
  ...
  (let ((buf (generate-new-buffer "csm")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 10))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (cl-symbol-macrolet ((my-str "XX"))
          (goto-char 6)
          (insert (concat my-str "-")))
        (undo-boundary)
...
#[test]
fn combo_cl_symbol_macrolet_narrow_marker() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_symbol_macrolet_clone_overlay() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_symbol_macrolet_multi_macro() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((buf (generate-new-buffer "cmm")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 10))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (cl-symbol-macrolet ((s1 "XX") (s2 "YY"))
          (goto-char 6)
          (insert s1)
          (goto-char 12)
          (insert s2))
...
#[test]
fn combo_cl_symbol_macrolet_overlay_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
