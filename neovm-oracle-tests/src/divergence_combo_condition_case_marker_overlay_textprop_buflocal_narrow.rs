//! Combo: condition-case + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests error handling with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_condition_case_marker_overlay_textprop_buflocal_narrow_undo() {
...
#[test]
fn combo_condition_case_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
      ...
      (with-current-buffer b2
        ...
        (condition-case err
            (progn
              ...
              (signal 'test-error '("test")))
          (test-error
           (let ((mp (marker-position m2))
                 (os (overlay-start ov2))
                 (s (buffer-string)))
             (primitive-undo 1 buffer-undo-list)
             (list mp os s
                   (marker-position m2)
...
#[test]
fn combo_condition_case_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_condition_case_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
