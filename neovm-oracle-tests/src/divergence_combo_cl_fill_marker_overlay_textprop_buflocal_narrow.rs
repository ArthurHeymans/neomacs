//! Combo: cl-fill + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests sequence fill with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_fill_marker_overlay_textprop_buflocal_narrow_undo() {
...
#[test]
fn combo_cl_fill_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_fill_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_fill_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let ((vec [1 2 3 4 5]))
          ...
          (replace-match (format "%s-%s" (append vec nil) my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_fill_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
