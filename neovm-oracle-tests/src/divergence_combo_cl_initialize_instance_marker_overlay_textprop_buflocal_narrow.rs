//! Combo: cl-initialize-instance + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests initialize-instance with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_initialize_instance_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "iib")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (undo-boundary)
        (let* ((obj (list 'a 1 'b 2 'c 3 'd 4))
               (result (cl-getf obj 'b))
               (result2 (cl-getf obj 'd)))
          (put-text-property (point-min) (point-max) 'z 'changed)
          (setf (char-after (point-min)) ?Z)
          (setf (marker-position m) 11)
          (goto-char (point-min))
          (insert (format "%s-%s-%s-" result result2 my-var)))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_initialize_instance_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "iic")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "iic-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((obj (list 'x 10 'y 20 'z 30 'w 40))
                 (result (cl-getf obj 'y))
                 (result2 (cl-getf obj 'w)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
...
#[test]
fn combo_cl_initialize_instance_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "ii1"))
        (b2 (generate-new-buffer "ii2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b2
        (undo-boundary)
        (let* ((obj (list 'd 4 'e 5 'f 6))
               (result (cl-getf obj 'e)))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_initialize_instance_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "isr")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((obj (list 'a 1 'b 2 'c 3 'd 4))
          ...
          (replace-match (format "%s-%s-%s" result result2 my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_initialize_instance_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "imo")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 10))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (let* ((obj (list 'a 1 'b 2 'c 3 'd 4 'e 5))
               (result (cl-getf obj 'c))
               (result2 (cl-getf obj 'e)))
          (setf (char-after 6) ?Z)
          (setf (marker-position m) 11)
          (goto-char 6)
          (insert (format "%s-%s-%s-" result result2 my-var)))
