...
fn divergence_make_hash_table_custom_test() {
  ...
  (list (gethash "key1" ht)
        ...
        (hash-table-count ht)))"#,
        expect_test::expect![[r#""OK (val1 val2 missing 2)""#]],
    );
}
...
fn divergence_hash_table_removal() {
  ...
  (list (gethash 'x ht 'gone)
        ...
        (remhash 'z ht)
        (hash-table-count ht)))"#,
        expect_test::expect![[r#""OK (gone 20 1 nil 1)""#]],
    );
}
...
fn divergence_hash_table_weakness() {
  ...
  (list (hash-table-weakness ht)
        ...
        (hash-table-p ht)
        (hash-table-test ht)))"#,
        expect_test::expect![[r#""OK (key-and-value (1 2 3) t eql)""#]],
    );
}
...
fn divergence_copy_hash_table() {
  ...
  (list (gethash 'a ht)
        ...
        (gethash 'b ht)
        (gethash 'b ht2)))"#,
        expect_test::expect![[r#""OK (1 nil nil 2)""#]],
    );
}
...
fn divergence_record_type() {
  ...
  (list (record-type-p desc)
        ...
        expect_test::expect![[r#""ERR (void-function make-record-type)""#]],
    );
}
...
fn divergence_obarray_symbols() {
  ...
  (list (intern-soft "my-obtest-sym")
        ...
        expect_test::expect![[r#""OK (my-obtest-sym nil my-obtest-sym t)""#]],
    );
}
...
(list
  ...
  (mapconcat #'identity '("a" "b" "c") "-"))"#,
        expect_test::expect![[r#""OK ((2 3 4) (1 2 3) (a 1 b 2 c 3) \"a-b-c\")""#]],
    );
}
...
fn divergence_assoc_list_ops() {
  ...
  (list (assoc 'b alist)
        ...
        expect_test::expect![[r#""OK ((b . 2) (a . 1) (c . 3) (a . 1) 3 missing)""#]],
    );
}
...
fn divergence_plist_ops() {
  ...
  (list (plist-get pl 'b)
        ...
        expect_test::expect![[r#""OK (2 nil (c 3 d 4) (a 1 b 2 c 3 d 4) 2)""#]],
    );
}
