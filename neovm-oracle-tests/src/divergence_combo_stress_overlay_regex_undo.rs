  ...
  (let ((ovs nil))
    ...
    (let ((at-point (overlays-at (match-beginning 0)))
      ...
      (list (= (length at-point) 1)
            ...
            (= (buffer-size) 26))))) "#,
        expect_test::expect![[r#""ABCDEFGHIJKLMNOPQRSTUVWXYZOK (t t (0 1 2 3 4) t)""#]],
    );
}
  ...
  (let ((ov1 (make-overlay 1 3))
    ...
    (let ((remaining (delq nil (mapcar (lambda (ov)
      ...
      (list (buffer-string)
            remaining
            (= (length remaining) 1))))) "#,
        expect_test::expect![[r#""---DDDD-EEEEOK (\"---DDDD-EEEE\" nil nil)""#]],
    );
}
    ...
    (let ((p1 (marker-position m-insert))
      ...
      (let ((p3 (marker-position m-insert))
        ...
        (list (= p1 2) (= p2 3)
              ...
              (buffer-string)
              (= (buffer-size) 6)))))) "#,
        expect_test::expect![[r#""ABBAAAOK (t t t nil \"ABBAAA\" t)""#]],
    );
}
    ...
    (let ((s1 (buffer-string))
      ...
      (let ((s2 (buffer-string))
        ...
        (list s1 s2 (buffer-string)
              ...
              (list (overlay-start ov) (overlay-end ov))
              (overlay-get ov 'tag)))))) "#,
        expect_test::expect![[r#""STXXARYYTERR (wrong-type-argument listp t)""#]],
    );
}
