...
fn div_cx451_string_lines() {
  ...
  (list (string-lines "hello\nworld\nthird")
        (string-lines "")))"##,
        expect_test::expect![[r#""OK ((\"hello\" \"world\" \"third\") (\"\"))""#]],
    );
}
...
fn div_cx451_string_split() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(string-split "one two three" " " t)"##,
...
fn div_cx451_backtrace_get_frames() {
  ...
  (list (vectorp frames)
        (> (length frames) 0)))"##,
        expect_test::expect![[r#""ERR (void-function backtrace-get-frames)""#]],
    );
}
...
fn div_cx451_encode_time_decoded_time_ext() {
    ...
    (let ((dt (decode-time (encode-time 0 0 12 16 6 2024 nil))))
      (list dt (length dt)))
  (error (car e)))"##,
        expect_test::expect![[r#""OK ((0 0 12 16 6 2024 0 t -14400) 9)""#]],
    );
}
