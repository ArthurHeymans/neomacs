//! Combo: cl-loop with destructuring + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests loop destructuring with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_loop_destructure_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "ldb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (undo-boundary)
        (let* ((data (list (list 'a 1) (list 'b 2) (list 'c 3)))
               (result (cl-loop for (sym num) in data
                                collect (list sym num))))
...
#[test]
fn combo_cl_loop_destructure_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "ldc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "ldc-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((data (list (list 'x 10) (list 'y 20)))
                 (result (cl-loop for (sym num) in data
                                  collect (list sym num))))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
...
#[test]
fn combo_cl_loop_destructure_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "ld1"))
        (b2 (generate-new-buffer "ld2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (let* ((data (list (list 'a 1) (list 'b 2)))
               (result (cl-loop for (sym num) in data
                                collect (list sym num))))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
      ...
      (with-current-buffer b2
        (undo-boundary)
        (let* ((data (list (list 'd 4) (list 'e 5)))
               (result (cl-loop for (sym num) in data
                                collect (list sym num))))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
...
#[test]
fn combo_cl_loop_destructure_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "lsr")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_loop_destructure_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
