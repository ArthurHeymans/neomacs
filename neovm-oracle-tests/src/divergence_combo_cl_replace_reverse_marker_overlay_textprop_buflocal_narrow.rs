//! Combo: cl-replace/cl-nreverse/cl-reverse + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests sequence replacement/reversal with buffer state.
  ...
  (let ((buf (generate-new-buffer "nrc")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "nrc-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((lst (list 'a 'b 'c 'd))
                 (result (cl-nreverse lst)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
...
fn combo_cl_reverse_multi_buffer_undo() {
    ...
    let expect = expect_test::expect![[r#""ERR (void-function cl-reverse)""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  ...
  (let ((buf (generate-new-buffer "rsr")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((seq1 (list 'a 'b 'c 'd))
               (seq2 (list 'x 'y)))
          (cl-replace seq1 seq2 :start1 0 :end1 2)
          (setf (char-after 6) ?Z)
          (setf (marker-position m) 11)
          (goto-char 6)
          (re-search-forward "BBBB")
          (replace-match (format "%s-%s" seq1 my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
fn combo_cl_nreverse_multi_overlay_undo() {
    ...
    let expect = expect_test::expect![[r#""ERR (void-function cl-nreverse)""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
