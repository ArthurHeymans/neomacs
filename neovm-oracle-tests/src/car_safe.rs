...
fn oracle_prop_car_safe_basics() {
    ...
    let (oracle_cons, neovm_cons) = crate::common::eval_oracle_and_neovm_expect(
        "(car-safe (cons 1 2))",
        expect_test::expect![[r#""OK 1""#]],
    );
    assert_ok_eq("1", &oracle_cons, &neovm_cons);

    let (oracle_atom, neovm_atom) = crate::common::eval_oracle_and_neovm_expect(
        "(car-safe 1)",
...
fn oracle_prop_car_safe_wrong_arity_error() {
    ...
    let (oracle, neovm) = crate::common::eval_oracle_and_neovm_expect(
        "(car-safe)",
        expect_test::expect![[r#""ERR (wrong-number-of-arguments car-safe 0)""#]],
    );
