//! Combo: overlay + marker + textprop + buflocal + clone + narrow + setf + replace + undo + multi-buffer + undo + undo.
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_setf_replace_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "o1a"))
        (b2 (generate-new-buffer "o1b")))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "o1c")))
           (c2 (with-current-buffer b2 (clone-buffer "o1d"))))
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_replace_setf_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "r1a"))
        (b2 (generate-new-buffer "r1b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "r1c")))
           (c2 (with-current-buffer b2 (clone-buffer "r1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_multi_overlay_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "m1a"))
        (b2 (generate-new-buffer "m1b")))
    (with-current-buffer b1
    ...
    (let* ((ov1a (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "m1c")))
           (c2 (with-current-buffer b2 (clone-buffer "m1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_textprop_replace_undo_multi_undo_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "t1a"))
        (b2 (generate-new-buffer "t1b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'kind 'a)
      (put-text-property 6 10 'kind 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "t1c")))
           (c2 (with-current-buffer b2 (clone-buffer "t1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_setf_replace_undo_multi_undo_undo_2() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "z1a"))
        (b2 (generate-new-buffer "z1b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "z1c")))
           (c2 (with-current-buffer b2 (clone-buffer "z1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
