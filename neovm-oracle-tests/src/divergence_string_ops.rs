...
fn divergence_substring_edge_cases() {
  ...
  (list (substring s 0 5)
        ...
        (substring s -6)
        (substring s -6 -1)))"#,
        expect_test::expect![[r#""OK (\"Hello\" \"World!\" \"World!\" \"World\")""#]],
    );
}
...
fn divergence_substring_properties() {
  ...
  (list (get-text-property 0 'face sub)
        ...
        (length sub)))"#,
        expect_test::expect![[r#""OK (bold bold 2)""#]],
    );
}
...
fn divergence_string_bytes_vs_length() {
  ...
  (list (length s)
        ...
        (string< "abc" "abd")
        (string> "abd" "abc")))"#,
        expect_test::expect![[r#""OK (8 13 t t t)""#]],
    );
}
...
fn divergence_string_multibyte_concat() {
  ...
  (list s
        ...
        (string= s "abc中文def")))"#,
        expect_test::expect![[r#""OK (\"abc中文def\" 8 12 t t)""#]],
    );
}
...
fn divergence_string_unibyte() {
  ...
  (list s
        ...
        expect_test::expect![[r#""ERR (void-variable 0xc0)""#]],
    );
}
...
fn divergence_string_make_unibyte() {
  ...
  (list (multibyte-string-p ms)
        ...
        (length us)))"#,
        expect_test::expect![[r#""OK (t nil 5)""#]],
    );
}
...
fn divergence_string_replace() {
    ...
    crate::common::assert_oracle_parity_expect(
        ...
        expect_test::expect![[r#""OK (\"hello Emacs\" \"f00 b00 m00\" \"a#b#c#\")""#]],
    );
}
...
fn divergence_string_pad() {
  ...
  (string-pad "hi" 10 ?-)
  (string-pad "hello" 3))"#,
        expect_test::expect![[r#""OK (\"hi        \" \"hi--------\" \"hello\")""#]],
    );
}
