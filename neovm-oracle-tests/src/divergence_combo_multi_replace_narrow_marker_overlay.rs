//! Combo: marker + overlay + textprop + undo + multi-replace + narrow.
//! Tests complex multi-replace+narrow interactions with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_multi_replace_narrow_marker_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mrn")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD-EEEE")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      (put-text-property 11 15 'z 'c)
      (put-text-property 16 20 'z 'd)
      (put-text-property 21 25 'z 'e)
      (let* ((ov (make-overlay 6 20))
             (_ (overlay-put ov 'face 'bold))
             (m1 (make-marker))
             (m2 (make-marker))
             (_ (set-marker m1 8))
             (_ (set-marker m2 18)))
        (narrow-to-region 6 20)
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "BBBB")
        (replace-match "XX")
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "DDDD")
        (replace-match "YY")
        (undo-boundary)
        (let ((mp1 (marker-position m1))
              (mp2 (marker-position m2))
              (os (overlay-start ov))
              (oe (overlay-end ov))
              (bs (buffer-substring (point-min) (point-max))))
          (primitive-undo 2 buffer-undo-list)
          (widen)
          (list mp1 mp2 os oe bs
                (marker-position m1)
                (marker-position m2)
                (overlay-start ov)
                (overlay-end ov)
                (buffer-string)))))
    (kill-buffer buf)))"#,
    );
}

#[test]
fn combo_multi_replace_narrow_clone_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mrc")))
    (with-current-buffer buf
      (insert "hello world test foo bar baz")
      (put-text-property 1 6 'w 'hello)
      (put-text-property 7 12 'w 'world)
      (put-text-property 13 17 'w 'test)
      (put-text-property 18 21 'w 'foo)
      (put-text-property 22 25 'w 'bar)
      (put-text-property 26 29 'w 'baz)
      (let* ((ov (make-overlay 7 25))
             (_ (overlay-put ov 'face 'highlight))
             (m (make-marker))
             (_ (set-marker m 13))
             (clone (clone-buffer "mrc-clone")))
        (with-current-buffer clone
          (narrow-to-region 7 25)
          (undo-boundary)
          (goto-char (point-min))
          (re-search-forward "world")
          (replace-match "W")
          (undo-boundary)
          (goto-char (point-min))
          (re-search-forward "foo")
          (replace-match "F")
          (undo-boundary)
          (let ((mp (marker-position m))
                (os (overlay-start ov))
                (oe (overlay-end ov))
                (bs (buffer-substring (point-min) (point-max))))
            (primitive-undo 2 buffer-undo-list)
            (widen)
            (list mp os oe bs
                  (marker-position m)
                  (overlay-start ov)
                  (overlay-end ov)
                  (buffer-string)))))
      (kill-buffer clone)
      (kill-buffer buf)))"#,
    );
}

#[test]
fn combo_multi_replace_narrow_textprop_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mrt")))
    (with-current-buffer buf
      (insert "aaa bbb ccc ddd eee fff")
      (put-text-property 1 4 'p 1)
      (put-text-property 5 8 'p 2)
      (put-text-property 9 12 'p 3)
      (put-text-property 13 16 'p 4)
      (put-text-property 17 20 'p 5)
      (put-text-property 21 24 'p 6)
      (let* ((ov (make-overlay 5 20))
             (_ (overlay-put ov 'face 'region))
             (m (make-marker))
             (_ (set-marker m 10)))
        (narrow-to-region 5 20)
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "bbb")
        (replace-match "XX")
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "ddd")
        (replace-match "YY")
        (undo-boundary)
        (let ((mp (marker-position m))
              (os (overlay-start ov))
              (oe (overlay-end ov))
              (k (get-text-property (point-min) 'p))
              (bs (buffer-substring (point-min) (point-max))))
          (primitive-undo 2 buffer-undo-list)
          (widen)
          (list mp os oe k bs
                (marker-position m)
                (overlay-start ov)
                (overlay-end ov)
                (buffer-string)))))
    (kill-buffer buf)))"#,
    );
}

#[test]
fn combo_multi_replace_narrow_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mro")))
    (with-current-buffer buf
      (insert "aa1 bb2 cc3 dd4 ee5 ff6")
      (put-text-property 1 4 'z 'a)
      (put-text-property 5 8 'z 'b)
      (put-text-property 9 12 'z 'c)
      (put-text-property 13 16 'z 'd)
      (put-text-property 17 20 'z 'e)
      (put-text-property 21 24 'z 'f)
      (let* ((ov1 (make-overlay 1 12))
             (ov2 (make-overlay 13 24))
             (_ (overlay-put ov1 'priority 1))
             (_ (overlay-put ov2 'priority 2))
             (m (make-marker))
             (_ (set-marker m 10)))
        (narrow-to-region 1 24)
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "aa1")
        (replace-match "AA1")
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "dd4")
        (replace-match "DD4")
        (undo-boundary)
        (let ((mp (marker-position m))
              (os1 (overlay-start ov1))
              (oe1 (overlay-end ov1))
              (os2 (overlay-start ov2))
              (oe2 (overlay-end ov2))
              (bs (buffer-string)))
          (primitive-undo 2 buffer-undo-list)
          (widen)
          (list mp os1 oe1 os2 oe2 bs
                (marker-position m)
                (overlay-start ov1)
                (overlay-end ov2)
                (buffer-string)))))
    (kill-buffer buf)))"#,
    );
}

#[test]
fn combo_multi_replace_narrow_buflocal_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mrb")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      (put-text-property 11 15 'z 'c)
      (put-text-property 16 20 'z 'd)
      (setq-local my-var 'base)
      (let* ((ov (make-overlay 6 15))
             (_ (overlay-put ov 'face 'bold))
             (m (make-marker))
             (_ (set-marker m 8)))
        (narrow-to-region 6 15)
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "BBBB")
        (replace-match (format "%s-XX" my-var))
        (undo-boundary)
        (goto-char (point-min))
        (re-search-forward "XX")
        (replace-match "YY")
        (undo-boundary)
        (let ((mp (marker-position m))
              (os (overlay-start ov))
              (oe (overlay-end ov))
              (v my-var)
              (bs (buffer-substring (point-min) (point-max))))
          (primitive-undo 2 buffer-undo-list)
          (widen)
          (list mp os oe v bs
                (marker-position m)
                (overlay-start ov)
                (overlay-end ov)
                (buffer-string)))))
    (kill-buffer buf)))"#,
    );
}
