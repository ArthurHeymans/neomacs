...
fn oracle_prop_defmacro_basic_definition_and_invocation() {
    ...
    let (oracle, neovm) =
        crate::common::eval_oracle_and_neovm_expect(form, expect_test::expect![[r#""OK (4 4)""#]]);
...
fn oracle_prop_macroexpand_nonmacro_form_passthrough() {
    ...
    let (oracle, neovm) = crate::common::eval_oracle_and_neovm_expect(
        "(macroexpand '(list 1 2 3))",
        expect_test::expect![[r#""OK (list 1 2 3)""#]],
    );
...
fn oracle_prop_defmacro_and_macroexpand_error_shapes() {
    ...
    crate::common::assert_oracle_parity_expect(
        ...
        expect_test::expect![[r#""OK (wrong-number-of-arguments (2 . 2) 0)""#]],
    );
    crate::common::assert_oracle_parity_expect(
    ...
    crate::common::assert_oracle_parity_expect(
        ...
        expect_test::expect![[r#""OK (wrong-type-argument symbolp 'vm-oracle-dm)""#]],
    );
    crate::common::assert_oracle_parity_expect(
        "(condition-case err (macroexpand '(when t 1) 1) (error err))",
        expect_test::expect![[r#""OK (wrong-type-argument listp 1)""#]],
    );
}
...
fn oracle_prop_macroexpand_backquote_splice_shape() {
    ...
    let (oracle, neovm) = crate::common::eval_oracle_and_neovm_expect(
        form,
        expect_test::expect![[r#""OK (list 1 2 3 4)""#]],
    );
...
fn oracle_prop_macroexpand_environment_symbol_callable() {
    ...
    let (oracle, neovm) =
        crate::common::eval_oracle_and_neovm_expect(form, expect_test::expect![[r#""OK '(7 7)""#]]);
...
fn oracle_prop_macroexpand_fallback_macro_shadow_and_override_via_env() {
    ...
    let (oracle_shadow, neovm_shadow) = crate::common::eval_oracle_and_neovm_expect(
        ...
        expect_test::expect![[r#""OK (when t 1 2)""#]],
    );
    ...
    let (oracle_override, neovm_override) = crate::common::eval_oracle_and_neovm_expect(
        override_form,
        expect_test::expect![[r#""OK (if t (progn 1 2) override-tail)""#]],
    );
    assert_ok_eq(
...
fn oracle_prop_macroexpand_quoted_nested_macro_payload_not_walked() {
    ...
    let (oracle, neovm) = crate::common::eval_oracle_and_neovm_expect(
        form,
        expect_test::expect![[r#""OK '(neovm--dm-inner 9)""#]],
    );
...
fn oracle_prop_macroexpand_stops_at_fixpoint_with_identity_expander() {
    ...
    crate::common::assert_oracle_parity_expect(
        form,
        expect_test::expect![[r#""OK (neovm--dm-id 1)""#]],
    );
}
