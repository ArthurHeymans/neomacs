//! Combo: cl-remf + marker + overlay + textprop + buflocal + narrow + undo.
...
#[test]
fn combo_cl_remf_marker_overlay_textprop_buflocal_narrow_undo() {
  ...
  (let ((buf (generate-new-buffer "rfb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (goto-char (point-min))
        (insert (format "%s-%s-%s-" (cl-getf pl 'a) (cl-getf pl 'c) my-var))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_remf_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "rfc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (setf (marker-position m) 11)
          (goto-char (point-min))
          (insert (format "%s-%s-%s-" (cl-getf pl 'x) (cl-getf pl 'z) my-var))
          (undo-boundary)
          (let ((v my-var)
                (mp (marker-position m))
                (os (overlay-start ov))
...
#[test]
fn combo_cl_remf_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "rf1"))
        (b2 (generate-new-buffer "rf2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        ...
        (let ((pl (list 'a 1 'b 2)))
          (cl-remf pl 'a)
          (goto-char 6)
          (insert (format "%s-%s-" (cl-getf pl 'b) my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (let ((pl (list 'x 1 'y 2)))
          (cl-remf pl 'x)
          (goto-char 6)
          (insert (format "%s-%s-" (cl-getf pl 'y) my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_remf_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((buf (generate-new-buffer "rsr")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (replace-match (format "%s-%s-%s" (cl-getf pl 'a) (cl-getf pl 'c) my-var))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_remf_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
