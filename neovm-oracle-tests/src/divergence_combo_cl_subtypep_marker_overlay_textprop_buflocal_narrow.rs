//! Combo: cl-subtypep + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests subtypep with buffer state.
  ...
  (let ((buf (generate-new-buffer "stb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (undo-boundary)
        (let* ((r1 (cl-subtypep 'integer 'number))
               (r2 (cl-subtypep 'string 'sequence))
               (r3 (cl-subtypep 'cons 'list)))
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((r1 (cl-subtypep 'fixnum 'integer))
                 (r2 (cl-subtypep 'string 'list)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
...
fn combo_cl_subtypep_multi_buffer_undo() {
  ...
  (let ((b1 (generate-new-buffer "st1"))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (let ((r (cl-subtypep 'integer 'number)))
          (goto-char 6)
          (insert (format "%s-%s-" r my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (let ((r (cl-subtypep 'string 'sequence)))
          (goto-char 6)
          (insert (format "%s-%s-" r my-var)))
        (undo-boundary))
