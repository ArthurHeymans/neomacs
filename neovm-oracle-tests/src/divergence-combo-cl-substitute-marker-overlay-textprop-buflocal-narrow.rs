//! Combo: cl-substitute/cl-nsubstitute + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests sequence substitution with buffer state.
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((lst (list 'a 'b 'c 'd 'b 'a))
               (result (cl-substitute 'x 'b lst))
               (result2 (cl-substitute-if (lambda (x) (eq x 'c)) 'y lst)))
...
#[test]
fn combo_cl_nsubstitute_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (let* ((lst (list 'a 'b 'c 'b 'a))
                 (result (cl-nsubstitute 'x 'b lst))
                 (result2 (cl-nsubstitute-if (lambda (x) (eq x 'c)) 'y lst)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
