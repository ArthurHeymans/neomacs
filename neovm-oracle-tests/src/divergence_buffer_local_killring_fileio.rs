...
fn divergence_buffer_local_set() {
  ...
  (list my-bl-test
        ...
        expect_test::expect![[r#""OK (42 0 t 42)""#]],
    );
}
...
fn divergence_buffer_local_value_across_buffers() {
  ...
  (let ((buf (generate-new-buffer " *cross-buf-test*")))
    ...
    (prog1
      ...
      (kill-buffer buf))))"#,
        expect_test::expect![[r#""OK (0 55 0)""#]],
    );
}
...
fn divergence_kill_ring_basic() {
  ...
  (list (car kill-ring)
        ...
        (nth 2 kill-ring)
        (length kill-ring)))"#,
        expect_test::expect![[r#""OK (\"third\" \"second\" \"first\" 3)""#]],
    );
}
...
fn divergence_kill_ring_max_size() {
  ...
  (list (length kill-ring)
        ...
        (nth 1 kill-ring)
        (nth 2 kill-ring)))"#,
        expect_test::expect![[r#""OK (3 \"4\" \"3\" \"2\")""#]],
    );
}
      ...
      (progn
        ...
        (list (file-exists-p tmp)
              ...
              (with-temp-buffer
                ...
                (buffer-string))))
    (delete-file tmp)))"#,
        expect_test::expect![[r#""OK (t t t \"hello\")""#]],
    );
}
