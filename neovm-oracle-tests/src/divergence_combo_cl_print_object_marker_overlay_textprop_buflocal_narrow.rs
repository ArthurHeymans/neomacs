//! Combo: cl-print-object + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests print-object with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_print_object_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "pob")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (undo-boundary)
        (let* ((obj (list 'a 'b 'c 'd 'e))
               (result (cl-subseq obj 1 3)))
...
#[test]
fn combo_cl_print_object_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "poc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "poc-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((obj (list 'x 'y 'z 'w 'v))
                 (result (cl-subseq obj 2 4)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
...
#[test]
fn combo_cl_print_object_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((b1 (generate-new-buffer "po1"))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (let* ((obj (list 'a 'b 'c 'd))
               (result (cl-subseq obj 0 2)))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (let* ((obj (list 'd 'e 'f 'g))
               (result (cl-subseq obj 1 3)))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_print_object_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_print_object_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
