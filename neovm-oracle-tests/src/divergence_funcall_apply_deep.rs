...
fn divergence_funcall_basic() {
  ...
  (funcall 'concat "a" "b" "c")) "#,
        expect_test::expect![[r#""OK (3 (1 2 3) 25 \"abc\")""#]],
    );
}
...
fn divergence_apply_basic() {
  ...
  (apply 'list nil)
  (apply 'vector '(1 2 3))) "#,
        expect_test::expect![[r#""OK (6 10 nil [1 2 3])""#]],
    );
}
...
fn divergence_funcall_composed() {
  ...
  (fboundp 'apply-partially)) "#,
        expect_test::expect![[r#""OK (3 (a b c) t)""#]],
    );
}
...
fn divergence_higher_order() {
  ...
  (list (funcall (nth 0 fns) '(a b c))
        ...
        expect_test::expect![[r#""OK (a (b c) b)""#]],
    );
}
...
fn divergence_closure_capture() {
  ...
  (let ((add5 (funcall make-adder 5))
    ...
    (list (funcall add5 3)
          (funcall add10 3)
          (funcall add5 10)))) "#,
        expect_test::expect![[r#""OK (8 13 15)""#]],
    );
}
...
fn divergence_lambda_varargs() {
  ...
  (funcall (lambda (&optional x y) (list x y)) 1 2)) "#,
        expect_test::expect![[r#""OK ((1 2 3) (1 2 3) (1 2))""#]],
    );
}
...
fn divergence_funcall_inline() {
  ...
  (funcall (lambda (s) (downcase s)) "HELLO")) "#,
        expect_test::expect![[r#""OK ((3 9 27) \"HELLO\" \"hello\")""#]],
    );
}
...
fn divergence_recursive_funcall() {
  ...
  (list (funcall fact 1)
        (funcall fact 5)
        (funcall fact 10))) "#,
        expect_test::expect![[r#""OK (1 120 3628800)""#]],
    );
}
...
fn divergence_function_quote() {
  ...
  (type-of (lambda (x) x))
  (type-of 'car)) "#,
        expect_test::expect![[r#""OK (t t interpreted-function symbol)""#]],
    );
}
...
fn divergence_advice_funcall() {
  ...
  (let ((orig (symbol-function 'test-fn-adv-xxx)))
    (list (funcall orig 5)
        ...
        expect_test::expect![[r#""OK (10 10 t)""#]],
    );
}
