//! Combo: marker + overlay + textprop + buflocal + narrow + clone + setf + replace + undo + multi-buffer + undo.
...
#[test]
fn combo_marker_overlay_textprop_buflocal_narrow_clone_setf_replace_undo_multi_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "n1a"))
        (b2 (generate-new-buffer "n1b")))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "n1c")))
           (c2 (with-current-buffer b2 (clone-buffer "n1d"))))
...
#[test]
fn combo_marker_overlay_textprop_buflocal_narrow_clone_replace_setf_undo_multi_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "r2a"))
        (b2 (generate-new-buffer "r2b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "r2c")))
           (c2 (with-current-buffer b2 (clone-buffer "r2d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_marker_overlay_textprop_buflocal_narrow_clone_multi_overlay_undo_multi_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "x1a"))
        (b2 (generate-new-buffer "x1b")))
    (with-current-buffer b1
    ...
    (let* ((ov1a (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "x1c")))
           (c2 (with-current-buffer b2 (clone-buffer "x1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
...
#[test]
fn combo_marker_overlay_textprop_buflocal_narrow_clone_textprop_replace_undo_multi_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "y1a"))
        (b2 (generate-new-buffer "y1b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'kind 'a)
      (put-text-property 6 10 'kind 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "y1c")))
           (c2 (with-current-buffer b2 (clone-buffer "y1d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_marker_overlay_textprop_buflocal_narrow_clone_setf_replace_undo_multi_undo_2() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "z2a"))
        (b2 (generate-new-buffer "z2b")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "z2c")))
           (c2 (with-current-buffer b2 (clone-buffer "z2d"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
