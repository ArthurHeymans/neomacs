//! Combo: cl-ecase + markers + overlays + textprop + undo.
//! Tests exhaustive case dispatch with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_ecase_marker_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "cec")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 10))
        ...
        (undo-boundary)
        (cl-ecase 'b
          (a (goto-char 1) (insert "A"))
          (b (goto-char 6) (insert "B-"))
          (c (goto-char 11) (insert "C")))
        (undo-boundary)
...
#[test]
fn combo_cl_ecase_narrow_marker() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "cen")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
      ...
      (let* ((ov (make-overlay 1 15))
        ...
        (cl-ecase 'b
          (a (goto-char 1) (insert "A"))
          (b (goto-char (point-min)) (insert "XX"))
          (c (goto-char 11) (insert "C")))
        (undo-boundary)
        (let ((mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_ecase_clone_overlay() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "cel")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
      (put-text-property 1 5 'seg 'a)
      (put-text-property 6 10 'seg 'b)
  ...
  (let ((buf (generate-new-buffer "cem")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 10))
        ...
        (cl-ecase 'b
          (a (goto-char 1) (insert "A"))
          (b (goto-char 6) (insert "XX")
             (goto-char 12) (insert "YY"))
          (c (goto-char 11) (insert "C")))
        (undo-boundary)
        (let ((mp (marker-position m))
...
#[test]
fn combo_cl_ecase_overlay_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "ceo")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
