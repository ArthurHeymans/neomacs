...
fn divergence_format_propertize() {
    ...
    crate::common::assert_oracle_parity_expect(
        ...
        expect_test::expect![[r#""ERR (void-function format-propertize)""#]],
    );
}
...
fn divergence_print_circle() {
  ...
  (setcar (nthcdr 2 x) x)
  (prin1-to-string x))"#,
        expect_test::expect![[r##""OK \"#1=(1 2 #1#)\"""##]],
    );
}
...
fn divergence_print_gensym() {
  ...
  (list (symbol-name gs)
        (prin1-to-string gs)))"#,
        expect_test::expect![[r##""OK (\"test-0\" \"#:test-0\")""##]],
    );
}
...
fn divergence_charset_map() {
  ...
  (decode-char 'ascii 65))"#,
        expect_test::expect![[r#""OK (t t 65 65)""#]],
    );
}
...
fn divergence_char_width() {
    ...
    crate::common::assert_oracle_parity_expect(
        r#"(list (char-width ?A)
              ...
              (char-width ?ā)
              (char-width ? ))"#,
        expect_test::expect![[r#""OK (1 2 1 1)""#]],
    );
}
...
fn divergence_truncate_string() {
  ...
  (truncate-string-to-width "abcdefghij" 5 nil ?…))"#,
        expect_test::expect![[r#""OK (\"abcde\" \"abcde\")""#]],
    );
}
