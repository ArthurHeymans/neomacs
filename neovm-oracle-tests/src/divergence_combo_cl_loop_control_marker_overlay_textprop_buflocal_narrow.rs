//! Combo: cl-loop with while/until/repeat + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests loop control flow with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_loop_while_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "lwb")))
...
#[test]
fn combo_cl_loop_until_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "luc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (let* ((i 0)
                 (acc nil)
                 (result (cl-loop do (push (list i) acc)
                                  (cl-incf i)
                                  until (>= i 3)
                                  finally (return (nreverse acc)))))
            (put-text-property (point-min) (point-max) 'z 'changed)
...
#[test]
fn combo_cl_loop_repeat_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "lr1"))
        (b2 (generate-new-buffer "lr2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        ...
        (let ((acc nil))
          (cl-loop repeat 3
                   do (push (list (cl-incf (car acc))) acc))
          (goto-char 6)
...
#[test]
fn combo_cl_loop_while_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_loop_until_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
