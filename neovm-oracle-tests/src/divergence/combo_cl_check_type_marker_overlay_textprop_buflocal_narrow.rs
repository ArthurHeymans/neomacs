//! Combo: cl-check-type + marker + overlay + textprop + buflocal + narrow + undo.
...
#[test]
fn combo_cl_check_type_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "ctb")))
...
#[test]
fn combo_cl_check_type_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_check_type_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "ct1"))
        (b2 (generate-new-buffer "ct2")))
...
#[test]
fn combo_cl_check_type_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_check_type_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
