//! Combo: cl-pushnew + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests conditional push with buffer state.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_pushnew_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "pnb")))
...
#[test]
fn combo_cl_pushnew_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "pnc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_pushnew_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "pn1"))
        (b2 (generate-new-buffer "pn2")))
...
#[test]
fn combo_cl_pushnew_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((buf (generate-new-buffer "psr")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (replace-match (format "%s-%s" lst my-var))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_pushnew_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
