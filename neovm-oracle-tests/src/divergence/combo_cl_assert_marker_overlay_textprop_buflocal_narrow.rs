//! Combo: cl-assert + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests assertions with buffer state.
...
fn combo_cl_assert_marker_overlay_textprop_buflocal_narrow_undo() {
    ...
    let expect = expect_test::expect![[r#""ERR (cl-assertion-failed (stringp my-var))""#]];
