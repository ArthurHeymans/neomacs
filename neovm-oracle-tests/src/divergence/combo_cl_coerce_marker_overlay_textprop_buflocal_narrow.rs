//! Combo: cl-coerce + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests coerce with buffer state.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_coerce_marker_overlay_textprop_buflocal_narrow_undo() {
  ...
  (let ((buf (generate-new-buffer "cob")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (undo-boundary)
        (let* ((r1 (cl-coerce '(a b c) 'vector))
               (r2 (cl-coerce "hello" 'list))
               (r3 (cl-coerce 65 'string)))
...
#[test]
fn combo_cl_coerce_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((r1 (cl-coerce '(x y z) 'vector))
                 (r2 (cl-coerce "world" 'list)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
...
#[test]
fn combo_cl_coerce_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((b1 (generate-new-buffer "co1"))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (let ((r (cl-coerce '(a b c) 'vector)))
          (goto-char 6)
          (insert (format "%s-%s-" r my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (let ((r (cl-coerce "hello" 'list)))
          (goto-char 6)
          (insert (format "%s-%s-" r my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_coerce_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_coerce_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
