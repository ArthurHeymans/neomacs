//! Complex combo batch 250 — MEGA 250th batch milestone: combine all
//! major subsystem axes into 5 extreme integration stress tests.
...
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX250=v1" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX250"))))
      (exit-code (let ((p (make-process :name "neo-cx250-ec1"
                                          :command '("sh" "-c" "exit 5")))
      ...
      (rec (record 'neo-cx250-tag :a :b :c))
      (lexical-binding t)
      (calls nil))
  ...
  (puthash "alpha" 1 ht)
  (puthash "beta" 2 ht)
  (defun neo-cx250-target (x) (push (list :primary x) calls) (* x 2))
  (advice-add 'neo-cx250-target :before
              (lambda (x) (push (list :before x) calls))
              '((name . mega-advice)))
  (let* ((text "café 世界 😀 mega-250")
    ...
    (let ((buf (get-buffer-create " *neo-cx250-mega-1*")))
      (with-current-buffer buf
        (buffer-enable-undo)
        (setq-local neo-cx250-counter 0)
        (insert text)
        (put-text-property 1 4 'face 'bold)
        (put-text-property 6 10 'display "XX")
        (add-to-invisibility-spec 'neo-cx250-h)
        (let ((m (set-marker (make-marker) 8))
              (invis-ov (make-overlay 3 8))
              (face-ov (make-overlay 6 14)))
          (overlay-put invis-ov 'invisible 'neo-cx250-h)
          (overlay-put face-ov 'face 'italic)
          (overlay-put face-ov 'priority 5)
          (narrow-to-region 2 16)
          (cl-incf neo-cx250-counter)
          (aset rec 2 :modified)
          (delete-region 5 8)
          (insert "MEGA")
          (cl-incf neo-cx250-counter)
          (let ((r (neo-cx250-target 5)))
            (let ((state (list timer-fired env-val exit-code
              ...
              (undo) (undo) (undo)
              (widen)
              (advice-remove 'neo-cx250-target 'mega-advice)
              (kill-buffer buf)
...
(let ((lexical-binding t)
  ...
  (defun neo-cx250-target () :orig)
  (advice-add 'neo-cx250-target :override (lambda () :overridden))
  (with-temp-buffer
    (buffer-enable-undo)
    ...
    (let ((m (set-marker (make-marker) 12))
      ...
      (let* ((matches (cl-loop for i from 0 below 3
             ...
             (advice-result (neo-cx250-target)))
        (let ((snapshot (list matches pcase-result time-str advice-result
          ...
          (widen)
          (jump-to-register ?c)
          (advice-remove 'neo-cx250-target (advice--p (advice-member-p nil 'neo-cx250-target)))
...
fn div_cx250_mega_milestone_process_buflocal_coding_env_timer_weak_hash_textprop_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let expect = expect_test::expect![[r#""ERR (wrong-number-of-arguments puthash 6)""#]];
    crate::common::assert_oracle_parity_expect(
        r##"
(let ((timer-fired nil)
      (env-val (let ((process-environment (cons "NEO_CX250=v2" process-environment)))
                 (string-trim (shell-command-to-string "echo $NEO_CX250"))))
      (exit-code (let ((p (make-process :name "neo-cx250-ec2"
                                          :command '("sh" "-c" "exit 8")))
                       (weak-ht (make-hash-table :weakness 'key :test 'eq)))
        (puthash (cons 1 nil) :v weak-ht
        (garbage-collect)
  ...
  (run-with-timer 0 nil (lambda () (setq timer-fired :t)))
  (sit-for 0.01)
  (let ((buf (get-buffer-create " *neo-cx250-final*")))
    (with-current-buffer buf
      (buffer-enable-undo)
      (setq-local neo-cx250-buf :local)
      (insert "Mega 250 café 世界 buffer content")
      (put-text-property 1 6 'face 'bold)
      (put-text-property 8 14 'display "XX"))
    (let ((p (make-process :name "neo-cx250-mega-p"
                           :command '("sh" "-c" "printf 'MEGA250'")
                           :buffer buf)))
