//! Combo: cl-etypecase + markers + overlays + textprop + undo.
...
#[test]
fn combo_cl_etypecase_marker_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "cet")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 10))
        ...
        (cl-etypecase "hello"
          ...
          (integer (goto-char 6) (insert "INT-")))
        (undo-boundary)
...
#[test]
fn combo_cl_etypecase_narrow_marker() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((buf (generate-new-buffer "cen")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 1 15))
        ...
        (undo-boundary)
        (cl-etypecase "test"
          (string (goto-char (point-min)) (insert "XX"))
          (integer (goto-char (point-min)) (insert "YY")))
        (undo-boundary)
        (let ((mp (marker-position m))
              (os (overlay-start ov))
              (oe (overlay-end ov))
...
#[test]
fn combo_cl_etypecase_clone_overlay() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_etypecase_multi_type() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((buf (generate-new-buffer "cem")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 10))
        ...
        (cl-etypecase "hello"
          (string
           (goto-char 6) (insert "S1")
           (goto-char 12) (insert "S2"))
          (integer
           (goto-char 6) (insert "I1")))
        (undo-boundary)
        (let ((mp (marker-position m))
...
#[test]
fn combo_cl_etypecase_overlay_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
