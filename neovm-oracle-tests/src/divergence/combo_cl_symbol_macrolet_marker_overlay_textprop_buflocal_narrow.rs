//! Combo: cl-symbol-macrolet + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests symbol macros with buffer state.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_symbol_macrolet_marker_overlay_textprop_buflocal_narrow_undo() {
...
#[test]
fn combo_cl_symbol_macrolet_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_symbol_macrolet_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (wrong-type-argument sequencep b1-base)""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        ...
        (cl-symbol-macrolet ((my-str "XX"))
          (goto-char 6)
          (insert (concat my-str "-" my-var "-")))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (cl-symbol-macrolet ((my-str "YY"))
          (goto-char 6)
          (insert (concat my-str "-" my-var "-")))
        (undo-boundary))
...
#[test]
fn combo_cl_symbol_macrolet_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (cl-symbol-macrolet ((my-str "XX"))
          ...
          (replace-match (concat my-str "-" my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_symbol_macrolet_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
