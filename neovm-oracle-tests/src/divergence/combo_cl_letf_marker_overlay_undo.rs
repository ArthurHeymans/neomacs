//! Combo: cl-letf + markers + overlays + textprop + undo.
//! Tests cl-letf bindings with buffer state.
  ...
  (let ((buf (generate-new-buffer "clf")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 10))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (cl-letf (((symbol-function 'my-goto)
                   (lambda (p) (goto-char p))))
          (my-goto 6)
          (insert "XX-"))
        (undo-boundary)
...
#[test]
fn combo_cl_letf_clone_overlay() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "clc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
      (put-text-property 1 5 'seg 'a)
      (put-text-property 6 10 'seg 'b)
  ...
  (let ((buf (generate-new-buffer "clm")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 10))
        ...
        (cl-letf (((symbol-function 'my-goto)
                   (lambda (p) (goto-char p)))
                  ((symbol-function 'my-ins)
                   (lambda (p s) (goto-char p) (insert s))))
          (my-ins 6 "XX")
          (my-ins 12 "YY"))
        (undo-boundary)
        (let ((mp (marker-position m))
