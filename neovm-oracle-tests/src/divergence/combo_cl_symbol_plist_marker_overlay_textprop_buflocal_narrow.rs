//! Combo: cl-symbol-plist/cl-setplist + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests symbol plist operations with buffer state.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_symbol_plist_marker_overlay_textprop_buflocal_narrow_undo() {
  ...
  (let ((buf (generate-new-buffer "spb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (undo-boundary)
        (let* ((sym (make-symbol "test-sym"))
               (_ (setplist sym (list 'a 1 'b 2 'c 3)))
               (pl (symbol-plist sym))
               (v1 (cl-getf pl 'b)))
          ...
          (goto-char (point-min))
          (insert (format "%s-%s-%s-" pl v1 my-var)))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_symbol_plist_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "spc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "spc-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (let* ((sym (make-symbol "test-sym"))
                 (_ (setplist sym (list 'x 10 'y 20)))
                 (pl (symbol-plist sym))
                 (v (cl-getf pl 'y)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
            (insert (format "%s-%s-%s-" pl v my-var)))
          (undo-boundary)
          (let ((v my-var)
                (mp (marker-position m))
                (os (overlay-start ov))
...
#[test]
fn combo_cl_symbol_plist_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "sp1"))
        (b2 (generate-new-buffer "sp2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        ...
        (let* ((sym (make-symbol "s1"))
               (_ (setplist sym (list 'a 1 'b 2)))
               (v (cl-getf (symbol-plist sym) 'b)))
          (goto-char 6)
      ...
      (with-current-buffer b2
        ...
        (let* ((sym (make-symbol "s2"))
               (_ (setplist sym (list 'd 4 'e 5)))
               (v (cl-getf (symbol-plist sym) 'e)))
          (goto-char 6)
          (insert (format "%s-%s-" v my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_symbol_plist_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_symbol_plist_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
