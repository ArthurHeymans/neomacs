//! Combo: cl-macrolet + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests local macros with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_macrolet_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mlb")))
...
#[test]
fn combo_cl_macrolet_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_macrolet_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_macrolet_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((buf (generate-new-buffer "msr")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (cl-macrolet ((my-replace (old new)
...
#[test]
fn combo_cl_macrolet_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
