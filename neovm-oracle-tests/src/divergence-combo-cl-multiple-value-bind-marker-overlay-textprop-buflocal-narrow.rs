//! Combo: cl-multiple-value-bind + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests multiple value bindings with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_multiple_value_bind_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mvb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (cl-multiple-value-bind (a b c)
          ...
          (goto-char (point-min))
          (insert (format "%s-%s-%s-%s-" a b c my-var)))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_multiple_value_bind_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "mvc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (cl-multiple-value-bind (a b)
            ...
            (setf (marker-position m) 11)
            (goto-char (point-min))
            (insert (format "%s-%s-%s-" a b my-var)))
          (undo-boundary)
          (let ((v my-var)
                (mp (marker-position m))
                (os (overlay-start ov))
...
#[test]
fn combo_cl_multiple_value_bind_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "mv1"))
        (b2 (generate-new-buffer "mv2")))
...
#[test]
fn combo_cl_multiple_value_bind_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (cl-multiple-value-bind (a b)
          ...
          (replace-match (format "%s-%s-%s" a b my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_multiple_value_bind_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
