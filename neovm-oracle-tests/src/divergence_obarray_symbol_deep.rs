...
fn divergence_intern_soft() {
  ...
  (null (intern-soft "nonexistent-symbol-xyz-456"))) "#,
        expect_test::expect![[r#""OK (car nil t t)""#]],
    );
}
...
fn divergence_intern_create() {
  ...
  (list (symbolp sym)
        ...
        expect_test::expect![[r#""OK (t t test-intern-symbol-unique-999)""#]],
    );
}
...
fn divergence_symbol_function_boundp() {
  ...
  (null (fboundp 'nonexistent-fn-xyz))) "#,
        expect_test::expect![[r#""OK (t t nil #<subr car> t)""#]],
    );
}
...
fn divergence_symbol_plist() {
  ...
  (list (get sym 'test-prop)
        (symbol-plist sym)
        (get sym 'nonexistent))) "#,
        expect_test::expect![[r#""OK (test-val (test-prop test-val) nil)""#]],
    );
}
...
fn divergence_symbol_name() {
  ...
  (equal (symbol-name 'foo) "foo"))"#,
        expect_test::expect![[r#""OK (\"car\" \"cdr\" t t)""#]],
    );
}
...
fn divergence_make_symbol_uninterned() {
  ...
  (list (symbolp sym)
        ...
        expect_test::expect![[r#""OK (t \"uninterned\" t t)""#]],
    );
}
...
fn divergence_keyword_symbol() {
  ...
  (equal (symbol-name :hello) "hello"))"#,
        expect_test::expect![[r#""OK (t nil t \":hello\" nil)""#]],
    );
}
