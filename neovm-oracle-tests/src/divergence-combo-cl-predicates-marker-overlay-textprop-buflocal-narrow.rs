//! Combo: cl-every/cl-some/cl-notany/cl-notevery + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests sequence predicates with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_every_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "evb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((lst1 (list 'a 'b 'c))
               (lst2 (list 'a 'x 'c))
               (ev (cl-every (lambda (x) (memq x '(a b c))) lst1))
               (sm (cl-some (lambda (x) (eq x 'x)) lst2))
               (na (cl-notany (lambda (x) (eq x 'z)) lst1))
               (ne (cl-notevery (lambda (x) (eq x 'a)) lst1)))
          (put-text-property (point-min) (point-max) 'z 'changed)
          (setf (char-after (point-min)) ?Z)
          (setf (marker-position m) 11)
          (goto-char (point-min))
          (insert (format "%s-%s-%s-%s-%s-" ev sm na ne my-var)))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_some_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (let* ((lst (list 'a 'b 'c 'd))
                 (ev (cl-every (lambda (x) (memq x '(a b c d))) lst))
                 (sm (cl-some (lambda (x) (eq x 'c)) lst)))
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
            (insert (format "%s-%s-%s-" ev sm my-var)))
          (undo-boundary)
          (let ((v my-var)
                (mp (marker-position m))
                (os (overlay-start ov))
...
#[test]
fn combo_cl_notany_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((b1 (generate-new-buffer "na1"))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (let* ((lst (list 'a 'b 'c))
               (na (cl-notany (lambda (x) (eq x 'z)) lst)))
          (goto-char 6)
          (insert (format "%s-%s-" na my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (let* ((lst (list 'd 'e 'f))
               (ne (cl-notevery (lambda (x) (eq x 'd)) lst)))
          (goto-char 6)
          (insert (format "%s-%s-" ne my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_notevery_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((lst (list 'a 'b 'c 'a 'b))
               (ev (cl-every (lambda (x) (memq x '(a b c))) lst)))
          (setf (char-after 6) ?Z)
          (setf (marker-position m) 11)
          (goto-char 6)
          (re-search-forward "BBBB")
          (replace-match (format "%s-%s" ev my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_every_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "emo")))
    (with-current-buffer buf
