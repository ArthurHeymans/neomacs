//! Strong state-max oracle tests — maximum mutable state capture.
//!
...
// ═══════════════════════════════════════════════════════════════════════
// State max: headline + todo + tags + priority after edit
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_headline_edit_all() {
...
// ═══════════════════════════════════════════════════════════════════════
// State max: property cycle
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_property_cycle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: property inheritance
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_property_inherit() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
  ...
  (let ((v3i (org-entry-get nil "V" 'inherit))
    ...
    (let ((v2i (org-entry-get nil "V" 'inherit))
      ...
      (let ((v1i (org-entry-get nil "V" 'inherit))
        ...
        expect_test::expect![[r#""OK (\"l1\" \"l1\" nil \"l1\" \"l3\" \"l3\")""#]],
    );
}

// ═══════════════════════════════════════════════════════════════════════
// State max: table formula
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// State max: checkbox stats
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_checkbox_stats() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
  ...
  (let ((h0 (buffer-substring-no-properties (line-beginning-position) (line-end-position))))
    ...
    (let ((h1 (buffer-substring-no-properties (line-beginning-position) (line-end-position))))
      ...
      (let ((h2 (buffer-substring-no-properties (line-beginning-position) (line-end-position))))
        (list h0 h1 h2)))))"##,
        expect_test::expect![[r#""OK (\"* T [0%]\" \"* T [33%]\" \"* T [0%]\")""#]],
    );
}

// ═══════════════════════════════════════════════════════════════════════
// State max: sparse tree
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// State max: element parse modify
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_element_parse_modify() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: planning repeaters
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_planning_repeaters() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: timestamp repeater
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_timestamp_repeater() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
fn sm_drawer_types() {
  ...
  (let* ((tree (org-element-parse-buffer))
    ...
    dr))"##,
        expect_test::expect![[r#""OK ((\"LOGBOOK\" 30 53) (\"MYDRAWER\" 53 77))""#]],
    );
}

// ═══════════════════════════════════════════════════════════════════════
// State max: block switches
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_block_switches() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: footnote markup
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_footnote_markup() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
         ...
         (fd (org-element-map tree 'footnote-definition
               (lambda (d)
                 (list (org-element-property :label d)
                       ...
                       (org-element-property :end d))))))
    (list fn fd)))"##,
        expect_test::expect![[r#""OK ((\"1\" \"2\") ((\"1\" 24 47) (\"2\" 47 68)))""#]],
    );
}

// ═══════════════════════════════════════════════════════════════════════
// State max: inline task
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// State max: hierarchy contents
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_hierarchy_contents() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: agenda entries
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_agenda_entries() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: colview format
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_colview_format() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: macro expansion
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_macro_expansion() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: entity replacement
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_entity_replacement() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
fn sm_radio_targets() {
  ...
  (let* ((tree (org-element-parse-buffer))
    ...
    targets))"##,
        expect_test::expect![[r#""OK (\"target1\" \"target2\")""#]],
    );
}

// ═══════════════════════════════════════════════════════════════════════
// State max: structure template
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_structure_template() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
...
// ═══════════════════════════════════════════════════════════════════════
// State max: refile targets
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn sm_refile_targets() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    crate::common::assert_oracle_parity_expect(
        r##"(with-temp-buffer
  (org-mode)
