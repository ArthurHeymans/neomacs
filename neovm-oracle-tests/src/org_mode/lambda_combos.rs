//! Lambda-strict combo tests for org-mode extreme edge cases.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex headline + tags + todo + priority combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_headline_all_features() {
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex planning + clock + property combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_planning_clock_property_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex table + formula + export combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_table_formula_export_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex list + checkbox + description combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_list_checkbox_description_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex link + footnote + citation combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_link_footnote_citation_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex block + drawer + property combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_block_drawer_property_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex inline markup combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_inline_markup_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex timestamp + planning combos
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_timestamp_planning_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex export headline features combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_export_headline_features_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex export footnote edge cases combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_export_footnote_edge_cases_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex export options combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_export_options_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex export backend chain combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_export_backend_chain_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex property inheritance chain combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_property_inheritance_chain_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex element operations chain combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_element_operations_chain_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex deferred chain combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_deferred_chain_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex parse-and-interpret round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_parse_interpret_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex link round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_link_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex footnote round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_footnote_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex block round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_block_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex inline round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_inline_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex table round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_table_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex timestamp round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_timestamp_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex keyword/comment round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_keyword_comment_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Lambda: org-element with complex citation round-trip combo
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn lambda_citation_roundtrip_combo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
