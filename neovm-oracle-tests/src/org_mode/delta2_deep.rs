//! Delta-2 strict combo tests for org-mode extreme edge cases.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex document parsing (all element types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_full_document_all_elements() {
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export round-trip (all features)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_roundtrip_all_features() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex property inheritance (4 levels)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_property_inheritance_4_levels() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex element operations chain
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_element_operations_chain() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex deferred chain
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_deferred_chain() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex parse-and-interpret round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_parse_interpret_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex link round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_link_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex footnote round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_footnote_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex block round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_block_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex inline round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_inline_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex table round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_table_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex timestamp round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_timestamp_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex keyword/comment round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_keyword_comment_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex citation round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_citation_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export options (all 24+)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_all_options() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export headline numbers (all levels)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_headline_numbers_all_levels() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export footnote numbers (all types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_footnote_numbers_all_types() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export tags/categories
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_tags_categories() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export first/last sibling
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_first_last_sibling() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export filter apply
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_filter_apply() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export backend chain
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_backend_chain() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export read-attribute
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_read_attribute() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export caption
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_caption() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export optional title
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_optional_title() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Delta-2: org-element with complex export node property
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn delta2_export_node_property() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
