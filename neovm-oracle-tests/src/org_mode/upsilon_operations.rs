//! Upsilon-strict combo tests for org-mode extreme edge cases.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex table formulas (all types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_table_formula_sum() {
...
#[test]
fn upsilon_table_formula_multiply() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_formula_column_sum() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_formula_with_title_row() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_formula_remote() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex table operations (all types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_table_align() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_insert_column() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_delete_column() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_move_column_right() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_move_column_left() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_move_row_down() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_table_convert_region() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex table references
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_table_convert_refs_to_an() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-table)
...
#[test]
fn upsilon_table_convert_refs_to_rc() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-table)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex list operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_list_struct() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex timer operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_timer_secs_to_hms() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-timer)
...
#[test]
fn upsilon_timer_fix_incomplete() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-timer)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex duration operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_duration_to_minutes() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
#[test]
fn upsilon_duration_from_minutes() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-duration)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex column view
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_columns_compile_format() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-colview)
...
#[test]
fn upsilon_columns_uncompile_format() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-colview)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex macro operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_macro_replace_all() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-macro)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex footnote operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_footnote_new() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex archive operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_archive_subtree() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-archive)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex datetree operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_datetree_find_date_create() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex protocol operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_protocol_parse_parameters() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex pcomplete operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_pcomplete_entity() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex fold operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_fold_hide_drawer_toggle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-fold)
...
#[test]
fn upsilon_fold_hide_block_toggle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-fold)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex num operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_num_max_level() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-num)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex capture operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_capture_fill_template() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-capture)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex clock operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_clock_table_data() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex refile operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_refile_get_targets() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex sparse tree
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_match_sparse_tree() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex tag operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_toggle_tag() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex todo operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_todo_cycle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex property operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_entry_get() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex planning operations
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex repeat/timestamp
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_get_repeat() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn upsilon_timestamp_has_time_p() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Upsilon: org-element with complex category
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn upsilon_get_category() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
