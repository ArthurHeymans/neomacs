//! Epsilon-2 strict combo tests for org-mode extreme edge cases.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex table formulas (all types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_table_formula_sum() {
...
#[test]
fn epsilon2_table_formula_multiply() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_formula_column_sum() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_formula_with_title_row() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_formula_remote() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex table operations (all types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_table_align() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_insert_column() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_delete_column() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_move_column_right() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_move_column_left() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_move_row_down() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_table_convert_region() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex table references
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_table_convert_refs_to_an() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-table)
...
#[test]
fn epsilon2_table_convert_refs_to_rc() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-table)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex list operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_list_struct() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex timer operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_timer_secs_to_hms() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-timer)
...
#[test]
fn epsilon2_timer_fix_incomplete() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-timer)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex duration operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_duration_to_minutes() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
#[test]
fn epsilon2_duration_from_minutes() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-duration)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex column view
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_columns_compile_format() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-colview)
...
#[test]
fn epsilon2_columns_uncompile_format() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-colview)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex macro operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_macro_replace_all() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-macro)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex footnote operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_footnote_new() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex archive operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_archive_subtree() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-archive)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex datetree operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_datetree_find_date_create() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex protocol operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_protocol_parse_parameters() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex pcomplete operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_pcomplete_entity() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex fold operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_fold_hide_drawer_toggle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-fold)
...
#[test]
fn epsilon2_fold_hide_block_toggle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-fold)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex num operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_num_max_level() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-num)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex capture operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_capture_fill_template() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-capture)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex clock operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_clock_table_data() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex refile operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_refile_get_targets() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex sparse tree
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_match_sparse_tree() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex tag operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_toggle_tag() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex todo operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_todo_cycle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex property operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_entry_get() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex planning operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_deadline() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex repeat/timestamp
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_get_repeat() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn epsilon2_timestamp_has_time_p() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Epsilon-2: org-element with complex category
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn epsilon2_get_category() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
