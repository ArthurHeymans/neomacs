//! Omicron-strict combo tests for org-mode extreme edge cases.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex document parsing (all element types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_full_document_all_elements() {
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export round-trip (all features)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_roundtrip_all_features() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex property inheritance (4 levels)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_property_inheritance_4_levels() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex element operations chain
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_element_operations_chain() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex deferred chain
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_deferred_chain() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-element)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex parse-and-interpret round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_parse_interpret_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex link round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_link_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex footnote round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_footnote_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex block round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_block_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex inline round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_inline_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex table round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_table_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex timestamp round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_timestamp_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex keyword/comment round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_keyword_comment_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex citation round-trips
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_citation_roundtrips() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export options (all 24+)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_all_options() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export headline numbers (all levels)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_headline_numbers_all_levels() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export footnote numbers (all types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_footnote_numbers_all_types() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export tags/categories
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_tags_categories() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export first/last sibling
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_first_last_sibling() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export filter apply
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_filter_apply() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export backend chain
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_backend_chain() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export read-attribute
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_read_attribute() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export caption
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_caption() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export optional title
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_optional_title() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
...
// ═══════════════════════════════════════════════════════════════════════
// Omicron: org-element with complex export node property
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn omicron_export_node_property() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'ox)
