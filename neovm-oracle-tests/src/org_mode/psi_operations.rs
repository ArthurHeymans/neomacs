//! Psi-strict combo tests for org-mode extreme edge cases.

use crate::common::assert_oracle_parity;
use crate::common::return_if_neovm_enable_oracle_proptest_not_set;

// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex table formulas (all types)
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_table_formula_sum() {
...
#[test]
fn psi_table_formula_column_sum() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
#[test]
fn psi_table_formula_with_title_row() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex table operations (all types)
// ═══════════════════════════════════════════════════════════════════════
...
#[test]
fn psi_table_move_column_right() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex table references
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_table_convert_refs_to_an() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-table)
...
#[test]
fn psi_table_convert_refs_to_rc() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-table)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex list operations
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex timer operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_timer_secs_to_hms() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-timer)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex duration operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_duration_to_minutes() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex column view
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_columns_compile_format() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-colview)
...
#[test]
fn psi_columns_uncompile_format() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-colview)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex macro operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_macro_replace_all() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-macro)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex footnote operations
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex archive operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_archive_subtree() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-archive)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex datetree operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_datetree_find_date_create() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex protocol operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_protocol_parse_parameters() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex pcomplete operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_pcomplete_entity() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex fold operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_fold_hide_drawer_toggle() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-fold)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex num operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_num_max_level() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-num)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex capture operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_capture_fill_template() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org-capture)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex clock operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_clock_table_data() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex refile operations
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_refile_get_targets() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex sparse tree
// ═══════════════════════════════════════════════════════════════════════

#[test]
fn psi_match_sparse_tree() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    assert_oracle_parity(
        r##"(progn
  (require 'org)
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex tag operations
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex todo operations
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex property operations
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex planning operations
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex repeat/timestamp
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// Psi: org-element with complex category
// ═══════════════════════════════════════════════════════════════════════
