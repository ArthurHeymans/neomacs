//! Combo: marker + overlay + textprop + buflocal + narrow + clone + setf + replace + undo + multi-buffer.
  ...
  (let ((b1 (generate-new-buffer "mnb"))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "mnc")))
           (c2 (with-current-buffer b2 (clone-buffer "mnf"))))
...
fn combo_marker_overlay_textprop_buflocal_narrow_clone_replace_setf_undo_multi() {
  ...
  (let ((b1 (generate-new-buffer "orb"))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "orc")))
           (c2 (with-current-buffer b2 (clone-buffer "orf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
fn combo_marker_overlay_textprop_buflocal_narrow_clone_multi_overlay_undo_multi() {
  ...
  (let ((b1 (generate-new-buffer "xmb"))
    ...
    (let* ((ov1a (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "xmc")))
           (c2 (with-current-buffer b2 (clone-buffer "xmf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
...
fn combo_marker_overlay_textprop_buflocal_narrow_clone_textprop_replace_undo_multi() {
  ...
  (let ((b1 (generate-new-buffer "ytb"))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "ytc")))
           (c2 (with-current-buffer b2 (clone-buffer "ytf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
