...
fn divergence_catch_throw_with_nested_unwind() {
  ...
  (catch 'done
    (unwind-protect
      ...
      (push 'cleanup result)))
  result)"#,
        expect_test::expect![[r#""OK (cleanup)""#]],
    );
}
...
fn divergence_save_excursion_buffer() {
  ...
  (list (eq (current-buffer) buf1)
        (with-current-buffer " *se-test*"
          (buffer-string))))"#,
        expect_test::expect![[r#""OK (t \"in other buffer\")""#]],
    );
}
...
fn divergence_save_restriction_nested() {
  ...
  (list (point-min) (point-max)))"#,
        expect_test::expect![[r#""CDEFOK (3 7)""#]],
    );
}
  ...
  (let ((m (match-data)))
    (save-match-data
      ...
      (list (match-string 0 "version 28.2")
            m
            (eq m (match-data)))))"#,
        expect_test::expect![[r#""OK (\"28.2\" (8 12 8 10) nil)""#]],
    );
}
...
fn divergence_match_data_across_replace() {
  ...
  (list (match-string 0)
        ...
        (replace-match "NUM")
        (buffer-string)))"#,
        expect_test::expect![[r#""abcNUMdefOK (\"123\" 4 7 nil \"abcNUMdef\")""#]],
    );
}
...
fn divergence_deep_recursion() {
  ...
  (list (my-recurse 100)
        (my-recurse 500)))"#,
        expect_test::expect![[r#""OK (done done)""#]],
    );
}
...
fn divergence_defmacro_expansion() {
  ...
  (macroexpand '(my-when t (list 1 2 3))))"#,
        expect_test::expect![[r#""OK (if t (progn (list 1 2 3)))""#]],
    );
}
...
fn divergence_apply_partial() {
  ...
  (list (funcall add3 4)
        (funcall add3 10)
        (funcall add3 0)))"#,
        expect_test::expect![[r#""OK (7 13 3)""#]],
    );
}
