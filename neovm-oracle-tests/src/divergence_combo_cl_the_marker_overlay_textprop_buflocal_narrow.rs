//! Combo: cl-the + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests type assertions with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_the_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "thb")))
...
#[test]
fn combo_cl_the_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "thc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_the_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "th1"))
        (b2 (generate-new-buffer "th2")))
...
#[test]
fn combo_cl_the_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let ((val (cl-the string "XX")))
          ...
          (replace-match (format "%s-%s" val my-var)))
        (undo-boundary)
        (let ((v my-var)
              (mp (marker-position m))
              (os (overlay-start ov))
...
#[test]
fn combo_cl_the_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
