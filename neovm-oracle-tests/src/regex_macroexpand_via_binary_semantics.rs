...
fn oracle_string_match_with_groups_via_binary() {
  ...
  (list (match-string 0 "hello 42")
        ...
        expect_test::expect![[r#""OK (\"hello 42\" \"hello\" \"42\")""#]],
    );
...
fn oracle_macroexpand_when_via_binary() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (o, n) = crate::common::eval_oracle_and_neovm_expect(
        r#"(macroexpand '(when t 42))"#,
        expect_test::expect![[r#""OK (if t (progn 42))""#]],
    );
...
fn oracle_macroexpand_unless_via_binary() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (o, n) = crate::common::eval_oracle_and_neovm_expect(
        r#"(macroexpand '(unless nil 42))"#,
        expect_test::expect![[r#""OK (if nil nil 42)""#]],
    );
