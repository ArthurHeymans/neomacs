//! Combo: cl-union/intersection/set-difference + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests set operations with buffer state.
...
#[test]
fn combo_cl_intersection_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "inc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
  ...
  (let ((b1 (generate-new-buffer "sd1"))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        ...
        (let* ((s1 (list 'a 'b 'c))
               (s2 (list 'b 'c 'd))
               (result (cl-set-difference s1 s2)))
          (goto-char 6)
          (insert (format "%s-%s-" result my-var)))
        (undo-boundary))
      (with-current-buffer b2
