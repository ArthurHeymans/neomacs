...
fn div_cx528_window_state_roundtrip() {
  ...
  (let ((w (selected-window))
    ...
    (let ((state (window-state-get w t)))
      ...
      (window-buffer w))))
"##,
        expect_test::expect![[r#""OK #<buffer *scratch*>""#]],
    );
}
...
"##,
        expect_test::expect![[r#""ERR (void-function window-state-usable-state)""#]],
    );
}
...
"##,
        expect_test::expect![[r#""ERR (void-function window-state-ignored-parameters)""#]],
    );
}
