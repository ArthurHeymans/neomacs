//! Oracle parity for number-sequence + concat integer edges.
//! GNU src/fns.c.
...
fn oracle_number_sequence_ascending() {
    ...
    let (o, n) = eval_oracle_and_neovm(r#"(number-sequence 1 5)"#);
...
fn oracle_number_sequence_with_step() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (o, n) = eval_oracle_and_neovm(r#"(number-sequence 0 10 2)"#);
...
fn oracle_number_sequence_descending() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (o, n) = eval_oracle_and_neovm(r#"(number-sequence 5 1 -1)"#);
...
fn oracle_number_sequence_single() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (o, n) = eval_oracle_and_neovm(r#"(number-sequence 3 3)"#);
    assert_ok_eq("(3)", &o, &n);
}

#[test]
fn oracle_concat_integers_is_string() {
    return_if_neovm_enable_oracle_proptest_not_set!();
