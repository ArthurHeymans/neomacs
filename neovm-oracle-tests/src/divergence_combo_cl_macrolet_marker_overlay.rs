//! Combo: cl-macrolet + markers + overlays + textprop + undo.
//! Tests local macro bindings with buffer state.
...
fn combo_cl_macrolet_marker_overlay_undo() {
    ...
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "cml")))
...
#[test]
fn combo_cl_macrolet_narrow_marker() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "cmn")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
...
#[test]
fn combo_cl_macrolet_clone_overlay() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_macrolet_multi_macro() {
    return_if_neovm_enable_oracle_proptest_not_set!();
  ...
  (let ((buf (generate-new-buffer "cmm")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 10))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (cl-macrolet ((ins-at (p s)
                       `(progn (goto-char ,p) (insert ,s))))
          (ins-at 6 "XX")
          (ins-at 12 "YY"))
        (undo-boundary)
        (let ((mp (marker-position m))
...
#[test]
fn combo_cl_macrolet_overlay_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
