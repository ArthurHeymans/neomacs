...
fn deficiency_let_star_sequential_binding() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (y (+ x 10))\n\
         (z (* y x)))\n\
         (list x y z)))", expect_test::expect![[r#""OK (1 11 11)""#]]);
}
...
fn deficiency_let_parallel_binding_old_values() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         (let ((x 1) (y 2))\n\
         (let ((x y) (y x))\n\
         (list x y))))", expect_test::expect![[r#""OK (2 1)""#]]);
}
...
fn deficiency_closure_captures_at_creation_time() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (mapcar (lambda (f) (funcall f)) (nreverse closures))))", expect_test::expect![[r#""OK (0 1 2 3 4)""#]]);
}
...
fn deficiency_dynbind_with_symbol_value() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (let ((dyn-test 'local))\n\
         (list dyn-test (symbol-value 'dyn-test)))))", expect_test::expect![[r#""OK (global global (local local))""#]]);
}
...
fn deficiency_setq_in_let_body_visible_to_closures() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (let ((f (lambda () x)))\n\
         (setq x 30)\n\
         (list (funcall f) x))))", expect_test::expect![[r#""OK (30 30)""#]]);
}
...
fn deficiency_nested_let_shadows_correctly() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (let ((x 'mid))\n\
         (let ((x 'inner))\n\
         (list x)))))", expect_test::expect![[r#""OK (inner)""#]]);
}
...
fn deficiency_function_cell_vs_value_cell() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (fboundp 'my-fn)))))", expect_test::expect![[r#""OK (original redefined nil)""#]]);
}
...
fn deficiency_buffer_local_let_shadow_chain() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (list blsc-var\n\
         (symbol-value 'blsc-var))))", expect_test::expect![[r#""OK (let-bound let-bound)""#]]);
}
...
fn deficiency_default_value_with_buffer_local() {
    ...
    crate::common::assert_oracle_parity_expect(
        "(progn\n\
         ...
         (kill-buffer buf)))", expect_test::expect![[r#""OK t""#]]);
}
