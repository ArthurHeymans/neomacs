...
fn divergence_regex_backreference() {
  ...
  (match-string 0 "abab"))"#,
        expect_test::expect![[r#""OK (0 nil \"abab\")""#]],
    );
}
...
fn divergence_regex_word_boundary() {
  ...
  (string-match "\\<hello\\>" "sayhello world"))"#,
        expect_test::expect![[r#""OK (4 nil nil)""#]],
    );
}
...
fn divergence_regex_non_greedy() {
  ...
  (progn
    (string-match "<.*>" "<a><b><c>")
    (match-string 0 "<a><b><c>")))"#,
        expect_test::expect![[r#""OK (0 \"<a>\" \"<a><b><c>\")""#]],
    );
}
...
fn divergence_regex_char_class_alternation() {
  ...
  (string-match "[a-z&&[^aeiou]]" "b"))"#,
        expect_test::expect![[r#""OK (nil 0 nil nil)""#]],
    );
}
...
fn divergence_regex_multiline() {
  ...
  (list
    ...
    (string-match "line2$" s)))"#,
        expect_test::expect![[r#""ERR (wrong-type-argument fixnump t)""#]],
    );
}
...
fn divergence_regex_shy_group() {
  ...
  (list (match-string 0 "abcd")
        (match-string 1 "abcd")
        (match-string 2 "abcd")))"#,
        expect_test::expect![[r#""OK (\"abcd\" \"cd\" nil)""#]],
    );
}
...
fn divergence_regex_named_group() {
  ...
  (list (match-beginning 0)
        ...
        (match-beginning 1)
        (match-end 1)))"#,
        expect_test::expect![[r#""OK (0 4 2 4)""#]],
    );
}
...
fn divergence_regex_replace_count() {
  ...
  (let ((count 0))
    ...
    (list count (buffer-string))))"#,
        expect_test::expect![[r#""XXX bbb XXX bbb XXXOK (3 \"XXX bbb XXX bbb XXX\")""#]],
    );
}
...
fn divergence_regex_unicode_classes() {
  ...
  (string-match "[[:space:]]" "hello world"))"#,
        expect_test::expect![[r#""OK (0 \"hello\" 5 \"123\" 5)""#]],
    );
}
...
fn divergence_regex_syntax_classes() {
  ...
  (string-match "\\s(" "(foo)")
  (string-match "\\s)" "(foo)"))"#,
        expect_test::expect![[r#""OK (0 \"hello\" 0 4)""#]],
    );
}
