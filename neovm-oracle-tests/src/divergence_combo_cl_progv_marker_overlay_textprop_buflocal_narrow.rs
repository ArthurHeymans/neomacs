//! Combo: cl-progv + marker + overlay + textprop + buflocal + narrow + undo.
...
#[test]
fn combo_cl_progv_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (defvar progv-test-var 'global-default)
  (let ((buf (generate-new-buffer "pgb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (cl-progv '(progv-test-var) '(bound-val)
          ...
          (goto-char (point-min))
          (insert (format "%s-%s-%s-" progv-test-var my-var (default-value 'progv-test-var))))
        (undo-boundary)
        (let ((v my-var)
              (dv (default-value 'progv-test-var))
...
#[test]
fn combo_cl_progv_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK nil""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (defvar progv-clone-var 'global)
  (let ((buf (generate-new-buffer "pgc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "pgc-clone")))
        (with-current-buffer clone
          (setq-local my-var 'cloned)
          (narrow-to-region 6 15)
          (undo-boundary)
          (cl-progv '(progv-clone-var) '(bound-val)
            (put-text-property (point-min) (point-max) 'z 'changed)
            (setf (char-after (point-min)) ?Z)
            (setf (marker-position m) 11)
            (goto-char (point-min))
            (insert (format "%s-%s-%s-" progv-clone-var my-var (default-value 'progv-clone-var))))
          (undo-boundary)
          (let ((v my-var)
                (dv (default-value 'progv-clone-var))
                (mp (marker-position m))
                (os (overlay-start ov))
                (oe (overlay-end ov))
                (k (get-text-property (point-min) 'z))
...
#[test]
fn combo_cl_progv_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""OK t""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (defvar progv-multi-var 'global)
  (let ((b1 (generate-new-buffer "pg1"))
        (b2 (generate-new-buffer "pg2")))
    ...
    (let* ((ov1 (with-current-buffer b1
      ...
      (with-current-buffer b1
        (undo-boundary)
        (cl-progv '(progv-multi-var) '(b1-bound)
          (goto-char 6)
          (insert (format "%s-%s-" progv-multi-var my-var)))
        (undo-boundary))
      (with-current-buffer b2
        (undo-boundary)
        (cl-progv '(progv-multi-var) '(b2-bound)
          (goto-char 6)
          (insert (format "%s-%s-" progv-multi-var my-var)))
        (undo-boundary))
...
#[test]
fn combo_cl_progv_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (defvar progv-setf-var 'global)
  (let ((buf (generate-new-buffer "psr")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (cl-progv '(progv-setf-var) '(bound-val)
          (setf (char-after 6) ?Z)
          (setf (marker-position m) 11)
          (goto-char 6)
          (re-search-forward "BBBB")
          (replace-match (format "%s-%s-%s" progv-setf-var my-var (default-value 'progv-setf-var))))
        (undo-boundary)
        (let ((v my-var)
              (dv (default-value 'progv-setf-var))
              (mp (marker-position m))
              (os (overlay-start ov))
              (oe (overlay-end ov))
              (s (buffer-string)))
...
#[test]
fn combo_cl_progv_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (defvar progv-ov-var 'global)
  (let ((buf (generate-new-buffer "pov")))
    (with-current-buffer buf
      ...
      (let* ((ov1 (make-overlay 1 10))
             ...
             (_ (set-marker m 8)))
        (undo-boundary)
        (cl-progv '(progv-ov-var) '(bound-val)
          (setf (char-after 6) ?Z)
          (setf (marker-position m) 11)
          (goto-char 6)
          (insert (format "%s-%s-%s-" progv-ov-var my-var (default-value 'progv-ov-var))))
        (undo-boundary)
        (let ((v my-var)
              (dv (default-value 'progv-ov-var))
              (mp (marker-position m))
              (os1 (overlay-start ov1))
              (oe1 (overlay-end ov1))
              (os2 (overlay-start ov2))
