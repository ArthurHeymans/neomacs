//! Combo: overlay + marker + textprop + buflocal + clone + narrow + setf + replace + undo + multi-buffer.
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_setf_replace_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "omb"))
        (b2 (generate-new-buffer "omd")))
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "omc")))
           (c2 (with-current-buffer b2 (clone-buffer "omf"))))
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_replace_setf_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_multi_overlay_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "pmb"))
        (b2 (generate-new-buffer "pmd")))
    (with-current-buffer b1
    ...
    (let* ((ov1a (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "pmc")))
           (c2 (with-current-buffer b2 (clone-buffer "pmf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_textprop_replace_undo_multi() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "rtb"))
        (b2 (generate-new-buffer "rtd")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'kind 'a)
      (put-text-property 6 10 'kind 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "rtc")))
           (c2 (with-current-buffer b2 (clone-buffer "rtf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
...
#[test]
fn combo_overlay_marker_textprop_buflocal_clone_narrow_setf_replace_undo_multi_2() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    let expect = expect_test::expect![[r#""ERR (void-function \\(setf\\ char-after\\))""#]];
    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((b1 (generate-new-buffer "zmb"))
        (b2 (generate-new-buffer "zmd")))
    (with-current-buffer b1
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
    ...
    (let* ((ov1 (with-current-buffer b1
           ...
           (m2 (with-current-buffer b2
                 (let ((m (make-marker))) (set-marker m 8) m)))
           (c1 (with-current-buffer b1 (clone-buffer "zmc")))
           (c2 (with-current-buffer b2 (clone-buffer "zmf"))))
      (with-current-buffer c1
        (setq-local my-var 'c1-cloned)
        (narrow-to-region 6 15)
        (undo-boundary)
