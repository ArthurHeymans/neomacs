...
fn divergence_closure_captures_outer_lexical() {
  ...
  (funcall
   (let ((x 20))
     (lambda () x))))"#, expect_test::expect![[r#""OK 20""#]]);
}
...
fn divergence_closure_mutates_captured_var() {
  ...
  (let ((inc (lambda () (setq counter (1+ counter)))))
    ...
    (funcall inc)
    (funcall inc)
    counter))"#, expect_test::expect![[r#""OK 3""#]]);
}
...
fn divergence_let_parallel_vs_let_star() {
  ...
  (let ((a 10)) (let* ((a 1) (a (1+ a))) a)))"#, expect_test::expect![[r#""OK (11 2)""#]]);
}
...
fn divergence_lexical_shadow_dynamic() {
  ...
  (let ((dyn-var 200))
    (list dyn-var
          ...
          dyn-var
          (eval 'dyn-var))))"#, expect_test::expect![[r#""OK (200 300 200 200)""#]]);
}
...
fn divergence_closure_over_loop_variable() {
  ...
  (dotimes (i 5)
    (push (lambda () i) fns))
  (mapcar #'funcall (nreverse fns)))"#, expect_test::expect![[r#""OK (0 1 2 3 4)""#]]);
}
...
fn divergence_funcall_with_and_rest() {
    ...
    crate::common::assert_oracle_parity_expect(r#"(funcall (lambda (a &rest b) (list a b)) 1 2 3 4)"#, expect_test::expect![[r#""OK (1 (2 3 4))""#]]);
}
...
fn divergence_funcall_with_and_optional() {
  ...
  (funcall (lambda (a &optional b c) (list a b c)) 1 2 3))"#, expect_test::expect![[r#""OK ((1 nil nil) (1 2 nil) (1 2 3))""#]]);
}
...
fn divergence_function_type_introspection() {
  ...
  (commandp 'car))"#, expect_test::expect![[r#""OK (t nil t t nil)""#]]);
}
...
fn divergence_apply_spreads_last_arg() {
  ...
  (apply #'+ 1 2 '(3 4 5))
  (apply #'+ nil)
  (apply #'list 1 2 '(3 4)))"#, expect_test::expect![[r#""OK (15 0 (1 2 3 4))""#]]);
}
...
fn divergence_mapcar_mapc() {
  ...
  (let ((acc nil))
    ...
    acc))"#, expect_test::expect![[r#""OK ((2 3 4 5 6) (c b a))""#]]);
}
...
fn divergence_recursive_lambda() {
  ...
  (funcall fact 10))"#, expect_test::expect![[r#""OK 3628800""#]]);
}
...
fn divergence_dyn_wind_nested() {
  ...
  (condition-case nil
    ...
    (error nil))
  log)"#, expect_test::expect![[r#""OK (outer-cleanup inner-cleanup body)""#]]);
}
...
fn divergence_catch_throw_nested() {
  ...
  (catch 'outer
    ...
    'not-reached)
  (catch 'tag
    (throw 'tag 42)))"#, expect_test::expect![[r#""OK (from-inner 42)""#]]);
}
...
fn divergence_eval_with_lexical_flag() {
  ...
  (eval '(let ((x 5)) (funcall (lambda () x))) nil))"#, expect_test::expect![[r#""OK (5 5)""#]]);
}
...
fn divergence_closure_with_docstring() {
  ...
  (list (funcall fn 41) (documentation fn)))"#, expect_test::expect![[r#""OK (42 \"docstring\")""#]]);
}
