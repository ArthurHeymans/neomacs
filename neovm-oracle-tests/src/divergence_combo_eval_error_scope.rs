...
fn divergence_nested_condition_unwind_catch_throw() {
  ...
  (catch 'done
    (unwind-protect
      ...
      (setq x (+ x 100))))
  (list (nreverse log) x)) ",
        expect_test::expect![[r#""OK ((body cleanup) 101)""#]],
    );
}
...
fn divergence_dynamic_rebinding_through_funcall_chain() {
  ...
  (list (test-chain-a-xxx)
        ...
        (test-chain-c-xxx)
        test-chain-var-xxx)) ",
        expect_test::expect![[r#""OK ((a 0) (b 20 (a 20)) (b 20 (a 20)) 0)""#]],
    );
}
  ...
  (let ((fns nil))
    ...
    (let ((results (mapcar #'funcall fns)))
      (list results
            ...
            (funcall (nth 0 fns))
            test-clo-var-xxx)))) ",
        expect_test::expect![[r#""OK ((a b c) c a a)""#]],
    );
}
...
fn divergence_catch_from_mapcar_lambda() {
  ...
  (mapcar (lambda (x)
          ...
          '(1 3 7 2 9))) ",
        expect_test::expect![[r#""OK (found 7)""#]],
    );
}
...
fn divergence_advised_function_in_condition_case() {
  ...
  (let ((r1 (condition-case err
    ...
    (list r1 r2 (test-adv-fn-xxx 7)))) ",
        expect_test::expect![[r#""OK (110 (caught (error \"too big: 15\")) 14)""#]],
    );
}
...
fn divergence_cl_block_return_from_nested_flet() {
  ...
  (cl-block outer
    (list (check 3)
          (check 7)
          'not-reached))) ",
        expect_test::expect![[r#""ERR (void-variable --cl-block-outer--)""#]],
    );
}
