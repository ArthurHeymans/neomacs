//! Combo: cl-dotimes + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests dotimes iteration with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_dotimes_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "dtb")))
...
#[test]
fn combo_cl_dotimes_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "dtc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_dotimes_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((b1 (generate-new-buffer "dt1"))
        (b2 (generate-new-buffer "dt2")))
...
#[test]
fn combo_cl_dotimes_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_dotimes_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
