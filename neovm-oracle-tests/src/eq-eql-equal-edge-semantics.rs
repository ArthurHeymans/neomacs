...
fn oracle_eq_identical_symbols() {
    ...
    let (oracle, neovm) = eval_oracle_and_neovm("(eq 'foo 'foo)");
...
fn oracle_eq_distinct_floats_are_not_eq() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm("(eq 1.0 1.0)");
...
fn oracle_eql_same_integers() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm("(eql 42 42)");
    assert_ok_eq("t", &oracle, &neovm);
}
...
fn oracle_eql_float_and_integer_are_not_eql() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm("(eql 1 1.0)");
    assert_ok_eq("nil", &oracle, &neovm);
}
...
fn oracle_equal_same_strings() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm(r#"(equal "hello" "hello")"#);
    assert_ok_eq("t", &oracle, &neovm);
}
...
fn oracle_equal_same_vectors() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm(r#"(equal [1 2 3] [1 2 3])"#);
    assert_ok_eq("t", &oracle, &neovm);
}
...
fn oracle_equal_nested_conses() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm(r#"(equal '(a (b c)) '(a (b c)))"#);
    assert_ok_eq("t", &oracle, &neovm);
}
...
fn oracle_eql_distinct_cons_not_eql() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm(r#"(eql '(a) '(a))"#);
    assert_ok_eq("nil", &oracle, &neovm);
}
...
fn oracle_eq_nil_and_false_list() {
    return_if_neovm_enable_oracle_proptest_not_set!();
    let (oracle, neovm) = eval_oracle_and_neovm("(eq nil '())");
    assert_ok_eq("t", &oracle, &neovm);
}
