//! Combo: cl-destructuring-bind + markers + overlays + textprop + undo.
//! Tests destructuring bindings with buffer state.
  ...
  (let ((buf (generate-new-buffer "cdn")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 1 15))
        ...
        (undo-boundary)
        (cl-destructuring-bind (x y) (list "XX" "YY")
          (goto-char (point-min))
          (insert x))
        (undo-boundary)
        (let ((mp (marker-position m))
...
#[test]
fn combo_cl_destructuring_clone_overlay() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    crate::common::assert_oracle_parity_expect(
        r#"(progn
  (let ((buf (generate-new-buffer "cdc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC")
      (put-text-property 1 5 'seg 'a)
      (put-text-property 6 10 'seg 'b)
      ...
      (let* ((ov (make-overlay 6 10))
             ...
             (m (make-marker))
             (_ (set-marker m 8))
             (clone (clone-buffer "cdc-clone")))
        (with-current-buffer clone
          (undo-boundary)
          (cl-destructuring-bind (a b) (list "XX" "-")
            (goto-char 6)
            (insert (concat a b)))
          (undo-boundary)
          (let ((mp (marker-position m))
      ...
      (let* ((ov1 (make-overlay 1 10))
        ...
        (let ((mp (marker-position m))
          ...
          (list mp os1 oe1 os2 oe2 s
                ...
                (buffer-string)))))
    (kill-buffer buf)))"#,
        expect_test::expect![[r#""ERR (wrong-type-argument listp \"WW\")""#]],
    );
}
  ...
  (let ((buf (generate-new-buffer "cdo")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 10))
        ...
        (narrow-to-region 6 10)
        (undo-boundary)
        (cl-destructuring-bind (x y z) (list "X" "Y" "Z")
          (goto-char (point-min))
          (insert (concat x y z)))
        (undo-boundary)
        (let ((mp (marker-position m))
