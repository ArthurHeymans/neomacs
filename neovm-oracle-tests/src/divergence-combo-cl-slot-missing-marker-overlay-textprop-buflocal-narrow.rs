//! Combo: cl-slot-missing + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests slot-missing with buffer state.

use super::common::assert_oracle_parity;
use super::common::return_if_neovm_enable_oracle_proptest_not_set;

#[test]
fn combo_cl_slot_missing_marker_overlay_textprop_buflocal_narrow_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "smb")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((pl (list 'a 1 'b 2 'c 3 'd 4 'e 5))
          ...
          (goto-char (point-min))
          (insert (format "%s-%s-%s-%s-" v1 v2 v3 my-var)))
        (undo-boundary)
        (let ((v my-var)
...
#[test]
fn combo_cl_slot_missing_clone_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();

    assert_oracle_parity(
        r#"(progn
  (let ((buf (generate-new-buffer "smc")))
    (with-current-buffer buf
      (insert "AAAA-BBBB-CCCC-DDDD")
      (put-text-property 1 5 'z 'a)
      (put-text-property 6 10 'z 'b)
...
#[test]
fn combo_cl_slot_missing_multi_buffer_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_slot_missing_setf_replace_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
...
#[test]
fn combo_cl_slot_missing_multi_overlay_undo() {
    return_if_neovm_enable_oracle_proptest_not_set!();
