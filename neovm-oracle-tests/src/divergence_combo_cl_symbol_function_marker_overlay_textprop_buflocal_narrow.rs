//! Combo: cl-symbol-function/cl-symbol-value/cl-boundp + marker + overlay + textprop + buflocal + narrow + undo.
//! Tests symbol function/value access with buffer state.
    ...
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((sym (make-symbol "test"))
          ...
          (goto-char (point-min))
          (insert (format "%s-%s-%s-" fn result my-var)))
        (undo-boundary)
        (let ((v my-var)
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (with-current-buffer clone
          ...
          (let* ((sym (make-symbol "test"))
            ...
            (setf (marker-position m) 11)
            (goto-char (point-min))
            (insert (format "%s-%s-%s-" v bp my-var)))
          (undo-boundary)
          (let ((v my-var)
                (mp (marker-position m))
                (os (overlay-start ov))
  ...
  (let ((buf (generate-new-buffer "sfr")))
    (with-current-buffer buf
      ...
      (let* ((ov (make-overlay 6 15))
        ...
        (let* ((sym (make-symbol "test"))
               (_ (fset sym (lambda (x) (list x))))
               (result (funcall (symbol-function sym) 42)))
          (setf (char-after 6) ?Z)
          (setf (marker-position m) 11)
          (goto-char 6)
