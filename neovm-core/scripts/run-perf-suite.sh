...
#
# Usage:
#   neovm-core/scripts/run-perf-suite.sh [aot_startup_samples]   # default 30
#   SKIP_AOT=1 neovm-core/scripts/run-perf-suite.sh              # skip heavy AOT stage
#
...
run_section "1. JIT micro-benches (scripts/run-jit-bench.sh)" \
    bash "$ROOT/neovm-core/scripts/run-jit-bench.sh"
...
if [[ "${SKIP_AOT:-0}" == "1" ]]; then
    ...
    run_section "5. AOT verdict bench (scripts/run-aot-bench.sh, N=$N)" \
        bash "$ROOT/neovm-core/scripts/run-aot-bench.sh" "$N"
else
    {
        echo
