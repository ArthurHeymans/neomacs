...
//! `neovm-core/unicode-data/`.  Whichever ran last decided the file.
//!
...
/// copy under `neovm-core/unicode-data/` was deleted with the reimplementation
...
fn every_recipe_reads_gnus_own_script_and_data_and_nothing_else() {
    ...
    assert!(
        !root.join("neovm-core").join("unicode-data").exists(),
        "neovm-core/unicode-data is a second copy of admin/unidata's inputs; it \
