...
fn prepare_statement(
    ...
    let ret = unsafe { ffi::sqlite3_prepare_v2(db, sql.as_ptr(), -1, &mut stmt, ptr::null_mut()) };
...
fn bind_values(
    ...
    for (index, value) in values.iter().enumerate() {
        ...
        let ret = match value {
            ...
            BindValue::Integer(value) => unsafe { ffi::sqlite3_bind_int64(stmt, index, *value) },
...
pub(crate) fn open(eval: &mut crate::emacs_core::eval::Context, args: Vec<Value>) -> EvalResult {
    ...
    let connection = match file {
        ...
        Some(file) => {
            let expanded =
                crate::emacs_core::fileio::builtin_expand_file_name(eval, vec![file, Value::NIL])?;
...
pub(crate) fn execute(eval: &mut crate::emacs_core::eval::Context, args: Vec<Value>) -> EvalResult {
...
pub(crate) fn select(eval: &mut crate::emacs_core::eval::Context, args: Vec<Value>) -> EvalResult {
