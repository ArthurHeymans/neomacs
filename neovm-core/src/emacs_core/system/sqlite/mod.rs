...
/// SQLite is part of every Neomacs build through rusqlite's bundled SQLite.
...
pub(crate) fn builtin_sqlite_open(eval: &mut super::eval::Context, args: Vec<Value>) -> EvalResult {
    ...
    let connection = match file {
        ...
        Some(file) => {
            let expanded = super::fileio::builtin_expand_file_name(eval, vec![file, Value::NIL])?;
            ...
            let path = super::fileio::lisp_file_name_to_path_buf(expanded_string);
...
pub(crate) fn builtin_sqlite_execute(
    eval: &mut super::eval::Context,
    args: Vec<Value>,
) -> EvalResult {
...
pub(crate) fn builtin_sqlite_select(
    eval: &mut super::eval::Context,
    args: Vec<Value>,
) -> EvalResult {
