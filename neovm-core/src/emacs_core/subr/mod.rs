...
///
/// Their implementation is the evaluator's special-form dispatch rather than
/// a Rust function pointer, but their Lisp-visible identity and arity belong in
/// the same declaration layer as ordinary native subrs.
const EVALUATOR_SUBRS: &[SubrSpec] = &[
    SubrSpec::evaluator(
        "quote",
    ...
    SubrSpec::evaluator("if", SubrArity::new(2, None), SubrDispatchKind::SpecialForm),
    SubrSpec::evaluator(
        "and",
        SubrArity::new(0, None),
        SubrDispatchKind::SpecialForm,
    ),
    SubrSpec::evaluator("or", SubrArity::new(0, None), SubrDispatchKind::SpecialForm),
    SubrSpec::evaluator(
        "cond",
        SubrArity::new(0, None),
        SubrDispatchKind::SpecialForm,
...
pub(crate) fn evaluator_dispatch_kind(name: &str) -> Option<SubrDispatchKind> {
    EVALUATOR_SUBRS
        ...
        .map(|spec| spec.dispatch_kind())
}

pub(crate) fn register_evaluator_subrs(ctx: &mut super::eval::Context) {
    ctx.register_subrs(EVALUATOR_SUBRS);
}
