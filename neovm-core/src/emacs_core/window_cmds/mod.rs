...
fn redisplay_window_outer(
    frames: &FrameManager,
    fid: FrameId,
    wid: WindowId,
) -> Result<Option<neomacs_display_protocol::types::Rect>, Flow> {
    let frame = frames
        .get(fid)
        .ok_or_else(|| signal("error", vec![Value::string("Frame not found")]))?;
    Ok(frame
        .redisplay_snapshot(wid)
        .map(|snapshot| snapshot.regions.outer))
}
...
pub(crate) fn builtin_window_pixel_edges(
    ...
    let (left, top, right, bottom) = match redisplay_window_outer(&eval.frames, fid, wid)? {
        Some(rect) => (
            rect.x as i64,
            rect.y as i64,
...
/// Register this module's subrs. GNU: `syms_of_window` in `src/window.c`.
/// Extracted verbatim from the former flat `builtins::init_builtins`.
pub(crate) fn syms_of_window(ctx: &mut crate::emacs_core::eval::Context) {
    ctx.defsubr(
        "run-window-configuration-change-hook",
        crate::emacs_core::builtins::hooks::builtin_run_window_configuration_change_hook,
        0,
        None,
    );
    ctx.defsubr(
    ...
    crate::emacs_core::builtins::register_builtin(
        ...
        crate::emacs_core::builtins::BuiltinRegistration::placeholder(
            ...
            0,
            Some(2),
            crate::emacs_core::builtins::BuiltinNoEvalPlaceholder::WindowLineHeight,
        ),
    );
    ctx.defsubr(
    ...
    crate::emacs_core::builtins::register_builtin(
        ...
        crate::emacs_core::builtins::BuiltinRegistration::placeholder(
            ...
            0,
            None,
            crate::emacs_core::builtins::BuiltinNoEvalPlaceholder::FixnumZero,
        ),
    );
    ctx.defsubr(
