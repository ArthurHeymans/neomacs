...
/// backed; what it does not ship is GTK's xwidget widget set, which is what
/// `xwidget-internal` advertises to `lisp/xwidget.el`.  `c_features.rs` records
/// that as `NotBuilt` (ledger 190/192) and these three rows follow it rather
/// than re-deciding it.
const XWIDGET_LAYER: &str = "ledger 190/192: c_features.rs answers NotBuilt for xwidget-internal while xwidget.rs \
     implements 23 of GNU's 36 xwidget subrs over WPE/WebKit";

/// `font-use-system-font` is `(car byte-boolean-vars)` in both engines, pinned
/// by `neovm-oracle-tests/src/defvar_bool_byte_boolean_vars.rs:42`
