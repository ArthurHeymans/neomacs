...
/// `backend/wkwebview` places a native `WKWebView` and the browse-url path is
/// complete -- so these three rows describe a build that has the feature, and
/// binding the variables matches GNU exactly rather than excusing a gap.  On
...
const XWIDGET_LAYER: &str = "ledger 190/192: xwidget.rs implements GNU's xwidget subrs over WPE/WebKit \
     (Linux, xwidget-internal NotBuilt) or a native WKWebView (macOS, provided)";
