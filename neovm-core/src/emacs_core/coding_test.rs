...
/// their TRAIL byte alike.  Ported by hand it was not free: entry 151 added the
...
///
/// Row one is Big5 (`caf <c3>` at the end: `<c3>` is a valid Big5 lead), row two
/// is Shift-JIS (`<82>` likewise), row three is Big5 again with nothing else in
/// reach.  Row two answering `(raw-text)` is what "every category rejected"
/// looks like: raw-text is never rejected, because the walk skips categories at
/// or past it (:8813).
///
/// Measured under GNU Emacs 31.0.90 (`tmp/pw156/pin.el`, `tmp/pw156/pin-gnu.txt`).
#[test]
fn a_lone_trailing_lead_byte_refutes_big5_and_sjis_in_a_last_block() {
    crate::test_utils::init_test_tracing();
    assert_eq!(
        detect_list(&[
            104, 101, 108, 108, 111, 32, 99, 97, 102, 195, 169, 32, 119, 111, 114, 108, 100, 32,
            99, 97, 102, 195
        ]),
        vec![
            "iso-latin-1",
    ...
    assert_eq!(detect_list(&[97, 130, 160, 130]), vec!["raw-text"]);
    assert_eq!(
        detect_list(&[97, 164, 164, 164]),
        vec![
            "iso-latin-1",
            "emacs-mule",
            "in-is13194-devanagari",
