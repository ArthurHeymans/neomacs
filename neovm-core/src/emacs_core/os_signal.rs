...
/// 180 §9.6 opened.
///
/// **SIGCHLD is deliberately NOT here, and ledger 200 is why.**  GNU installs
/// one (`catch_child_signal`, src/process.c:8650) because its `pselect` has no
/// per-child descriptor, so `child_signal_notify`'s byte on a self-pipe is the
/// only thing that can return the block -- GNU states exactly that at
/// :7539-7547 and adds that *"WINDOWSNT doesn't need this facility because its
/// 'pselect' emulation ... waits on a subprocess handle, which becomes
/// signaled when the process exits"*.  This port is the WINDOWSNT case on
/// every backend that has one: it registers a `pidfd` per child with the wait
...
/// disposition (SIGWINCH's `change_frame_size`) is a new variant plus one arm
/// in [`drain_pending_os_signals`], and still no new code in signal context.
///
/// **A new variant has to decide which of GNU's two safe points is its own**,
/// and they are different functions with different bodies -- the distinction
/// ledger 193 collapsed and ledger 198 named:
///
/// * `process_pending_signals` (src/keyboard.c:8367-8372) --
///   `pending_signals = false; handle_async_input (); do_pending_atimers ();`,
///   reached from `maybe_quit` (src/lisp.h:3896-3900 -> src/eval.c:1868-1876).
///   `grep -c status_notify` over it is **0**, so nothing it does can be
///   observed by a Lisp program.  Every disposition here today is of that
///   kind, which is why [`drain_pending_os_signals`] IS this drain.
/// * `wait_reading_process_output`'s `status_notify` calls (src/process.c:5554,
///   :5854), which run sentinels.  Nothing reaches that site through a signal
///   any more; the child-status walk is called there directly, and the site is
///   a capability only `wait.rs` can mint
...
impl AsyncSignalScope {
    ...
    /// (:8511-8512) while `handle_child_signal` does NEITHER.  Ledger 193 gave
    /// the two signals one wire and that wire carried its drain to the wrong
    /// safe point; ledger 198 split them, and ledger 200 measured that the
    /// child-status half was buying nothing and removed it.
    ...
    ///
    /// **It wakes nobody today, and that is measured rather than assumed**
    /// (ledger 200): `InstallReport::self_pipe_read_fd` is registered with no
...
extern "C" fn deliver_user_signal(sig: libc::c_int) {
    ...
    if let Some(signal) = HandledSignal::from_raw(sig) {
        ...
        match signal.disposition() {
            ...
            //
            // `record` is the deferral: everything GNU does BEFORE
            // `p->npending++; pending_signals = true;` (src/keyboard.c:
            // 8511-8512) needs Lisp state, so it happens at the safe point
            // instead.  The `set_pending_signals` beside it is GNU's own
            // second store, and it is what `maybe_quit` tests.
