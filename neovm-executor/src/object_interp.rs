...
mod tests {
    ...
    #[test]
    fn executes_cl_find_if_not_returns_nil_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-find-if-not #'evenp nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }
    ...
    #[test]
    fn executes_cl_position_if_not_returns_nil_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-position-if-not #'evenp nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }

    #[test]
    fn executes_cl_count_if_not_returns_zero_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (= (cl-count-if-not #'evenp nil) 0)",
        );
        assert_eq!(value, Some(LispValue::TRUE));
    }

    #[test]
    fn executes_cl_member_if_not_returns_nil_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-member-if-not #'evenp nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }

    #[test]
    fn executes_cl_rassoc_if_not_returns_nil_for_nil_alist() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-rassoc-if-not #'evenp nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }
    ...
    #[test]
    fn executes_cl_assoc_if_not_returns_nil_for_nil_alist() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-assoc-if-not #'evenp nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }

    #[test]
    fn executes_cl_subst_if_not_returns_nil_for_nil_tree() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-subst-if-not #'evenp 'new nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }
    ...
    #[test]
    fn executes_reverse_returns_nil_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (reverse nil)",
        );
    ...
    #[test]
    fn executes_cl_nreverse_returns_nil_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-nreverse nil)",
    ...
    #[test]
    fn executes_car_safe_returns_nil_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (car-safe nil)",
        );
    ...
    #[test]
    fn executes_cdr_safe_returns_nil_for_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (cdr-safe nil)",
        );
    ...
    #[test]
    fn executes_nth_returns_nil_for_nil_list() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (nth 0 nil)",
        );
    ...
    #[test]
    fn executes_last_returns_nil_for_nil_list() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (last nil)",
        );
    ...
    #[test]
    fn executes_cl_subst_if_returns_nil_for_nil_tree() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-subst-if #'evenp 'new nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }

    #[test]
    fn executes_cl_substitute_if_returns_nil_for_nil_seq() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-substitute-if #'evenp 'new nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }

    #[test]
    fn executes_cl_substitute_if_not_returns_nil_for_nil_seq() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-substitute-if-not #'evenp 'new nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }
    ...
    #[test]
    fn executes_memql_returns_nil_for_nil_list() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (memql 42 nil)",
        );
    ...
    #[test]
    fn executes_cl_nsubstitute_if_returns_nil_for_nil_tree() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-nsubstitute-if #'evenp 'new nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }

    #[test]
    fn executes_cl_nsubstitute_if_not_returns_nil_for_nil_tree() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (require 'cl-lib)\n\
             (cl-nsubstitute-if-not #'evenp 'new nil)",
        );
        assert_eq!(value, Some(LispValue::NIL));
    }
    ...
    #[test]
    fn executes_subrp_nil_returns_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
             (subrp nil)",
        );
    ...
    #[test]
    fn executes_compiled_function_p_nil_returns_nil() {
        let (value, _) = execute(
            ";;; -*- lexical-binding: t; -*-\n\
