...
fn dump_surface_form(form: &SurfaceForm, indent: usize, out: &mut String) {
    ...
    match &form.kind {
        ...
        SurfaceKind::Labeled(n, form) => {
            let _ = writeln!(out, "{pad}labeled #{n}=");
            dump_surface_form(form, indent + 1, out);
        }
        SurfaceKind::Ref(n) => {
            let _ = writeln!(out, "{pad}ref #{n}#");
        }
...
fn dump_ssa_inst(kind: &SsaInstKind, function: &SsaFunction, out: &mut String) {
    match kind {
        ...
        SsaInstKind::CatchEnd { body_result } => match body_result {
            Some(v) => {
                let _ = write!(out, "catch-end body={}", value_name(function, *v));
            }
            None => {
        ...
        SsaInstKind::ConditionCaseEnd { body_result } => match body_result {
            Some(v) => {
                let _ = write!(out, "condition-case-end body={}", value_name(function, *v));
            }
            None => {
