...
/// `M-x whitespace-mode' is the documented way to see the `whitespace-tab' and
/// `whitespace-line' faces abyss styles.  Turning it off again has to remove
/// the highlighting completely.
#[test]
fn a_whitespace_review_session_flags_tabs_and_long_lines_and_clears_them_again() {
    let elisp_form = r####"
(let* ((root
        (file-name-as-directory
         (expand-file-name
...
/// abyss ships explicit `compilation-*', `error', `warning' and `success'
/// faces "used by many packages (e.g. envrc, flycheck, lsp)".  A build log in
/// `compilation-mode' is where a user meets them.
#[test]
...
/// A user who already runs a light theme tries abyss for one session.  abyss
/// has to win for every face it defines, leave the faces it does not define to
/// the theme underneath, and give the exact previous appearance back on
/// `disable-theme'.
#[test]
...
/// The package's only autoloaded command is `M-x abyss-theme'.  It has to
/// enable the theme that the installed directory registered on
/// `custom-theme-load-path', and running it twice must not stack a second copy
/// of the theme or duplicate its settings.
#[test]
...
/// abyss styles faces belonging to packages that are not its dependencies, so
/// the ordinary startup order declares them after the theme is already
/// enabled.  Their `defface' must not win, and disabling abyss must hand each
/// package its own appearance back.
#[test]
