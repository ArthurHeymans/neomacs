...
use super::{assert_asilea_autoload_parity, assert_asilea_parity};

#[test]
fn asilea_exact_pin_descriptor_dependencies_origin_and_feature_contract_match() {
...
#[test]
fn asilea_installed_payload_inventory_sizes_and_content_digests_match() {
    let elisp_form = r##"(let* ((descriptor
                  (cadr
                   (assq 'asilea package-alist)))
                 (directory
...
#[test]
fn asilea_complete_callable_command_arglist_doc_and_source_surface_matches() {
...
#[test]
fn asilea_every_configuration_variable_default_scope_doc_and_source_match() {
    let elisp_form = r##"(mapcar
         (lambda (symbol)
           (list
            symbol
...
#[test]
fn asilea_load_has_no_global_keymap_hook_command_or_process_side_effects() {
    let elisp_form = r##"(list
         ...
         (seq-filter
          ...
          (process-list)))"##;
    let expect = expect!["OK (t nil nil nil nil nil nil)"];
...
fn asilea_generated_autoload_file_exposes_run_entrypoints_without_loading_feature() {
    ...
    assert_asilea_autoload_parity(elisp_form, expect);
}
