...
use super::{assert_airline_themes_autoload_parity, assert_airline_themes_parity};

#[test]
fn airline_themes_registers_the_complete_public_surface_and_custom_contract() {
...
#[test]
fn airline_themes_every_installed_theme_loads_and_registers_real_face_settings() {
    let elisp_form = r##"(let* ((directory
                 (file-name-directory
                  (getenv "NEOMACS_PACKAGE_SOURCE")))
                (files
...
fn airline_themes_runtime_has_no_hidden_asset_dependency_and_locates_every_entrypoint() {
    let elisp_form = r##"(let* ((library (locate-library "airline-themes"))
