...
#[test]
fn auto_read_only_descriptor_and_payload_pin_exact_melpa_archive() {
...
#[test]
fn auto_read_only_complete_function_and_variable_inventory_has_exact_origins() {
...
#[test]
fn auto_read_only_custom_group_options_and_lighter_have_exact_metadata() {
    let elisp_form = r##"(list
         auto-read-only-mode-lighter
...
#[test]
fn auto_read_only_source_load_history_records_exact_owned_surface() {
    ...
    let elisp_form = r##"(let ((source
         ...
         (prog1
             (list
              ...
              (featurep 'auto-read-only))
           (auto-read-only-mode -1)))"##;
    let expect = expect![[r#"OK (("/vendor/" "\\.lock\\'") t " Protected" t 1 t)"#]];
    assert_auto_read_only_parity(elisp_form, expect);
}
