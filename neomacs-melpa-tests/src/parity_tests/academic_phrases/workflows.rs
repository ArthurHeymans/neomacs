...
#[test]
fn academic_phrases_inserts_a_chosen_phrase_into_a_paper_draft_at_point() {
...
#[test]
fn academic_phrases_by_section_restricts_the_categories_to_that_paper_section() {
    let elisp_form = r##"(with-temp-buffer
  (academic-test-answer
   (list "Methods"
...
#[test]
fn academic_phrases_fills_every_placeholder_of_a_multiple_choice_template() {
    let elisp_form = r##"(with-temp-buffer
  (academic-test-answer
...
#[test]
fn academic_phrases_offers_the_documented_sections_and_their_own_category_lists() {
...
fn academic_phrases_renders_every_category_and_phrase_without_a_leftover_placeholder() {
    let elisp_form = r##"(let* ((pick-first (lambda (candidates) (car candidates)))
       (categories
  ...
  (list (length categories)
        ...
        (nreverse leftovers)))"##;
    let expect = expect!["OK (57 592 592 57 nil nil)"];

    assert_academic_phrases_parity(elisp_form, expect);
}

#[test]
fn academic_phrases_answers_its_prompts_from_the_real_minibuffer() {
    let elisp_form = r##"(let ((buffer (generate-new-buffer "*paper draft*")))
  (unwind-protect
      (progn
    ...
    (kill-buffer buffer)))"##;
    let expect = expect![[r#"OK ("Motivation: We thank Dr. Y for her help." 41)"#]];
