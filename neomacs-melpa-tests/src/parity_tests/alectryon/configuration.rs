...
#[test]
fn alectryon_guesses_markup_modes_from_real_project_filenames_and_buffer_names() {
...
#[test]
fn alectryon_selects_available_markup_modes_without_prompting_when_unambiguous() {
    let elisp_form = r##"(let ((original alectryon-text-modes)
      records)
  (unwind-protect
      (dolist (modes
               '(((missing-mode :tag "x"))
...
#[test]
fn alectryon_ensure_text_mode_prefers_explicit_then_filename_then_user_selection() {
 ...
 (with-temp-buffer
   ...
   (list (alectryon--text-mode-with-fallback)
         alectryon-text-mode)))"##;
    let expect = expect!["OK (markdown-mode typst-ts-mode rst-mode (markdown-mode nil))"];
    assert_alectryon_parity(elisp_form, expect);
}
