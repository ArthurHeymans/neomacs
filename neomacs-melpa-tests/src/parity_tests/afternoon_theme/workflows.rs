...
use super::assert_afternoon_theme_parity;
...
#[test]
fn loading_afternoon_repaints_a_real_elisp_editing_session() {
    let elisp_form = r####"
...
/// afternoon's distinguishing feature: `background', `current-line' and
/// `block-background' are chosen when the file loads, from whether
/// `display-color-cells' is exactly 256.  Loading under each answer has to
/// produce a different palette, and going back has to restore the first one
/// exactly -- including `fci-rule-color' and the background slot of
/// `ansi-color-names-vector', which are derived from the same values.
#[test]
fn the_palette_follows_the_terminals_colour_count_at_load_time() {
    let elisp_form = r####"
...
/// Unlike the other themes in this suite afternoon carries `theme-value'
/// settings as well as `theme-face' ones -- `vc-annotate-color-map' built from
/// its palette, the ansi colour vectors, and `fci-rule-color'.  Disabling has
/// to restore every one of them, and the faces, to the captured baseline.
#[test]
fn the_theme_sets_six_variables_and_disable_theme_gives_them_all_back() {
    let elisp_form = r####"
...
/// A user who runs a light theme tries afternoon for an evening.  Pins which
/// faces afternoon wins, which stay with the theme underneath, that a second
/// `load-theme' changes nothing at all, and that `disable-theme' returns the
/// captured baseline exactly.
#[test]
fn stacking_afternoon_over_a_light_user_theme_and_taking_it_off_again() {
    let elisp_form = r####"
