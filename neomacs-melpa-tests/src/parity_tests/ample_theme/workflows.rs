...
/// report goes.
///
/// The report covers the whole enable/disable pair, so the effect and its
/// reversal are both on the record: every attribute comes back when the theme
/// is disabled, `equal' to the report taken before it was enabled.  That is
/// what makes this a suppression by the theme rather than a one-way mutation
/// of the face.
///
