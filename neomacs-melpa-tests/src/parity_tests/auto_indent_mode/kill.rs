...
#[test]
fn auto_indent_mode_ports_upstream_blank_line_kill_ert_as_real_editing_case() {
    let elisp_form = r##"(with-temp-buffer
         ...
         (let ((auto-indent-force-interactive-advices nil))
           ...
           (list
            ...
            (current-kill 0 t))))"##;
    let expect = expect![[r#"OK ("HISTSIZE=1000\nHISTFILESIZE=2000" 15 t "\n")"#]];
    assert_auto_indent_mode_parity(elisp_form, expect);
}

#[test]
fn auto_indent_mode_kill_line_at_eol_nil_joins_and_normalizes_whitespace() {
    let elisp_form = r##"(with-temp-buffer
         (emacs-lisp-mode)
         (insert "(alpha)\n    (beta)")
...
#[test]
fn auto_indent_mode_kill_line_whole_line_and_blanks_have_distinct_extent() {
...
#[test]
fn auto_indent_mode_kill_line_active_region_delegates_to_region_kill() {
    let elisp_form = r##"(auto-indent-test-error
         (lambda ()
           (with-temp-buffer
...
#[test]
fn auto_indent_mode_kill_region_function_deindents_multiline_kill_ring_entry() {
    let elisp_form = r##"(auto-indent-test-error
         (lambda ()
           (with-temp-buffer
             (insert "  first\n    second\n  third\nkeep")
...
#[test]
fn auto_indent_mode_kill_ring_save_copies_without_deleting_and_normalizes_indent() {
    let elisp_form = r##"(auto-indent-test-error
         (lambda ()
           (with-temp-buffer
             (insert "  first\n    second\n  third")
             (let ((kill-ring nil)
               ...
               (list
                ...
                kill-ring
                (current-kill 0 t))))))"##;
    let expect = expect!["OK (:signal void-variable (yank-handler))"];
    assert_auto_indent_mode_parity(elisp_form, expect);
}

#[test]
fn auto_indent_mode_deindent_last_kill_updates_only_latest_entry() {
    let elisp_form = r##"(let ((kill-ring
                                '("  alpha\n    beta\n gamma"
                                  "older")))
...
#[test]
fn auto_indent_mode_command_classifiers_cover_symbols_and_live_bindings() {
    let elisp_form = r##"(mapcar
         (lambda (command)
...
#[test]
fn auto_indent_mode_remove_advice_policy_handles_modes_minibuffer_and_commands() {
    let elisp_form = r##"(mapcar
         (lambda (case)
           (with-temp-buffer
...
#[test]
fn auto_indent_mode_invalid_kill_line_style_signals_exact_configuration_error() {
    let elisp_form = r##"(with-temp-buffer
         (insert "alpha\nbeta")
         (goto-char (point-at-eol))
         (let ((auto-indent-mode t)
           ...
           (auto-indent-test-error
            (lambda ()
              (auto-indent-kill-line 1)))))"##;
    let expect = expect!["OK (:value nil)"];
    assert_auto_indent_mode_parity(elisp_form, expect);
}
