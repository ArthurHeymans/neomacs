...
pub(crate) fn assert_ac_slime_parity(elisp_form: &str, expected: Expect) {
    let name = current_test_name();
    let report = ac_slime_oracle()
...
pub(crate) fn assert_ac_slime_batch(cases: &[(&str, &str, bool, Expect)]) {
    let name = current_test_name();
    assert_oracle_batch(
        ac_slime_oracle(),
        &name,
