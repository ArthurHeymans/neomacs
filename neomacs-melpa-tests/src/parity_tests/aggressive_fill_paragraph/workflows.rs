...
fn typing_a_paragraph_key_by_key_wraps_it_while_the_user_types_and_undo_takes_it_back() {
    let elisp_form = r##"(let ((buffer (afp-test-open 'text-mode 38)))
  (unwind-protect
      (progn
...
fn inserting_words_inside_a_filled_paragraph_reflows_it_and_leaves_point_after_the_typed_space() {
    let elisp_form = r##"(let ((buffer (afp-test-open 'text-mode 40)))
  (unwind-protect
      (progn
        (aggressive-fill-paragraph-mode 1)
...
#[test]
fn set_fill_column_changes_the_width_the_next_automatic_refill_uses() {
    let elisp_form = r##"(let ((buffer (afp-test-open 'text-mode 70)))
  (unwind-protect
      (progn
        (aggressive-fill-paragraph-mode 1)
        (set-fill-column 72)
...
fn only_the_configured_fill_keys_refill_and_a_second_whitespace_character_suppresses_it() {
    let elisp_form = r##"(let ((sentence "The parser retries every failed request before it gives up"))
...
#[test]
fn adding_ruby_mode_to_the_comments_only_list_stops_the_refill_rewrapping_code() {
    let elisp_form = r##"(cl-flet ((session (comments-only)
...
#[test]
fn typing_into_a_visited_file_wraps_the_paragraph_and_saves_the_wrapped_bytes() {
    let elisp_form = r##"(let* ((notes (expand-file-name "release-notes.txt"
