...
fn assert_auto_complete_exuberant_ctags_source_parity(
    source_file: &str,
    elisp_form: &str,
    expected: Expect,
) {
    let name = current_test_name();
    let report = auto_complete_exuberant_ctags_oracle(source_file)
        .run_value(&name, elisp_form)
        .unwrap_or_else(|error| {
            panic!("auto-complete-exuberant-ctags parity case `{name}` failed:\n{error}")
        });
    expected.assert_eq(&report.gnu_emacs.to_string());
}

pub(crate) fn assert_auto_complete_exuberant_ctags_parity(elisp_form: &str, expected: Expect) {
    assert_auto_complete_exuberant_ctags_source_parity(
...
pub(crate) fn assert_auto_complete_exuberant_ctags_autoload_parity(
    elisp_form: &str,
    expected: Expect,
) {
    assert_auto_complete_exuberant_ctags_source_parity(
