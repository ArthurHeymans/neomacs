...
fn ac_alchemist_setup_starts_the_project_server_and_sends_the_first_completion_request() {
    let elisp_form = r##"(ac-alchemist-test-session
    '(("COMP" "String.to_"
       "String.to_\nto_atom/1\nto_charlist/1\nto_existing_atom/1\nto_float/1\nto_integer/1\nto_integer/2"))
...
/// Once the answer has arrived the user asks for completion again, this time
/// through `ac-complete-alchemist`, the command `ac-define-source` generates
/// for this source.  The server lines `to_integer/1` and `to_integer/2` must
/// become one menu entry named `to_integer` carrying its arity in the popup
/// `symbol` property, the echoed hint `String.to_` must be filtered out by the
/// `to_` prefix, and selecting the third entry and pressing `ac-complete` must
/// replace exactly the typed prefix in the file.
#[test]
fn ac_alchemist_completes_a_dotted_call_with_arity_annotated_candidates() {
    let elisp_form = r##"(ac-alchemist-test-session
    '(("COMP" "String.to_"
       "String.to_\nto_atom/1\nto_charlist/1\nto_existing_atom/1\nto_float/1\nto_integer/1\nto_integer/2"))
  (let ((buffer (ac-alchemist-test-visit
...
/// auto-complete asks a source for help through the candidate's `document`
/// property, which ac-alchemist answers by sending a `DOCL` request for the
/// selected candidate.  The request has to carry the dotted module, the arity
/// taken from the popup symbol and the module surrounding point in `imports:`,
/// and the reply — real `h/1` output, so ANSI attributes and Unicode included —
/// has to reach the user with the escape sequences filtered out.
#[test]
fn ac_alchemist_documents_the_selected_function_from_the_alchemist_server() {
    let elisp_form = r##"(ac-alchemist-test-session
    '(("COMP" "String.to_"
       "String.to_\nto_atom/1\nto_charlist/1\nto_existing_atom/1\nto_float/1\nto_integer/1\nto_integer/2")
...
/// Completing a bare module name goes down the other branch of the source: the
/// hint has no dot, so the server echoes the hint back as a candidate in its
/// own right, every candidate gets the blank arity placeholder, and the
/// documentation query is the plain candidate without any arity suffix.
#[test]
fn ac_alchemist_completes_a_bare_module_name_and_documents_the_module() {
    let elisp_form = r##"(ac-alchemist-test-session
    '(("COMP" "Str" "Str\nStream\nString\nStringIO")
      ("DOCL" "String"
...
/// The user misspells a module, so the alchemist server answers with nothing
/// but its `END-OF-COMP` marker.  ac-alchemist must turn that into an empty
/// candidate list rather than into the marker itself, no candidate may be
/// selected, and `ac-complete` must leave the file exactly as the user typed
/// it, unmodified and with point where it was.
#[test]
fn ac_alchemist_reports_no_candidates_for_an_unknown_module_and_leaves_the_buffer_alone() {
    let elisp_form = r##"(ac-alchemist-test-session nil
  (let ((buffer (ac-alchemist-test-visit
                 "blogpost/lib/blogpost/slug.ex"
                 "defmodule Blogpost.Slug do\n  def build(title) do\n    Strng.to_a\n  end\nend\n"
                 "Strng.to_a")))
...
/// Two modules of the same Mix project must share one alchemist server keyed on
/// the project root: the `elixir` stand-in may be launched exactly once, a
/// repeated `ac-alchemist-setup` may not push a second copy of the source into
/// `ac-sources`, and each buffer's documentation request must carry its own
/// enclosing module in `imports:`.
#[test]
