...
fn assert_async_job_queue_source_parity(source_file: &str, elisp_form: &str, expected: Expect) {
    let name = current_test_name();
    let report = async_job_queue_oracle(source_file)
        .run_value(&name, elisp_form)
        .unwrap_or_else(|error| panic!("async-job-queue case `{name}` failed:\n{error}"));
    expected.assert_eq(&report.gnu_emacs.to_string());
}

pub(crate) fn assert_async_job_queue_parity(elisp_form: &str, expected: Expect) {
    assert_async_job_queue_source_parity("async-job-queue.el", elisp_form, expected);
}

pub(crate) fn assert_async_job_queue_autoload_parity(elisp_form: &str, expected: Expect) {
...
pub(crate) fn assert_async_job_queue_autoload_batch(cases: &[(&str, &str, bool, Expect)]) {
    let name = current_test_name();
    assert_oracle_batch(
...
pub(crate) fn assert_async_job_queue_batch(cases: &[(&str, &str, bool, Expect)]) {
    let name = current_test_name();
    assert_oracle_batch(
