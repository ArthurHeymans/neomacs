...
/// the theme.
#[test]
fn each_variant_of_the_family_registers_its_own_palette() {
...
/// A user on a terminal sets the documented "All terminals" value of
/// `alect-display-class' and switches between variants.  Now the palette really
/// reaches the faces: light paints the editor one way, loading dark on top
/// takes precedence for every face, disabling dark hands the session back to
/// light unchanged, and disabling light restores the captured baseline exactly.
#[test]
fn switching_between_variants_repaints_the_session_and_disabling_restores_it() {
    let elisp_form = r##"(unwind-protect
    (progn
      (alect-test-reset)
...
/// The documented way to tweak the family: change a colour in `alect-colors'
/// with `alect-set-color' and load the theme again -- the generator runs at load
/// time, so every face built from that colour is repainted, and faces built
/// from other colours are untouched.  `alect-get-color' reports the change, and
/// naming a colour or a theme that does not exist is an error rather than a
/// silent no-op.
#[test]
fn changing_a_palette_colour_repaints_every_face_generated_from_it() {
    let elisp_form = r##"(unwind-protect
    (progn
      (alect-test-reset)
      (setq alect-display-class nil)
...
/// The two escape hatches, both read while the theme loads.
/// `alect-overriding-faces' replaces the generated spec for the faces it names
/// and lets the user write palette colour *names* instead of colour values, so
/// the same override follows whichever variant is loaded.
/// `alect-ignored-faces' leaves the named faces alone entirely -- no
/// registration at all -- while the rest of the theme is unaffected.
#[test]
fn overriding_and_ignoring_faces_change_what_the_generator_produces() {
    let elisp_form = r##"(unwind-protect
    (progn
      (alect-test-reset)
      (setq alect-display-class nil)
...
/// The `-alt' half of the family is the same palette with the colour extremes
/// swapped: a face built from `blue-1' in `alect-light' is built from `blue+1'
/// in `alect-light-alt'.  `alect-get-color' does the swapping, and only for
/// names matching `alect-inverted-color-regexp' -- `fg-1' and `gray' are not
/// inverted -- so a user who narrows that regexp gets a different family.
#[test]
fn the_alt_variants_invert_the_colour_extremes() {
    let elisp_form = r##"(unwind-protect
    (progn
      (alect-test-reset)
      (setq alect-display-class nil)
