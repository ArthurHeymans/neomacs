...
#[test]
fn ast_grep_candidate_display_text_normalizes_multiline_real_source_snippets() {
    let elisp_form = r##"(mapcar
          ...
          '(nil
            ...
            "\r\n  function call(\n  alpha,\r\n  beta)\n"
            "single β界 line"))"##;
    let expect = expect![[r#"OK ("" "" "" "function call(   alpha,   beta)" "single β界 line")"#]];
    assert_ast_grep_parity(elisp_form, expect);
}

#[test]
fn ast_grep_candidate_lookup_supports_plists_properties_registry_and_legacy_paths() {
    let elisp_form = r##"(let* ((match
...
#[test]
fn ast_grep_stream_parser_handles_multiple_files_unicode_and_malformed_records() {
...
fn ast_grep_completion_table_filters_real_candidates_and_exposes_affixation_metadata() {
    let elisp_form = r##"(let* ((ast-grep-use-nerd-icons nil)
               (candidates
                (mapcar
...
#[test]
fn ast_grep_goto_match_visits_real_file_and_lands_on_character_index() {
    ...
    let expect = expect![[r#"OK (t 2 3 "target")"#]];
    assert_ast_grep_parity(elisp_form, expect);
}

#[test]
fn ast_grep_reset_candidate_table_removes_only_registered_session_data() {
