...
#[test]
fn ast_grep_descriptor_and_installed_source_inventory_pin_the_exact_release() {
    let elisp_form = r##"(let* ((descriptor (cadr (assq 'ast-grep package-alist)))
...
#[test]
fn ast_grep_main_core_sync_and_outline_callable_surface_is_complete() {
...
#[test]
fn ast_grep_all_declared_variables_have_exact_defaults_and_custom_contracts() {
    let elisp_form = r##"(mapcar
          (lambda (symbol)
            (list
             symbol
...
#[test]
fn ast_grep_consult_callable_surface_loads_without_optional_consult_dependency() {
...
#[test]
fn ast_grep_helm_callable_and_variable_surface_loads_without_optional_dependency() {
    let elisp_form = r##"(list
