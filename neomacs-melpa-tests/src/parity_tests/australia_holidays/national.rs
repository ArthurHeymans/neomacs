...
#[test]
fn australia_holidays_national_calendar_for_2025_matches_exact_dates_and_labels() {
    let elisp_form = r##"(australia-holidays-test-year
                          australia-holidays
                          2025)"##;
    let expect = expect![[
...
fn australia_holidays_cross_year_range_orders_december_and_january_holidays_chronologically() {
    let elisp_form = r##"(australia-holidays-test-between
                          australia-holidays
                          '(12 20 2024)
                          '(2 2 2025))"##;
    let expect = expect![[
...
fn australia_holidays_january_label_customization_is_read_at_each_calendar_evaluation() {
    let elisp_form = r##"(let ((australia-holidays-include-january-26
                                t))
                               ...
                               (let ((second
                                 ...
                                 (let ((empty-label
                                   ...
                                   (list
                                    ...
                                    second
                                    empty-label))))))"##;
    let expect = expect![[r#"OK (("Invasion Day") ("Survival Day") (""))"#]];
...
#[test]
fn australia_holidays_calendar_check_distinguishes_holidays_and_ordinary_dates() {
    let elisp_form = r##"(mapcar
                          (lambda (date)
                            (list
                             date
...
#[test]
fn australia_holidays_fixed_rules_do_not_synthesize_weekday_substitute_holidays() {
    let elisp_form = r##"(mapcar
                          (lambda (date)
                            (list
                             date
...
fn australia_holidays_three_month_calendar_window_handles_year_boundary_visibility() {
    let elisp_form = r##"(let ((calendar-holidays
                                australia-holidays)
                               (displayed-month 12)
                               (displayed-year 2025)
