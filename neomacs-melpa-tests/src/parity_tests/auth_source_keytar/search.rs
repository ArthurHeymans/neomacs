...
#[test]
fn auth_source_keytar_search_service_and_account_delegate_to_exact_keytar_lookup() {
          ...
          (cl-letf
            ...
            (list
             (auth-source-keytar-search
              ...
              :account "deploy")
             (nreverse calls))))"##;
    let expect = expect![[r#"OK ("production-secret" (("api.example" "deploy")))"#]];
...
#[test]
fn auth_source_keytar_search_service_account_take_precedence_over_host_user() {
    let elisp_form = r##"(let (calls)
          (cl-letf
              (((symbol-function 'keytar-get-password)
                (lambda (service account)
            ...
            (list
             ...
             (nreverse calls))))"##;
    let expect = expect![[r#"OK (:selected (("selected-service" "selected-account")))"#]];

    assert_auth_source_keytar_parity(elisp_form, expect);
}

#[test]
fn auth_source_keytar_search_service_only_builds_multiple_secret_entries() {
    let elisp_form = r##"(let (calls)
          (cl-letf
              (((symbol-function
            ...
            (list
             ...
             (nreverse calls))))"##;
    let expect = expect![[r#"OK (((:secret "second") (:secret "first")) ("registry.example"))"#]];

    assert_auth_source_keytar_parity(elisp_form, expect);
}

#[test]
fn auth_source_keytar_search_host_only_uses_host_as_build_result_service() {
    let elisp_form = r##"(let (calls)
          (cl-letf
              (((symbol-function
                 'auth-source-keytar--build-result)
            ...
            (list
             ...
             (nreverse calls))))"##;
    let expect = expect![[r#"OK (((:secret "database.internal-secret")) ("database.internal"))"#]];

    assert_auth_source_keytar_parity(elisp_form, expect);
}

#[test]
fn auth_source_keytar_search_partial_query_matrix_exposes_exact_branch_selection() {
    let elisp_form = r##"(let (calls)
          (cl-letf
              (((symbol-function 'keytar-get-password)
                (lambda (service account)
          ...
          (cl-letf
            ...
            (list
             (auth-source-keytar-search
              ...
              :custom-key "ignored")
             (nreverse calls))))"##;
    let expect = expect![[r#"OK ("secret" (("service" "account")))"#]];
    ...
    let elisp_form = r##"(let (calls)
          (cl-letf
            ...
            (list
             ...
             (nreverse calls))))"##;
    let expect = expect![[r#"OK (:found (("first-service" "first-account")))"#]];
