...
pub(crate) fn assert_ac_html_angular_parity(form: &str, expected: Expect) {
    let name = current_test_name();
    let report = ac_html_angular_oracle()
        .run_value(&name, form)
        .unwrap_or_else(|error| panic!("ac-html-angular parity case `{name}` failed:\n{error}"));
