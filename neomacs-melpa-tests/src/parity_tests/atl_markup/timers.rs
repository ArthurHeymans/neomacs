...
#[test]
fn atl_markup_post_command_cancels_live_timer_then_schedules_exact_idle_callback() {
    let elisp_form = r##"(let ((atl-markup--timer
                :old-timer)
               (atl-markup-delay
                0.375)
               events)
...
#[test]
fn atl_markup_post_command_keeps_non_timer_sentinel_while_still_scheduling() {
    let elisp_form = r##"(let ((atl-markup--timer
                :not-a-timer)
               (atl-markup-delay
                2.5)
...
#[test]
fn atl_markup_post_command_preserves_live_timer_when_cancellation_signals() {
    let elisp_form = r##"(let ((atl-markup--timer
                :live)
               events)
          (cl-letf
...
#[test]
fn atl_markup_post_command_clears_cancelled_timer_before_schedule_failure() {
    let elisp_form = r##"(let ((atl-markup--timer
                :live)
               (atl-markup-delay
                0.1)
...
fn atl_markup_repeated_post_commands_discard_scheduled_tokens_and_never_cancel_them() {
    let elisp_form = r##"(let ((atl-markup--timer nil)
...
#[test]
fn atl_markup_post_command_forwards_edge_delay_values_without_package_validation() {
