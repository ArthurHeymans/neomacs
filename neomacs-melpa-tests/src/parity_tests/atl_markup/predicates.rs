...
#[test]
fn atl_markup_inside_tag_respects_narrowing_and_preserves_point_and_contents() {
          ...
          (let ((original-point
            ...
            (list
             ...
             (equal
              ...
              (buffer-string))
             (buffer-narrowed-p))))"##;
    let expect = expect!["OK (t nil nil t t nil)"];
    assert_atl_markup_parity(elisp_form, expect);
}

#[test]
fn atl_markup_comment_predicate_follows_emacs_lisp_syntax_not_comment_like_text() {
    let elisp_form = r##"(mapcar
          (lambda (contents)
            (cons
             contents
...
#[test]
fn atl_markup_comment_predicate_handles_markup_comments_and_real_tag_attributes() {
    let elisp_form = r##"(mapcar
          (lambda (contents)
            (cons
             contents
...
#[test]
fn atl_markup_comment_predicate_obeys_buffer_local_custom_comment_syntax() {
    let elisp_form = r##"(mapcar
          (lambda (contents)
            (with-temp-buffer
              (set-syntax-table
