...
#[test]
fn atcoder_tools_multiple_problem_buffers_keep_paths_commands_and_urls_isolated() {
    let elisp_form = r##"(let* ((root
                (atcoder-tools-test-root))
               (problems
...
#[test]
fn atcoder_tools_multi_extension_source_uses_only_final_extension_for_executable() {
    let elisp_form = r##"(let* ((root
                (atcoder-tools-test-root))
               (source
                (atcoder-tools-test-write-file
              ...
              (list
               (atcoder-tools-test-error-data
                (lambda ()
                  (atcoder-tools-test)))
               compile-calls))))"##;
    let expect = expect!["OK ((:error wrong-type-argument (stringp nil)) nil)"];
    assert_atcoder_tools_parity(elisp_form, expect);
}
