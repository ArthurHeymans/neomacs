...
fn auto_auto_indent_raw_source_enable_without_legacy_cl_pushnew_surfaces_upstream_failure() {
    ...
    let expect = expect!["OK ((:error void-function (pushnew)) t (aai-post-command-hook) nil)"];

    assert_auto_auto_indent_parity(elisp_form, expect);
}

#[test]
fn auto_auto_indent_enable_installs_buffer_local_hooks_keymap_and_mode_line() {
...
#[test]
fn auto_auto_indent_repeated_enable_disable_and_reenable_are_hook_idempotent() {
    let elisp_form = r##"(with-temp-buffer
          (fundamental-mode)
          (let (states)
...
#[test]
fn auto_auto_indent_major_mode_setup_selects_defun_strategy_only_for_lisp_modes() {
...
#[test]
fn auto_auto_indent_mode_hook_runs_before_internal_hook_key_and_strategy_setup() {
    let elisp_form = r##"(with-temp-buffer
          (emacs-lisp-mode)
          (let (observed)
            (add-hook
...
#[test]
fn auto_auto_indent_c_v_binding_is_added_only_when_current_binding_is_cua_paste() {
    let elisp_form = r##"(mapcar
          (lambda (binding)
            (with-temp-buffer
              (use-local-map
...
#[test]
fn auto_auto_indent_mode_map_contains_every_documented_command_remapping() {
    let elisp_form = r##"(with-temp-buffer
          (fundamental-mode)
          (auto-auto-indent-mode 1)
          (mapcar
...
#[test]
fn auto_auto_indent_mode_state_hooks_and_change_flags_are_isolated_per_buffer() {
...
#[test]
fn auto_auto_indent_aai_mode_function_and_variable_aliases_stay_synchronized() {
    let elisp_form = r##"(with-temp-buffer
          (fundamental-mode)
          (let ((same-function
                 (eq
...
#[test]
fn auto_auto_indent_late_multiple_cursors_and_paredit_integrations_apply_once() {
    ...
    let expect = expect!["OK ((aai-mode) 1 aai-delete-char aai-backspace)"];
