...
#[test]
fn loading_the_theme_registers_its_documented_palette_for_every_probed_face() {
    let elisp_form = r##"(let ((before (list :themes (copy-sequence custom-enabled-themes)
...
/// Every option the theme documents, each one reloaded so the theme re-reads
/// it: padding the mode line gives the two mode-line faces a ten-pixel box in
/// their own background colour, graying the outlines replaces the coloured
/// heading palette with shades of gray, graying the rainbow delimiters
/// collapses the depth colours onto one gray, bolding `vertico-current' adds a
/// weight, and switching off the completions-first-difference highlight leaves
/// that face's clause with no attributes at all.
#[test]
fn each_documented_toggle_changes_the_registered_appearance() {
...
/// Turning the theme off again has to leave the editor exactly as it was: the
/// captured baseline of resolved attributes comes back, the theme's settings
/// are gone from every face it touched, and `custom-enabled-themes' and
/// `custom-theme-enabled-p' agree that nothing is enabled -- while
/// `custom-theme-p' still reports the theme as loaded, ready to be re-enabled.
#[test]
fn disabling_the_theme_restores_the_captured_baseline_exactly() {
    let elisp_form = r##"(unwind-protect
...
/// A user who keeps a small personal overlay theme on top of a colour scheme:
/// the theme loaded last comes first in `custom-enabled-themes' and its
/// settings go to the front of each face's registration, so the overlay wins
/// for the faces it names and adwaita still supplies the rest.  Disabling the
/// overlay hands those faces back to adwaita.
#[test]
fn a_second_theme_loaded_on_top_takes_precedence_and_disabling_it_hands_back() {
    let elisp_form = r##"(unwind-protect
    (progn
      (adwaita-test-reset)
      (load-theme 'adwaita-dark t)
...
/// The theme's optional extras, each one a documented `eval-after-load' call:
/// the arrow bitmaps replace the four standard continuation and wrap bitmaps in
/// place (the fringe slots keep their numbers, the bitmap data behind them
/// changes), the flymake extras point all three diagnostic bitmaps at the
/// theme's marker with the compilation faces, and the diff-hl extra installs a
/// bitmap function that answers with the theme's bitmap for any change type.
#[test]
fn the_fringe_extras_install_the_themes_own_bitmaps() {
    let elisp_form = r##"(unwind-protect
    (progn
      (adwaita-test-reset)
      (load-theme 'adwaita-dark t)
