...
#[test]
fn package_loads_exact_version_and_complete_core_feature_graph() {
    let elisp_form = r##"
(list
...
#[test]
fn backend_registry_exposes_complete_actionable_contracts() {
    let elisp_form = r##"
(mapcar
 (lambda (entry)
   (let ((key (car entry))
...
#[test]
fn repository_backend_affinity_is_normalized_isolated_and_replaceable() {
    let elisp_form = r##"
...
"##;
    let expect = expect![[r#"OK (t t gemini gemini "Gemini CLI")"#]];
    assert_ai_code_parity(elisp_form, expect);
}

#[test]
fn selecting_backend_rebinds_all_dispatch_points_and_cli_identity() {
    let elisp_form = r##"
...
#[test]
fn terminal_output_cleanup_preserves_content_and_rejects_control_noise() {
    let elisp_form = r##"
...
"##;
    let expect = expect![[r#"OK ("dashboard" "done" "beforeafter" (nil nil nil nil 0 0))"#]];
    assert_ai_code_parity(elisp_form, expect);
}

#[test]
fn safe_terminal_paste_routes_single_and_multiline_input_correctly() {
    let elisp_form = r##"
(let (events)
