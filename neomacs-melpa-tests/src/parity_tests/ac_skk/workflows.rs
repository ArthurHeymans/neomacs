...
#[test]
fn converts_a_typed_reading_into_kanji_through_the_completion_menu() {
    let elisp_form = r##"
        (progn
...
/// SKK learns.  After committing 感じ the personal dictionary is reordered in
/// memory so the chosen conversion heads its reading, and the next conversion
/// of the same reading offers 感じ first.  Nothing is written to disk: DDSKK's
/// `skk-save-jisyo' is a no-op under `noninteractive', so the sandbox home
/// stays empty apart from the record file DDSKK creates and never fills.
#[test]
fn learns_the_chosen_conversion_and_offers_it_first_next_time() {
    let elisp_form = r##"
        (progn
          (ac-skk-test-install-jisyo)
          (ac-skk-test-open "memo.txt")
...
/// A real user's SKK-JISYO is not UTF-8.  With `skk-jisyo-code' left at its
/// DDSKK default the dictionary is read as euc-jis-2004, so this workflow
/// writes the fixture as raw EUC bytes — pinning the first entry's bytes so a
/// decoding regression cannot pass — and then converts out of it.  The
/// conversion has to produce the same kanji as the UTF-8 dictionary does.
#[test]
fn reads_a_legacy_encoded_dictionary_and_converts_out_of_it() {
    let elisp_form = r##"
...
/// ac-skk's second source works without henkan mode at all.  Typing plain
/// romaji leaves kana in the buffer; `ac-source-skk-hiracomp' segments the
/// trailing kana with tinysegmenter and offers the dictionary conversions of
/// that segment, the katakana reading, and one `▽'-marked variant per position
/// so the user can say where the conversion should start.  Completing a plain
/// candidate rewrites the kana; completing a `▽' variant instead runs
/// `ac-skk-hiracomp-mes', which sets the henkan point there and restarts
/// completion — leaving SKK in henkan mode on the tail.
#[test]
fn completes_plain_kana_with_the_hiracomp_source() {
    let elisp_form = r##"
        (progn
          (ac-skk-test-install-jisyo)
          (ac-skk-test-open "memo.txt")
...
/// ac-skk is opt-in and has to leave a buffer exactly as it found it.
/// `ac-skk-enable'/`ac-skk-disable'/`ac-skk-toggle' flip one flag and say so;
/// while it is off, turning on `skk-mode' must not touch `ac-sources' at all.
/// While it is on, `skk-mode' swaps in the SKK sources and makes `skk-insert' a
/// completion trigger, switching to ASCII input puts the original sources back,
/// returning to kana input restores them, and leaving `skk-mode' cleans up the
/// buffer-local state entirely.
#[test]
fn enabling_and_leaving_skk_mode_restore_the_original_sources() {
    let elisp_form = r##"
        (progn
          (ac-skk-test-install-jisyo)
          (list
