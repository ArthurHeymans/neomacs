...
fn atom_dark_theme_enable_applies_representative_faces_then_disable_restores_baseline() {
    let elisp_form = r##"(let ((base 'atom-dark-parity-deterministic-base)
               (requests
                '((default :foreground)
                  (default :background)
                  (default :weight)
...
#[test]
fn atom_dark_theme_stacks_over_an_existing_theme_and_reveals_it_after_disable() {
    let elisp_form = r##"(let ((base 'atom-dark-parity-base)
               during
               after)
...
#[test]
fn atom_dark_theme_later_theme_wins_and_disabling_it_restores_atom_dark() {
    let elisp_form = r##"(let ((overlay 'atom-dark-parity-overlay)
               atom
               overlaid
               restored)
...
#[test]
fn atom_dark_theme_font_families_and_inheritance_resolve_in_practical_rendering() {
    let elisp_form = r##"(let ((faces
                '(button
                  link
                  link-visited
...
fn atom_dark_theme_malformed_lifecycle_requests_signal_without_corrupting_registered_theme() {
    let elisp_form = r##"(let ((settings-before
                (copy-tree
                 (get 'atom-dark 'theme-settings))))
         (list
