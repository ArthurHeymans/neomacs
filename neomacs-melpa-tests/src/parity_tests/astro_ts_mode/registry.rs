...
use super::{assert_astro_ts_mode_autoload_signal_parity, assert_astro_ts_mode_parity};

#[test]
fn descriptor_records_exact_pin_dependency_and_installed_payload() {
    let elisp_form = r##"(let* ((desc (cadr (assq 'astro-ts-mode package-alist)))
...
#[test]
fn installed_source_has_exact_content_hash_feature_and_emacs_30_requirement() {
    let elisp_form = r##"(let ((source (locate-library "astro-ts-mode"))
...
#[test]
fn complete_declared_variable_surface_has_exact_defaults_kinds_groups_and_docs() {
    let elisp_form = r##"(mapcar
          (lambda (symbol)
            (list symbol
                  (boundp symbol)
...
#[test]
fn indent_registry_covers_astro_css_and_tsx_with_real_anchor_rules() {
...
#[test]
fn font_lock_registry_has_prefixed_embedded_features_and_native_astro_rules() {
...
#[test]
fn range_registry_embeds_tsx_and_css_in_every_documented_astro_context() {
    let elisp_form = r##"(list
...
#[test]
fn mode_metadata_auto_association_and_source_ownership_are_exact() {
    let elisp_form = r##"(list
...
#[test]
fn autoload_file_signals_when_treesit_was_not_preloaded_like_upstream() {
    let elisp_form = r##"(list
          (featurep 'astro-ts-mode)
          (fboundp 'astro-ts-mode))"##;
    let expect = expect!["ERR (void-function treesit-ready-p)"];
    assert_astro_ts_mode_autoload_signal_parity(elisp_form, expect);
}

#[test]
fn repeated_source_loading_is_idempotent_for_feature_association_and_api() {
    let elisp_form = r##"(let ((source (locate-library "astro-ts-mode"))
              snapshots)
          (dotimes (_ 3)
