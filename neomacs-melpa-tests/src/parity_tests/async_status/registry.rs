use super::{assert_async_status_autoload_parity, assert_async_status_parity};
use expect_test::expect;

#[test]
fn installed_descriptor_matches_the_exact_pin_commit_and_dependencies() {
...
#[test]
fn packaged_source_has_the_verified_melpa_payload_content_hash() {
    let elisp_form = r##"(let* ((description
        (cadr (assq 'async-status package-alist)))
       (source
        (expand-file-name
...
#[test]
fn complete_declared_function_surface_is_bound_after_loading() {
...
#[test]
fn generated_item_struct_surface_is_complete_and_callable() {
    let elisp_form = r##"(mapcar
 (lambda (symbol)
   (list
    symbol
...
#[test]
fn variables_custom_types_defaults_and_group_metadata_match() {
...
#[test]
fn item_constructor_accessors_copy_and_mutation_have_value_semantics() {
...
#[test]
fn item_constructor_accepts_partial_and_reordered_keyword_arguments() {
    let elisp_form = r##"(let ((empty (make-async-status--item))
      (partial
       (make-async-status--item
        :label "Only label"
        :msg-id "partial")))
...
#[test]
fn repeated_source_loads_preserve_customized_values_and_existing_defvar_state() {
  ...
  (prog1
    ...
    let expect = expect!["OK (t 77 91.5 (\"kept\"))"];
    assert_async_status_parity(elisp_form, expect);
}
...
fn generated_autoload_file_registers_only_its_feature_without_eager_runtime_loading() {
  ...
  (list
   ...
   (cdr history)))"##;
    ...
    assert_async_status_autoload_parity(elisp_form, expect);
}
