...
#[test]
fn evil_normal_motion_keys_move_by_words_lines_and_line_boundaries() {
...
#[test]
fn evil_insert_append_and_line_open_commands_preserve_text_point_and_final_state() {
...
#[test]
fn evil_yank_and_paste_commands_preserve_characterwise_and_linewise_shapes() {
    let elisp_form = r##"(mapcar
               (lambda (keys)
                 (with-temp-buffer
                   (let ((kill-ring nil)
...
#[test]
fn evil_visual_character_and_line_operations_transform_exact_regions() {
    let elisp_form = r##"(mapcar
               (lambda (keys)
                 (with-temp-buffer
                   (insert "alpha beta\nsecond line\n")
...
#[test]
fn evil_find_till_and_repeat_find_commands_track_direction_and_offsets() {
    let elisp_form = r##"(with-temp-buffer
               (insert "a1b2c3b4a")
               (goto-char (point-min))
               (evil-local-mode 1)
...
#[test]
fn evil_join_case_inversion_and_rot13_commands_match_vim_text_transformations() {
    let elisp_form = r##"(mapcar
               (lambda (case)
                 (with-temp-buffer
                   (insert (car case))
               ...
               '(("alpha\nbeta\n" . "J")
                 ...
                 ("Alpha beta" . "v e ~")
                 ("Alpha beta" . "g ? e")))"##;
    let expect = expect![[r#"OK (("J" 2 nil) ("J~" 3 nil) ("J~ve~" 6 nil) ("J~ve~g?e" 9 nil))"#]];
