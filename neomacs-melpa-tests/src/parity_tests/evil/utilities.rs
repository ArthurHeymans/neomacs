...
#[test]
fn evil_property_helpers_set_merge_and_query_single_and_all_symbols() {
...
#[test]
fn evil_filter_list_returns_and_destructively_relinks_nonmatching_items() {
    let elisp_form = r##"(let ((one '(nil))
                    (two '(nil 1 2 nil))
                    (three '(1 nil nil 2))
                    (four '(1 nil 2 nil 3)))
               (list
                ...
                (progn
                  ...
                  four)))"##;
    let expect = expect!["OK (nil (1 2) (1 2) (1 2 3))"];

    assert_evil_parity(elisp_form, expect);
}

#[test]
fn evil_concat_helpers_deduplicate_lists_and_override_association_values() {
...
#[test]
fn evil_sort_macro_orders_two_three_and_four_places_in_the_calling_scope() {
    let elisp_form = r##"(list
               (let ((a 2) (b 1))
                 (evil-sort a b)
                 (list a b))
               ...
               (let ((a 4) (b 3) (c 2) (d 1))
                 (evil-sort a b c d)
                 (list a b c d)))"##;
    let expect = expect!["OK ((1 2) (1 2 3) (1 2 3 4))"];

    assert_evil_parity(elisp_form, expect);
}

#[test]
fn evil_extract_count_parses_exact_commands_counts_and_trailing_keys() {
...
#[test]
fn evil_extract_count_rejects_a_count_without_a_command() {
    let elisp_form = r##"(with-temp-buffer
               (evil-local-mode 1)
               (evil-normal-state)
               (evil-extract-count "1230"))"##;
    let expect = expect![[r#"ERR (user-error "Key sequence contains no complete binding")"#]];

    assert_evil_signal_parity(elisp_form, expect);
}

#[test]
fn evil_vim_regexp_transform_handles_classes_escaped_backslashes_and_magic_modes() {
    let elisp_form = r##"(let ((patterns
                    '("x\\sx"
                      "x\\Dx"
                      "x\\wx"
                      "x\\Lx"
...
#[test]
fn evil_digraph_lookup_handles_builtin_reverse_user_and_missing_pairs() {
    let elisp_form = r##"(progn
               ...
               (let ((evil-digraphs-table-user
                 ...
                 (list
                  ...
                  (evil-digraph '(?~ ?~))
                  (evil-digraph '(?q ?q)))))"##;
    let expect = expect!["OK (9731 9731 229 196 nil nil)"];
