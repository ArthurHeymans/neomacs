...
#[test]
fn evil_ex_parser_builds_exact_command_range_and_argument_forms() {
...
#[test]
fn evil_ex_parser_builds_exact_percent_marker_numeric_and_relative_ranges() {
    let elisp_form = r##"(list
...
#[test]
fn evil_ex_line_and_range_helpers_clamp_offsets_and_cover_the_buffer() {
...
#[test]
fn evil_ex_command_definitions_support_exact_prefix_and_completed_bindings() {
    let elisp_form = r##"(let ((evil-ex-commands nil))
...
#[test]
fn evil_ex_completed_binding_rejects_an_ambiguous_command_prefix() {
    let elisp_form = r##"(let ((evil-ex-commands nil))
               (evil-ex-define-cmd "neotest" #'forward-char)
               (evil-ex-define-cmd "neotoggle" #'backward-char)
               (evil-ex-completed-binding "neot"))"##;
    let expect = expect![[r#"ERR (user-error "Unknown command: ‘neot’")"#]];

    assert_evil_signal_parity(elisp_form, expect);
}

#[test]
fn evil_ex_regex_case_helpers_cover_smart_explicit_and_escaped_overrides() {
    let elisp_form = r##"(list
...
#[test]
fn evil_search_moves_forward_backward_honors_case_and_wrap_settings() {
    let elisp_form = r##"(with-temp-buffer
...
#[test]
fn evil_ex_execute_applies_substitute_delete_copy_move_and_sort_commands() {
