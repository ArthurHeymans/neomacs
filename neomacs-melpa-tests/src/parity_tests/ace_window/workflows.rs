...
#[test]
fn jumping_between_labelled_windows_in_a_three_window_session() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
...
/// `aw-dispatch-when-more-than' is 2, so with one or two windows ace-window
/// behaves like `other-window' and reads no key at all.  The character typed
/// after the command therefore reaches the buffer, which is what proves no
/// label was consumed.
#[test]
fn one_and_two_window_layouts_switch_without_asking_for_a_label() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
...
/// Two documented dispatch actions.  `m' swaps the current window's buffer
/// with the labelled one; `n' takes no window argument at all and flips back
/// to where the user came from.
#[test]
fn swapping_two_windows_and_flipping_back_to_the_previous_one() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
...
/// `o' makes the labelled window the only one and `x' deletes it.  Both are
/// asserted through the resulting frame, and the deleted window's buffer has to
/// survive -- ace-window deletes windows, not buffers.
#[test]
fn deleting_a_window_and_maximizing_another_through_dispatch() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
...
/// `v', `b' and `F' split the labelled window rather than the current one.
/// The resulting edges are the whole point, so the snapshots pin the exact
/// geometry each split produces.
#[test]
fn splitting_a_chosen_window_vertically_horizontally_and_fairly() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
...
/// `aw-ignore-current' and `aw-ignored-buffers' change which windows are
/// candidates, which renumbers every label after them.  The same keystroke
/// therefore has to land somewhere else.
#[test]
fn ignoring_the_current_window_or_a_named_buffer_reshuffles_the_labels() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
...
/// The abort paths.  `C-g' quits immediately; a label with no window makes
/// avy say so and wait for another key.  Neither may change the frame, leave
/// `aw-action' set, or leave an overlay behind, and the next real label must
/// still work.
#[test]
fn quitting_or_pressing_an_unused_label_leaves_the_layout_untouched() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
...
/// `ace-window-display-mode' shows each window's label permanently.  It works
/// by setting an `ace-window-path' window parameter and prepending an entry to
/// the default `mode-line-format', both of which have to be undone on exit.
#[test]
fn display_mode_puts_each_windows_label_in_its_mode_line() {
    let elisp_form = r####"
(unwind-protect
    (progn
      (aw-test-session)
