...
#[test]
fn auto_dark_ns_applescript_adapter_uses_exact_program_and_truth_contract() {
...
#[test]
fn auto_dark_mac_applescript_adapter_uses_quoted_truth_contract() {
    let elisp_form = r##"(let (calls)
          (cl-letf
...
#[test]
fn auto_dark_shell_adapters_forward_exact_commands_and_parse_outputs() {
    let elisp_form = r##"(let (calls)
          (cl-letf
              (((symbol-function
...
#[test]
fn auto_dark_powershell_parser_selects_first_numeric_line_and_only_zero_is_dark() {
...
#[test]
fn auto_dark_winreg_adapter_forwards_exact_registry_query_and_type_contract() {
    let elisp_form = r##"(let (calls)
          (cl-letf
              (((symbol-function
                 'w32-read-registry)
...
#[test]
fn auto_dark_dbus_adapter_maps_portal_color_scheme_and_forwards_exact_call() {
    let elisp_form = r##"(let (calls)
          (cl-letf
              (((symbol-function
                 'dbus-call-method)
...
#[test]
fn auto_dark_current_system_mode_dispatches_every_configured_detector() {
    let elisp_form = r##"(let (events)
          (cl-letf
              (((symbol-function
                 'auto-dark--current-mode-applescript)
...
#[test]
fn auto_dark_current_system_mode_fallback_priority_and_warning_contract_match() {
...
fn auto_dark_detection_method_feature_matrix_selects_each_supported_platform_path() {
    let elisp_form = r##"(let (shell-calls dbus-calls)
          (cl-letf
              (((symbol-function 'ns-do-applescript)
                #'ignore)
               ((symbol-function
...
fn auto_dark_detection_method_unsupported_platform_warns_and_returns_display_warning_result() {
    let elisp_form = r##"(let ((system-type 'berkeley-unix)
                               (window-system nil)
                               (features
                                (remq 'dbus features))
                               warnings)
