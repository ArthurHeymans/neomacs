...
#[test]
fn auto_complete_distel_prefix_uses_the_narrowed_buffer_boundary_as_its_start() {
    let elisp_form = r##"(with-temp-buffer
          (insert
            ...
            (let ((prefix
              ...
              (list
               ...
               (buffer-substring-no-properties
                prefix
                (point))))))"##;
    let expect = expect![[r#"OK (15 24 15 "lists:map")"#]];

    assert_auto_complete_distel_parity(elisp_form, expect);
}

#[test]
fn auto_complete_distel_prefix_preserves_point_mark_and_existing_match_data() {
    let elisp_form = r##"(with-temp-buffer
          (insert
          ...
          (let ((before
            ...
            (list
             ...
             (mark)
             (match-data))))"##;
    let expect = expect![[r#"OK ((17 3 (0 8 0 4 4 8)) 8 "lists:map" 17 3 (0 8 0 4 4 8))"#]];
...
#[test]
fn auto_complete_distel_invalid_valid_syntax_values_signal_exactly() {
    let elisp_form = r##"(mapcar
          (lambda (syntax)
            (with-temp-buffer
              (insert "lists:map")
