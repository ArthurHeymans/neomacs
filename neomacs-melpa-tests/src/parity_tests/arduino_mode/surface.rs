...
use super::{assert_arduino_init_parity, assert_arduino_mode_parity, assert_ede_arduino_parity};

#[test]
fn installed_descriptor_dependencies_and_files_identify_the_exact_melpa_build() {
...
#[test]
fn legacy_init_file_registers_both_extensions_and_two_lazy_entry_points() {
    ...
    assert_arduino_init_parity(elisp_form, expect);
}

#[test]
fn main_module_complete_callable_surface_has_exact_arglists_and_command_status() {
...
#[test]
fn main_module_custom_options_and_runtime_state_have_exact_metadata() {
    let elisp_form = r##"(list
                    (mapcar
                     (lambda (symbol)
                       (list
...
#[test]
fn mode_keymap_and_menu_expose_every_documented_operation() {
    let elisp_form = r##"(list
                    (eq
...
#[test]
fn ede_module_complete_callable_surface_has_exact_arglists_and_command_status() {
    let elisp_form = r##"(mapcar
                    (lambda (symbol)
                      (list
                       symbol
...
#[test]
fn ede_custom_options_classes_and_project_registration_are_exact() {
    let elisp_form = r##"(list
                    (mapcar
                     (lambda (symbol)
                       (list
