...
use super::{
    assert_auto_complete_clang_async_autoload_parity, assert_auto_complete_clang_async_parity,
};
...
#[test]
fn auto_complete_clang_async_installed_payload_inventory_and_exact_hashes_match() {
    let elisp_form = r##"(let* ((descriptor
                                 (cadr
                                  (assq
                                   'auto-complete-clang-async
...
#[test]
fn auto_complete_clang_async_complete_prefixed_symbol_inventory_matches() {
...
#[test]
fn auto_complete_clang_async_faces_and_completion_source_contracts_match() {
...
#[test]
fn auto_complete_clang_async_source_load_history_records_complete_contract() {
...
fn auto_complete_clang_async_generated_autoload_contains_only_package_metadata_and_provide() {
    let elisp_form = r##"(let* ((file
                           ...
                           (list
                            ...
                            (boundp
                             'ac-source-clang-async)))"##;
    let expect = expect!["OK (t nil ((provide . auto-complete-clang-async-autoloads)) nil nil)"];

    assert_auto_complete_clang_async_autoload_parity(elisp_form, expect);
}
