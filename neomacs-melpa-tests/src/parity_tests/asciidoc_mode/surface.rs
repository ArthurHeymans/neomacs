...
use super::{assert_asciidoc_mode_autoload_parity, assert_asciidoc_mode_parity};

#[test]
fn exact_descriptor_runtime_bytes_and_archive_file_set_identify_the_melpa_build() {
...
#[test]
fn generated_autoloads_register_only_the_two_commands_and_both_file_extensions() {
 ...
 (boundp 'asciidoc-mode-map))"##;
    ...
    assert_asciidoc_mode_autoload_parity(elisp_form, expect);
}
...
#[test]
fn generated_public_mode_variables_preserve_values_types_and_source_ownership() {
    let elisp_form = r##"(mapcar
 (lambda (symbol)
   (list
    symbol
...
#[test]
fn every_public_option_preserves_default_type_group_documentation_and_source() {
    let elisp_form = r##"(mapcar
 (lambda (symbol)
   (list
    symbol
...
#[test]
fn public_constants_keymaps_menu_and_customization_group_are_complete() {
    let elisp_form = r##"(list
...
#[test]
fn every_package_face_exists_with_exact_group_inheritance_and_source_ownership() {
    let elisp_form = r##"(mapcar
 (lambda (face)
   (list
    face
