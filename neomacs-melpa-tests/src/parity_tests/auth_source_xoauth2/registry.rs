...
use super::{assert_auth_source_xoauth2_autoload_parity, assert_auth_source_xoauth2_parity};

#[test]
fn auth_source_xoauth2_descriptor_and_sources_pin_exact_melpa_payload() {
...
#[test]
fn auth_source_xoauth2_feature_and_definition_origins_are_exact() {
...
#[test]
fn auth_source_xoauth2_public_functions_have_exact_callable_contracts() {
...
#[test]
fn auth_source_xoauth2_defaults_and_backend_object_are_exact() {
    let elisp_form = r##"(list
         auth-source-xoauth2-creds
         auth-source-xoauth2-use-curl
         (mapcar
...
#[test]
fn auth_source_xoauth2_source_load_history_records_definitions_and_provide() {
...
#[test]
fn auth_source_xoauth2_generated_autoload_registers_enable_function() {
    let elisp_form = r##"(let* ((file
                 (locate-library
    ...
    assert_auth_source_xoauth2_autoload_parity(elisp_form, expect);
}

#[test]
fn auth_source_xoauth2_reload_preserves_configuration_and_parser_advice() {
    let elisp_form = r##"(let ((source
         ...
         (list
          ...
          (and
           ...
           t)
          (featurep 'auth-source-xoauth2)))"##;
    let expect = expect![[r#"OK ((:token-url "custom") t t t)"#]];
    assert_auth_source_xoauth2_parity(elisp_form, expect);
}
