...
#[test]
fn ac_sly_completes_a_typed_symbol_in_a_lisp_buffer_from_the_live_connection() {
    let elisp_form = r##"(ac-sly-test-session
 (ac-sly-test-connect)
...
/// `set-up-sly-ac' with a prefix argument installs the fuzzy source instead,
/// which asks the Lisp for flex completions and shows each result's
/// classification and score as the candidate's summary.  `ac-sly-show-flags' is
/// the package's one option: turned off, the same completions arrive with no
/// summary at all.
#[test]
fn ac_sly_fuzzy_source_shows_the_lisps_flags_unless_the_option_is_off() {
    let elisp_form = r##"(ac-sly-test-session
 (ac-sly-test-connect)
...
/// The other half of the package's documented setup is the REPL: with the
/// mrepl contrib active, connecting creates a real `sly-mrepl-mode' buffer with
/// a prompt, and `set-up-sly-ac' on its hook makes the same completions
/// available there.  The completion is inserted into the REPL's input field,
/// after the prompt.
#[test]
fn ac_sly_completes_at_the_sly_repl_prompt_after_set_up_sly_ac() {
    let elisp_form = r##"(ac-sly-test-session
...
/// Both sources carry a `document' function, which is what auto-complete's
/// quick help calls to describe the selected candidate.  ac-sly is a
/// translation of ac-slime and this one function was never translated: it asks
/// the Lisp for `swank:documentation-symbol', a package no slynk image has, so
/// the Lisp aborts the evaluation and the user gets an error instead of a
/// docstring.  The same request under the `slynk' package answers normally,
/// which shows the connection and the fixture are sound.
#[test]
fn ac_sly_documentation_asks_the_lisp_for_a_swank_symbol_it_cannot_read() {
    let elisp_form = r##"(ac-sly-test-session
 (ac-sly-test-connect)
...
/// Both candidate functions are guarded by `sly-connected-p', so before the
/// Lisp is up the source is installed and enabled but simply offers nothing --
/// no menu, no request, and nothing typed into the buffer is disturbed.  The
/// same keystrokes complete normally once the connection exists.
#[test]
fn ac_sly_offers_nothing_until_a_lisp_connection_exists() {
    let elisp_form = r##"(ac-sly-test-session
 (ac-sly-test-lisp-buffer "(ca")
 (set-up-sly-ac)
 (auto-complete-mode 1)
