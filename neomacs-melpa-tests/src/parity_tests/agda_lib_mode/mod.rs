...
fn assert_agda_lib_mode_source_parity(source_file: &str, elisp_form: &str, expected: Expect) {
    let name = current_test_name();
    let report = agda_lib_mode_oracle(source_file)
        .run_value(&name, elisp_form)
        .unwrap_or_else(|error| panic!("agda-lib-mode parity case `{name}` failed:\n{error}"));
    expected.assert_eq(&report.gnu_emacs.to_string());
}

pub(crate) fn assert_agda_lib_mode_parity(elisp_form: &str, expected: Expect) {
    assert_agda_lib_mode_source_parity("agda-lib-mode.el", elisp_form, expected);
}

pub(crate) fn assert_agda_lib_mode_autoload_parity(elisp_form: &str, expected: Expect) {
    assert_agda_lib_mode_source_parity("agda-lib-mode-autoloads.el", elisp_form, expected);
}
