...
use super::{assert_anki_connect_missing_dependency_signal, assert_anki_connect_parity};

#[test]
fn anki_connect_declared_install_cannot_create_a_hierarchical_deck_without_s() {
    let elisp_form = r##"(anki-connect-ensure-deck
                           "Languages::Spanish")"##;
    let expect = expect!["ERR (void-function s-split)"];

    assert_anki_connect_missing_dependency_signal(elisp_form, expect);
}

#[test]
fn anki_connect_creates_missing_deck_hierarchy_once_through_the_http_api() {
...
#[test]
fn anki_connect_discovers_a_model_then_adds_and_updates_a_realistic_note() {
...
#[test]
fn anki_connect_protocol_error_rejects_a_note_without_mutating_collection_state() {
    let elisp_form = r##"(let (requests response-buffer)
                           (unwind-protect
                               (cl-letf
                                   (((symbol-function
                                      'url-retrieve-synchronously)
