...
use super::{assert_auto_compile_autoload_parity, assert_auto_compile_parity};

#[test]
fn auto_compile_descriptor_and_sources_pin_exact_melpa_payload() {
...
#[test]
fn auto_compile_feature_aliases_and_definition_origins_are_exact() {
...
#[test]
fn auto_compile_public_commands_have_exact_interactive_contracts() {
...
#[test]
fn auto_compile_options_have_exact_defaults_and_custom_types() {
    let elisp_form = r##"(mapcar
         (lambda (symbol)
           (list
            symbol
...
#[test]
fn auto_compile_source_load_history_records_modes_advices_and_provider() {
    let elisp_form = r##"(let* ((file (locate-library "auto-compile"))
...
fn auto_compile_generated_autoloads_register_all_entry_points_without_loading_source() {
    ...
    assert_auto_compile_autoload_parity(elisp_form, expect);
}

#[test]
fn auto_compile_reload_preserves_user_options_and_installs_each_advice_once() {
    let elisp_form = r##"(let ((source (locate-library "auto-compile")))
         ...
         (list
          ...
          (featurep 'auto-compile)))"##;
    let expect = expect!["OK (nil nil nil (t t t t t) t)"];
    assert_auto_compile_parity(elisp_form, expect);
}

#[test]
fn auto_compile_group_links_and_mode_variable_metadata_are_exact() {
    let elisp_form = r##"(list
