...
#[test]
fn auto_compile_local_mode_adds_and_removes_only_its_buffer_local_save_hook() {
    ...
    let expect = expect!["OK ((nil nil nil) (t t t 1) (nil nil nil))"];
    assert_auto_compile_parity(elisp_form, expect);
}

#[test]
fn auto_compile_local_mode_rejects_non_elisp_buffers_and_rolls_back_state() {
    let elisp_form = r##"(with-temp-buffer
         (fundamental-mode)
         (list
          major-mode
...
#[test]
fn auto_compile_global_save_mode_updates_existing_eligible_buffers_and_cleans_up() {
    ...
    let expect = expect!["OK (((emacs-lisp-mode t t) (fundamental-mode nil nil)) nil (nil nil))"];
    assert_auto_compile_parity(elisp_form, expect);
}
...
fn auto_compile_failed_modified_toggle_changes_option_and_reports_both_transitions() {
    let elisp_form = r##"(let ((auto-compile-mark-failed-modified nil))
...
#[test]
fn auto_compile_custom_mode_line_setter_repositions_and_removes_control() {
    let elisp_form = r##"(let ((original-format
...
#[test]
fn auto_compile_on_load_mode_has_stable_global_toggle_lifecycle() {
    let elisp_form = r##"(progn
         ...
         (let ((disabled
           ...
           (let ((enabled
             ...
             (list disabled enabled
                   auto-compile-on-load-mode))))"##;
    let expect = expect![[r#"OK ((nil nil t t) (t t "") nil)"#]];
    assert_auto_compile_parity(elisp_form, expect);
}

#[test]
fn auto_compile_ding_obeys_option_without_leaking_terminal_side_effects() {
    let elisp_form = r##"(let ((count 0))
         (cl-letf (((symbol-function 'ding)
           ...
           (let ((auto-compile-ding t))
             ...
             (auto-compile-ding))
           count))"##;
    let expect = expect!["OK 2"];
    assert_auto_compile_parity(elisp_form, expect);
}

#[test]
fn auto_compile_display_log_signals_when_absent_and_selects_existing_compile_log() {
