...
#[test]
fn theme_loaded_before_org_styles_a_real_release_document_with_visible_semantics() {
    let elisp_form = r##"(let ((theme 'ahungry))
  (unwind-protect
      (progn
        (enable-theme theme)
        (require 'org)
...
#[test]
fn real_unified_diff_resolves_file_hunk_context_removed_and_added_semantics() {
    let elisp_form = r##"(let ((theme 'ahungry))
  (unwind-protect
      (progn
        (enable-theme theme)
...
#[test]
fn completion_search_and_navigation_ui_resolve_core_and_extension_faces() {
    let elisp_form = r##"(let ((theme 'ahungry)
      (extension-faces
       '(helm-selection
         helm-match
