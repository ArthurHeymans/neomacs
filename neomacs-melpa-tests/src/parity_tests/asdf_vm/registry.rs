...
use super::{assert_asdf_vm_autoload_parity, assert_asdf_vm_parity};

#[test]
fn asdf_vm_descriptor_pins_exact_release_origin_and_dependency_contract() {
...
#[test]
fn asdf_vm_installed_payload_has_exact_inventory_sizes_and_source_hashes() {
    let elisp_form = r##"(let* ((descriptor
                     (cadr
                      (assq 'asdf-vm
                            package-alist)))
...
#[test]
fn asdf_vm_complete_public_variable_surface_has_exact_custom_metadata() {
    let elisp_form = r##"(mapcar
               (lambda (symbol)
                 (list
                  symbol
...
#[test]
fn asdf_vm_public_scalar_variable_defaults_form_one_exact_runtime_configuration() {
    let elisp_form = r##"(mapcar
               (lambda (symbol)
                 (let ((value
                        (symbol-value symbol)))
...
#[test]
fn asdf_vm_public_keymap_variable_values_bind_every_declared_command_exactly() {
...
fn asdf_vm_generated_autoloads_expose_exact_user_commands_without_loading_runtime() {
    ...
    assert_asdf_vm_autoload_parity(elisp_form, expect);
}
