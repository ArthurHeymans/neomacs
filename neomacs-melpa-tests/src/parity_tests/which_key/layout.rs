...
#[test]
fn which_key_column_normalization_and_joining_preserve_row_and_column_order() {
...
#[test]
fn which_key_partition_list_handles_even_remainder_oversized_and_empty_inputs() {
    let elisp_form = r##"(list
    ...
    let expect = expect!["OK (((a b) (c d)) ((a b) (c d) (e)) ((a b)) ((a) (b) (c)) nil)"];

    assert_which_key_parity(elisp_form, expect);
}

#[test]
fn which_key_list_to_pages_reports_exact_page_strings_and_metadata() {
    let elisp_form = r##"(let* ((which-key-add-column-padding 0)
...
#[test]
fn which_key_list_to_pages_respects_the_maximum_column_limit() {
    let elisp_form = r##"(let ((keys '(("a" ":" "alpha" nil)
                           ("b" ":" "beta" nil)
                           ("c" ":" "charlie" nil)
                           ("d" ":" "delta" nil))))
               (let ((which-key-max-display-columns 1))
                 (let ((pages (which-key--list-to-pages keys 2 80)))
                   (list
                    ...
                    (which-key--pages-keys/page pages)
                    (which-key--pages-pages pages)))))"##;
    let expect = expect![[r#"OK (2 (2 2) ("a:alpha\nb:beta " "c:charlie\nd:delta  "))"#]];

    assert_which_key_parity(elisp_form, expect);
}

#[test]
fn which_key_list_to_pages_signals_when_no_cell_can_fit_the_width() {
    let elisp_form = r##"(which-key--list-to-pages
               '(("a" ":" "alpha" nil)
                 ("b" ":" "beta" nil))
               2
               3)"##;
    let expect = expect!["ERR (wrong-type-argument wholenump -4)"];

    assert_which_key_signal_parity(elisp_form, expect);
}

#[test]
fn which_key_page_rotation_updates_every_parallel_page_field() {
    let elisp_form = r##"(let ((pages
...
#[test]
fn which_key_formatting_replaces_groups_extracts_keys_and_truncates_descriptions() {
    let elisp_form = r##"(let ((which-key-separator " : ")
...
#[test]
fn which_key_docstring_formatting_covers_append_only_and_disabled_modes() {
