...
use super::assert_acme_theme_parity;
...
/// stays the same.
#[test]
fn loading_acme_repaints_a_real_elisp_editing_session_with_the_plan_nine_palette() {
    let elisp_form = r####"
(let* ((root
...
/// acme styles the whole Org surface -- headline levels with `:overline', the
/// TODO/DONE boxes, tables, inline code and source blocks.  Marking a real
/// checklist item done has to leave both the saved document and that styling
/// exact.
#[test]
...
/// A patch review in `diff-mode' with a refined hunk and an active region,
/// followed by `M-x occur'.  Covers the faces acme guards with a colour
/// display class (`region', `diff-*') and the `match' face that `occur'
...
/// `acme-theme-black-fg' is the theme's only documented option.  The palette
/// is computed inside a `let' when the file loads, so the option reaches the
/// user only on the next `load-theme' -- and then it has to rewrite every
/// face derived from the default foreground without touching the rest of the
/// palette.
#[test]
...
/// A developer who already runs a dark theme tries acme for one session.
/// acme has to win for every face it defines, leave `bold', `italic' and
/// `shadow' to the theme underneath, and give the exact previous appearance
/// back on `disable-theme'.
#[test]
...
/// The installed package registers its own directory on
/// `custom-theme-load-path' from an autoload cookie.  Loading the theme twice
/// must not stack a second copy, grow the settings list, add a second path
/// entry, or change any resolved colour.
#[test]
