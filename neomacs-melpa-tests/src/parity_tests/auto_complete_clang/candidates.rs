...
#[test]
fn auto_complete_clang_call_process_saved_uses_file_process_without_region() {
    let elisp_form = r##"(with-temp-buffer
         (let ((ac-clang-executable
                 "/fake/clang")
             ...
             (let ((result
               ...
               (list
                ...
                (mapcar
                 #'ac-clang-test-candidate-state
                 result))))))"##;
    let expect = expect![[r#"OK ((3 ("-cc1" "-bad")) (("partial" "int partial" nil)))"#]];
    assert_auto_complete_clang_parity(elisp_form, expect);
}

#[test]
fn auto_complete_clang_string_comment_detector_uses_real_c_syntax_state() {
    let elisp_form = r##"(with-temp-buffer
         (c-mode)
         (insert
          "int code = 1;\n"
         ...
         (mapcar
          ...
          '("code" "comment text"
            "literal" "block")))"##;
    let expect = expect![[r#"OK (("code" nil) ("comment text" 15) ("literal" 47) ("block" 58))"#]];
    assert_auto_complete_clang_parity(elisp_form, expect);
}

#[test]
fn auto_complete_clang_candidate_skips_process_inside_comment_and_string() {
    let elisp_form = r##"(with-temp-buffer
         (c-mode)
         (insert
          "int value;\n"
...
#[test]
fn auto_complete_clang_prefix_prefers_symbol_then_member_access_operators() {
    let elisp_form = r##"(mapcar
         ...
         '(("identifier" 3)
           ...
           ("colon:" nil)
           ("" nil)))"##;
    let expect = expect!["OK (3 8 10 7 nil nil nil)"];
    assert_auto_complete_clang_parity(elisp_form, expect);
}
