...
use super::{assert_asm_blox_autoload_parity, assert_asm_blox_parity};

#[test]
fn asm_blox_exact_pin_descriptor_dependency_origin_and_feature_contract_match() {
...
#[test]
fn asm_blox_installed_payload_inventory_sizes_and_content_digests_match() {
    let elisp_form = r##"(let* ((descriptor
                  (cadr
                   (assq 'asm-blox package-alist)))
                 (directory
...
#[test]
fn asm_blox_complete_callable_command_arglist_and_source_surface_matches() {
...
#[test]
fn asm_blox_keymaps_modes_syntax_completion_font_lock_and_eldoc_registry_match() {
...
#[test]
fn asm_blox_generated_autoload_registers_entry_points_without_loading_feature() {
    let elisp_form = r##"(list
         (featurep 'asm-blox)
         (featurep 'asm-blox-puzzles)
         (mapcar
    ...
    assert_asm_blox_autoload_parity(elisp_form, expect);
}
