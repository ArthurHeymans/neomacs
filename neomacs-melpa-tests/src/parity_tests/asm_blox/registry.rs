...
use super::{ParityBatchCase, assert_asm_blox_autoload_batch, assert_asm_blox_batch};
...
fn asm_blox_complete_owned_variable_defaults_scope_custom_and_source_surface_matches() -> ParityBatchCase {
    ParityBatchCase::new(
...
fn asm_blox_struct_layout_constructor_predicate_accessor_and_mutation_contracts_match() -> ParityBatchCase {
    ParityBatchCase::new(
...
#[test]
fn registry_asm_blox_autoload_batch() {
    let cases: Vec<ParityBatchCase> = vec![
        asm_blox_generated_autoload_registers_entry_points_without_loading_feature(),
    ];
