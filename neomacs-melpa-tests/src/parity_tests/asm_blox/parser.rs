...
#[test]
fn asm_blox_parser_reports_precise_malformed_input_boundaries_and_messages() {
    let elisp_form = r##"(mapcar
         (lambda (source)
           (list
            source
...
fn asm_blox_problem_banned_commands_are_enforced_during_practical_cell_compilation() {
    let elisp_form = r##"(let ((asm-blox--extra-gameboard-cells
...
#[test]
fn asm_blox_branch_compiler_rejects_out_of_scope_labels_at_nested_depths() {
    let elisp_form = r##"(cl-letf
         (((symbol-function 'random)
           #'asm-blox-test-random))
         (let ((asm-blox-test-random-values
