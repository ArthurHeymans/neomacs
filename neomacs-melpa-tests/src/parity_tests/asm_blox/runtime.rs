...
#[test]
fn asm_blox_get_blocks_until_neighbor_data_arrives_then_consumes_exactly_once() {
...
fn asm_blox_board_runs_code_cells_before_custom_cells_and_captures_thrown_runtime_errors() {
    let elisp_form = r##"(let* ((events nil)
         ...
         (list
          ...
          asm-blox--gameboard-state
          asm-blox-runtime-error))"##;
    let expect = expect![[r#"OK (((:custom 2 3 (5))) error ("fixture failure" 2 3))"#]];
