...
#[test]
fn asyncloop_run_processes_a_practical_import_pipeline_once_in_order() {
...
#[test]
fn asyncloop_run_deduplicates_back_to_back_hook_invocations_before_launch() {
...
#[test]
fn asyncloop_run_deduplicates_already_scheduled_hook_invocation_and_logs_reason() {
    let elisp_form = r##"(let (events logged)
         (asyncloop-test-reset)
         (asyncloop-test-with-scheduler
           (let* ((functions
                   (list
...
#[test]
fn asyncloop_completed_loop_reuses_identity_but_runs_full_series_again() {
    let elisp_form = r##"(let (events)
         (asyncloop-test-reset)
         (asyncloop-test-with-scheduler
           (let* ((functions
...
#[test]
fn asyncloop_identity_distinguishes_behavioral_options_and_log_destination() {
...
#[test]
fn asyncloop_run_recovers_half_finished_series_and_calls_recovery_hook_first() {
    let elisp_form = r##"(let (events)
         (asyncloop-test-reset)
         (asyncloop-test-with-scheduler
           (let* ((functions
...
#[test]
fn asyncloop_recovery_hook_can_cancel_stale_transaction_instead_of_resuming() {
    let elisp_form = r##"(let (events)
         (asyncloop-test-reset)
         (asyncloop-test-with-scheduler
           (let* ((functions
             ...
             (list
              ...
              (sort
               ...
               #'<)))))"##;
    let expect = expect!["OK (((:skipped :at 0 :id 1)) (:cleanup) nil nil nil (1))"];

    assert_asyncloop_parity(elisp_form, expect);
}

#[test]
fn asyncloop_run_refuses_implicit_resume_of_a_paused_partial_series() {
    let elisp_form = r##"(let (events logged)
         (asyncloop-test-reset)
         (asyncloop-test-with-scheduler
           (let* ((functions
...
#[test]
fn asyncloop_worker_can_replace_remainder_with_a_runtime_selected_branch() {
...
fn asyncloop_reentrant_same_series_invocation_has_deterministic_single_registry_lifecycle() {
    let elisp_form = r##"(let (events functions outer-loop nested-loop)
         (asyncloop-test-reset)
         (asyncloop-test-with-scheduler
           (setq functions
                 (list
...
fn asyncloop_worker_error_preserves_current_stage_then_same_run_retries_from_start() {
    let elisp_form = r##"(let ((attempt 0)
               events
               recovered
               recovery
               functions
...
fn asyncloop_two_simultaneous_series_follow_timer_insertion_order_without_cross_talk() {
    let elisp_form = r##"(let (events loop-a loop-b)
         (asyncloop-test-reset)
         (asyncloop-test-with-scheduler
           (setq loop-a
                 (asyncloop-run
...
#[test]
fn asyncloop_with_slots_reads_and_mutates_live_struct_places_lexically() {
    let elisp_form = r##"(let ((loop
         ...
         (let ((result
           ...
           (list
            ...
            (asyncloop-scheduled loop)
            (asyncloop-remainder loop))))"##;
    let expect = expect!["OK ((t nil #1=(zero one two)) t nil #1#)"];
