    ...
    let elisp_form = r##"(let (events)
         (let ((return
           ...
           (list
            return
            (nreverse events))))"##;
    let expect = expect![[r#"OK (:chain-return ((:step "input") (:chain "input -> custom" 8)))"#]];

    assert_async1_parity(elisp_form, expect);
}

#[test]
fn async1_sequential_step_schedules_plist_work_and_advances_only_after_callback() {
    let elisp_form = r##"(let (events)
...
#[test]
fn async1_parallel_step_empty_branch_set_passes_input_through_synchronously() {
    let elisp_form = r##"(let (events)
         (list
          (async1--handle-parallel-step
           ...
           10)
          events))"##;
    let expect = expect![[r#"OK (:empty-finished (("unchanged" 11)))"#]];

    assert_async1_parity(elisp_form, expect);
}

#[test]
fn async1_parallel_step_waits_for_every_callback_and_aggregates_completion_order() {
    let elisp_form = r##"(let (callbacks chain-events)
         (let ((specs
                (mapcar
                 (lambda (label)
                   (lambda (data callback)
...
#[test]
fn async1_parallel_step_virtual_timers_make_completion_and_push_order_explicit() {
...
#[test]
fn async1_parallel_step_accepts_custom_aggregator_at_beginning_middle_or_end() {
    let elisp_form = r##"(let ((aggregator
                (lambda (results)
                  (concat
                   "["
                   (mapconcat #'identity
...
#[test]
fn async1_start_runs_a_strict_sequential_pipeline_with_cumulative_due_times() {
    let elisp_form = r##"(let (final-values)
         (async1-test-reset-scheduler)
         (cl-letf
             (((symbol-function 'run-at-time)
...
#[test]
fn async1_start_runs_a_parallel_pipeline_and_uses_reverse_completion_aggregation() {
    let elisp_form = r##"(let (final-values)
         (async1-test-reset-scheduler)
         (cl-letf
             (((symbol-function 'run-at-time)
...
#[test]
fn async1_start_runs_the_readme_mixed_parallel_and_nested_sequential_workflow() {
    let elisp_form = r##"(let (final-values)
         (async1-test-reset-scheduler)
         (cl-letf
             (((symbol-function 'run-at-time)
...
#[test]
fn async1_start_composes_custom_async_functions_with_captured_external_data() {
    let elisp_form = r##"(let ((external "context")
               events
               final-values)
         (async1-test-reset-scheduler)
         (cl-letf
...
#[test]
fn async1_start_composes_mutable_lambda_factory_steps_in_declared_order() {
    let elisp_form = r##"(let (final-values)
         (async1-test-reset-scheduler)
         (cl-letf
             (((symbol-function 'run-at-time)
    ...
    let elisp_form = r##"(let (events)
         (let ((return
           ...
           (list
            return
            events)))"##;
    let expect = expect![[r#"OK (:synchronous-finish ("already complete"))"#]];

    assert_async1_parity(elisp_form, expect);
}

#[test]
fn async1_start_without_final_callback_prints_the_exact_final_result_once() {
...
#[test]
fn async1_start_deep_tree_combines_two_nested_subchains_and_a_direct_branch() {
    let elisp_form = r##"(let (final-values)
         (async1-test-reset-scheduler)
         (cl-letf
             (((symbol-function 'run-at-time)
...
#[test]
fn async1_start_returns_the_first_step_value_and_calls_final_callback_once() {
    let elisp_form = r##"(let (events)
         (let ((return
                (async1-start
                 "seed"
...
fn async1_start_propagates_multiple_callback_invocations_through_the_remaining_pipeline() {
    let elisp_form = r##"(let (events final-values)
         (let ((return
                (async1-start
                 "seed"
                 (list
...
#[test]
fn async1_start_stops_cleanly_when_a_custom_step_never_calls_its_callback() {
    let elisp_form = r##"(let (events final-values)
         (let ((return
                (async1-start
                 "seed"
           ...
           (list
            ...
            events
            final-values)))"##;
    let expect = expect![[r#"OK (:stalled-return ((:stalled "seed")) nil)"#]];
