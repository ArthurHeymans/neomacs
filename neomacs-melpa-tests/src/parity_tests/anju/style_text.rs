...
#[test]
fn anju_org_region_styling_performs_every_supported_real_edit() {
...
#[test]
fn anju_markdown_region_styling_uses_markdown_mode_commands_in_practical_text() {
    let elisp_form = r##"(mapcar
         (lambda (case)
           (with-temp-buffer
             (markdown-mode)
...
#[test]
fn anju_style_commands_without_a_region_select_the_balanced_expression_at_point() {
...
#[test]
fn anju_style_remove_strips_org_markup_but_leaves_markdown_unchanged() {
    let elisp_form = r##"(list
         (with-temp-buffer
           (org-mode)
         ...
         (with-temp-buffer
           ...
           (anju-style-remove)
           (buffer-string)))"##;
    let expect = expect![[r#"OK ("important and /urgent/" "**important**")"#]];

    assert_anju_parity(elisp_form, expect);
}

#[test]
fn anju_style_dwim_drives_real_org_and_markdown_workflows_from_completion() {
    let elisp_form = r##"(mapcar
         (lambda (case)
           (with-temp-buffer
             (funcall (nth 0 case))
...
#[test]
fn anju_style_support_and_menu_visibility_track_real_major_modes_and_state() {
    let elisp_form = r##"(mapcar
         (lambda (mode)
           (with-temp-buffer
             (funcall mode)
