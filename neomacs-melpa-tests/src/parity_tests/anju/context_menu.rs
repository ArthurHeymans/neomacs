...
#[test]
fn anju_copy_raw_org_link_and_exported_region_drive_real_clipboard_workflows() {
    let elisp_form = r##"(let (selections messages)
...
#[test]
fn anju_elisp_context_understands_defuns_ert_tests_lambdas_and_numeric_literals() {
    let elisp_form = r##"(mapcar
         (lambda (case)
           (with-temp-buffer
             (emacs-lisp-mode)
...
#[test]
fn anju_extract_lambda_replaces_the_call_site_and_builds_an_editable_defun() {
    let elisp_form = r##"(let ((source (generate-new-buffer " *anju-source*"))
               (created nil))
         (unwind-protect
...
#[test]
fn anju_narrow_context_selects_region_defun_org_markdown_and_widen_actions() {
    let elisp_form = r##"(mapcar
         (lambda (case)
           (with-temp-buffer
             (funcall (nth 1 case))
...
#[test]
fn anju_region_context_builds_practical_plain_org_markdown_and_read_only_menus() {
    let elisp_form = r##"(mapcar
         (lambda (case)
           (with-temp-buffer
             (funcall (nth 1 case))
...
#[test]
fn anju_compilation_makefile_info_markup_and_wordcount_contexts_are_exact() {
    let elisp_form = r##"(mapcar
         (lambda (case)
           (with-temp-buffer
             (insert (nth 2 case))
...
#[test]
fn anju_context_inventory_reconfiguration_is_ordered_idempotent_and_resettable() {
    let elisp_form = r##"(let ((before-default
...
#[test]
fn anju_tty_click_distance_and_point_adjustment_follow_actual_event_positions() {
           ...
           (cl-letf (((symbol-function 'display-graphic-p)
             ...
             (list
              ...
              (progn
                ...
                (length calls))))))"##;
    let expect = expect!["OK (nil t 0 1)"];
