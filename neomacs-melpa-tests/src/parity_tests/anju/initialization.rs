...
#[test]
fn anju_init_executes_the_enabled_mouse_ui_pipeline_in_user_visible_order() {
    let elisp_form = r##"(let ((context-menu-mode nil)
...
#[test]
fn anju_init_respects_disabled_subsystems_and_existing_context_mode() {
    let elisp_form = r##"(let ((context-menu-mode t)
...
#[test]
fn anju_main_file_options_and_register_menu_reconfiguration_changes_exact_keys() {
    let elisp_form = r##"(let ((anju-file-menu-replace-make-frame-on t))
...
#[test]
fn anju_text_menu_reconfiguration_replaces_centering_with_practical_submenus() {
...
#[test]
fn anju_imenu_reconfiguration_installs_mode_specific_hooks_and_deep_org_indexing() {
    let elisp_form = r##"(let ((markdown-mode-hook nil)
...
#[test]
fn anju_help_frame_commands_dispatch_their_distinct_interactive_targets() {
    let elisp_form = r##"(let (commands)
         (cl-letf
           ...
           (list
            ...
            (nreverse commands))))"##;
    let expect = expect!["OK (info info-display-manual man (info info-display-manual man))"];

    assert_anju_parity(elisp_form, expect);
}

#[test]
fn anju_main_menu_static_submenus_preserve_every_command_and_predicate() {
