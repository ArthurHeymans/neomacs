...
#[test]
fn starts_the_clang_server_and_opens_a_session_for_a_cxx_buffer() {
    let elisp_form = r##"
...
/// The package's headline feature: typing `.' after an object runs
/// `ac-clang-async-autocomplete-autotrigger' from `ac-clang--mode-key-map',
/// which asks the server to complete at the exact line and byte column after
/// the dot.  When the reply arrives the package turns the libclang result
/// vector into auto-complete candidates — merging the two `area' overloads
/// into one candidate carrying both prototypes and both result indices — and
/// pops up the menu.
#[test]
fn completes_widget_members_after_typing_a_dot() {
    let elisp_form = r##"
        (ac-clang-test-workflow
          (progn
            (ac-clang-test-install-server)
...
/// Choosing an overloaded candidate runs ac-clang's `action', which parses the
/// libclang prototype markup into one template candidate per overload and
/// starts a second, internal completion over them.  Choosing the two argument
/// overload turns `(<#int scale#>, <#bool rounded#>)' into the yasnippet
/// `(${int scale}, ${bool rounded})' and expands it over the text
/// auto-complete just inserted, leaving the user inside a live snippet whose
/// fields they can tab through.
#[test]
fn expands_the_chosen_overload_into_a_yasnippet_argument_template() {
    let elisp_form = r##"
        (ac-clang-test-workflow
          (progn
            (ac-clang-test-install-server)
...
/// The documented manual trigger, `<tab>', completes a half typed member.  The
/// enclosing line holds non-ASCII text, which matters because ac-clang reports
/// the completion point to the server as a *byte* column and ships the buffer
/// through `decode-coding-region' into a unibyte packet, so the request pins
/// both the byte column and the raw UTF-8 source bytes.  `la' matches exactly
/// one member, so auto-complete's DWIM path completes it outright; because
/// `label()' takes no arguments there is no snippet, only the cleaned up
/// prototype echoed to the user.
#[test]
fn manual_tab_trigger_completes_a_prefix_on_a_non_ascii_line() {
    let elisp_form = r##"
        (ac-clang-test-workflow
          (progn
            (ac-clang-test-install-server)
...
/// `M-.' (`ac-clang-jump-smart') asks the server where the symbol under point
/// is defined and follows the answer into another file, pushing the old
/// location on ac-clang's jump stack.  The destination buffer has no CFLAGS of
/// its own, so `ac-clang--inherit-environment' adopts the ones from the buffer
/// the user jumped from.  `M-,' (`ac-clang-jump-back') returns to the exact
/// line and column, and doing it once more with an empty stack is a no-op.
#[test]
fn smart_jump_follows_a_definition_into_a_header_and_returns() {
    let elisp_form = r##"
        (ac-clang-test-workflow
...
/// ac-clang refuses to start rather than half starting.  With no server binary
/// on `exec-path' the user gets a warning and no process; with a binary that
/// reports a version below the required 2.0.0 the client runs `--version',
/// rejects it, and warns again; only the current server is launched.  The
/// recorded `--version' probes show that the client really executed each
/// candidate binary.
#[test]
fn refuses_to_start_when_the_server_binary_is_missing_or_too_old() {
    let elisp_form = r##"
        (ac-clang-test-workflow
          (list
           :missing (progn
...
/// When the server answers a completion with an `:Error' the client must
/// report it and leave the buffer alone instead of showing stale or empty
/// candidates — and it must stay usable: the same transaction machinery has to
/// serve the user's retry.  Here the first `.' trigger fails, the `<tab>'
/// retry from the same point succeeds, and both requests are recorded.
#[test]
fn reports_a_server_command_error_and_completes_on_the_retry() {
    let elisp_form = r##"
        (ac-clang-test-workflow
          (progn
            (ac-clang-test-install-server)
