...
pub(crate) fn assert_ac_clang_parity(form: &str, expected: Expect) {
    let name = current_test_name();
    let report = ac_clang_oracle()
        .run_value(&name, form)
        .unwrap_or_else(|error| panic!("ac-clang parity case `{name}` failed:\n{error}"));
    expected.assert_eq(&report.gnu_emacs.to_string());
}

/// Multi-probe batch for `assert_ac_clang_parity` cases (2a).
pub(crate) fn assert_ac_clang_batch(cases: &[(&str, &str, bool, Expect)]) {
    let name = current_test_name();
    assert_oracle_batch(
        ac_clang_oracle(),
        &name,
