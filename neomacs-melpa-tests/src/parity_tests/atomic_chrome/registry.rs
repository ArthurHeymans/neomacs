...
use super::{assert_atomic_chrome_autoload_parity, assert_atomic_chrome_parity};

#[test]
fn atomic_chrome_descriptor_and_archive_sources_pin_exact_melpa_payload() {
...
#[test]
fn atomic_chrome_complete_prefixed_symbol_inventory_records_every_surface() {
...
#[test]
fn atomic_chrome_customization_group_and_declared_variables_have_exact_contracts() {
...
#[test]
fn atomic_chrome_internal_state_variables_and_mode_keymap_have_exact_contracts() {
    let elisp_form = r##"(list
          (mapcar
           (lambda (symbol)
             (list
...
fn atomic_chrome_generated_autoload_preserves_feature_history_prefix_and_command_contracts() {
    ...
    assert_atomic_chrome_autoload_parity(elisp_form, expect);
}
