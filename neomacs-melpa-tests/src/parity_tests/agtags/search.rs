...
#[test]
fn agtags_find_tag_lists_the_definition_jumps_to_it_and_reports_an_unknown_tag() {
    let elisp_form = r####"
...
/// `agtags-find-file' renders GNU GLOBAL's path output in its own
/// `*agtags-path*' buffer and RET opens the file at its top.  When the same
/// user then runs a tag search, agtags' `compilation-finish-functions' hook
/// removes the stale file list, so only one result buffer is ever alive, and
/// `agtags-switch-dwim' returns to whichever survived.
#[test]
fn agtags_find_file_opens_a_listed_file_and_the_next_search_retires_the_list() {
    let elisp_form = r####"
...
/// The two documented options from the README's setup demo.  Each one adds a
/// flag to GNU GLOBAL's command line and changes which lines come back:
/// `agtags-global-treat-text' brings the plain-text design notes into a grep
/// search, and `agtags-global-ignore-case' lets an upper-case query find the
/// lower-case identifier.  The last search is for a Japanese comment, so the
/// result buffer carries GNU GLOBAL's own non-ASCII output.
#[test]
fn agtags_search_options_add_global_flags_and_change_which_lines_match() {
    let elisp_form = r####"
