...
## 109. The first-change sentinel was recorded before the boundary check that gates the point entry -- FIXED
...
## 110. `x-popup-menu` rejected the frame GNU accepts in a POSITION's window slot -- FIXED
...
## 111. `x-popup-menu` demanded a string title from a keymap MENU -- FIXED
