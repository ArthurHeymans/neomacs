...
## 97. `let` refused to bind a keyword to its own value -- FIXED
...
## 98. `end-of-file` was raised without the stream that hit it -- FIXED
...
## 99. `format` lost a format-string property that spanned into a conversion -- FIXED
...
## 100. An XML entity reference split its text node in three -- FIXED
...
## 101. The first-change undo entry recorded a constant instead of the visited-file modtime -- FIXED
