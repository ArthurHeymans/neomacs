    ...
    (while (> arg 0)
      (while (setq next (pop list))     ;Exit inner loop at undo boundary.
        ...
        (cond
         ...
         ((and (consp next) (eq (car next) t))
          (let ((time (cdr next)))
            (unlock-buffer)
            (set-buffer-modified-p nil)))
         ;; Element (nil PROP VAL BEG . END) is property change.
         ((and (consp next) (null (car next)))
          (let* ((rest (cdr next))
                 (prop (car rest))
                 (val (car (cdr rest)))
                 (beg (car (cddr rest)))
         ...
         ((and (consp next) (integerp (car next)) (integerp (cdr next)))
          (let ((beg (car next)) (end (cdr next)))
            (when (or (> (point-min) beg) (< (point-max) end))
              (error "Changes to be undone are outside visible portion of buffer"))
            (goto-char beg)
            (delete-region beg end)))
         ;; Element (apply FUN . ARGS) means call FUN to undo.
         ((and (consp next) (eq (car next) 'apply))
          (let ((fun-args (cdr next))
                (currbuff (current-buffer)))
            (if (integerp (car fun-args))
                ;; Long format: (apply DELTA START END FUN . ARGS).
                (let ((delta (car fun-args))
                      (start (car (cdr fun-args)))
                      (end (car (cddr fun-args)))
                      (fun (car (cdddr fun-args)))
                      (args (cdr (cdddr fun-args)))
         ...
         ((and (consp next) (stringp (car next)) (integerp (cdr next)))
          (let* ((string (car next))
                 (pos (cdr next))
                 (valid-marker-adjustments nil)
                 (apos (abs pos)))
         ...
         ((and (consp next) (markerp (car next)) (integerp (cdr next)))
          (let ((marker (car next)) (offset (cdr next)))
