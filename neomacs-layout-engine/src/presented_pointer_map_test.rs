use neomacs_display_protocol::{
    ...
    PresentedPointerAppearance, PresentedPointerMapError, PresentedPrimitiveKind,
};

use super::{PresentedPointerMapBuilder, RenderedPointerRun};

fn rect(x: f32, y: f32, width: f32, height: f32) -> FrameRect {
    FrameRect::new(x, y, width, height).expect("valid test rectangle")
}

fn glyph_appearance(face_id: FaceId, spans: Vec<PresentedPaintSpan>) -> PresentedPointerAppearance {
...
fn presented_pointer_map_deduplicates_appearance_shared_by_distinct_interactions() {
    ...
    let appearance = glyph_appearance(
        face_id,
...
fn presented_pointer_map_leaves_bounds_validation_to_the_install_seam() {
    ...
    assert_eq!(error, PresentedPointerMapError::RegionOutsideFrame);
