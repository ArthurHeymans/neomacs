...
#[test]
fn window_chrome_display_row_request_carries_display_row_spec() {
    ...
    assert_eq!(request.matrix_row, 3);
    assert_eq!(request.output.row, 7);
    assert_eq!(request.row_spec.role, GlyphRowRole::HeaderLine);
    assert_eq!(request.row_spec.geometry.y, 20.0);
    assert_eq!(request.row_spec.geometry.width, 160.0);
    assert_eq!(request.row_spec.base_face_id, 42);
