...
impl FontResolver {
    ...
    fn resolve_from_spec(
        ...
        let charset_ranges = crate::fontconfig::query_charset_ranges(spec, ch);
        let registry_language = spec
            .registry
            .map(resolve_sym)
            .and_then(crate::fontconfig::registry_language);
        let languages =
            crate::fontconfig::combined_query_langs(registry_language, spec.lang.map(resolve_sym));
