...
struct LegacyDisplayRowGeometryVars<'a> {
    row: &'a mut usize,
    y: &'a mut f32,
...
impl LegacyDisplayRowGeometryVars<'_> {
    fn snapshot(&self) -> LegacyDisplayRowGeometry {
    ...
    fn apply(&mut self, state: DisplayRowGeometryState) {
        *self.row = state.row;
        *self.y = state.y;
...
struct DisplayRowGeometryCommitTarget<'a> {
...
impl<'a> DisplayRowGeometryCommitTarget<'a> {
    fn silent(vars: LegacyDisplayRowGeometryVars<'a>) -> Self {
        Self {
            vars,
...
impl DisplayRowGeometryCursor {
    ...
    fn from_legacy_vars(vars: LegacyDisplayRowGeometryVars<'_>) -> Self {
        Self::from_state(DisplayRowGeometryState::from_legacy(vars.snapshot()))
    }

    fn commit(&self, mut target: DisplayRowGeometryCommitTarget<'_>) {
...
fn render_overlay_string<B: super::neovm_bridge::LayoutBufferView>(
    ...
    macro_rules! finish_overlay_string_row {
        () => {{
            let mut row_cursor =
                DisplayRowGeometryCursor::from_legacy_vars(LegacyDisplayRowGeometryVars {
                    row: &mut *row,
                    y: &mut *y,
            ...
            if selective_display > 0 && ch == '\r' {
                ...
                while byte_idx < text.len() {
                    ...
                    if skip_ch == '\n' {
                        ...
                        let mut row_cursor = DisplayRowGeometryCursor::from_legacy_vars(
                            LegacyDisplayRowGeometryVars {
                                row: &mut row,
                                y: &mut y,
                                row_extra_y: &mut row_extra_y,
            ...
            if (ch < ' ' && ch != '\t') || ch == '\x7F' {
                ...
                if x + needed_width > content_x + (text_width - lnum_pixel_width) {
                    ...
                    if params.truncate_lines {
                        ...
                        let geometry_transition = row_cursor
                            .finish_begin_and_commit_next_text_matrix_row(
                                DisplayRowGeometryAdvanceTarget::visual_wrap(
                                    DisplayRowGeometryDefaults {
                                        text_y,
                                        height: char_h,
                                        ascent: default_face_ascent,
